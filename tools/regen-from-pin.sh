...
( cd "$HERE" && python3 tools/codegen/codegen_aggregations.py \
    --input tools/codegen/aggregation-descriptor.json \
    --output-root . )

# 4. build-verify the generated operators (the in-repo containerized build)
