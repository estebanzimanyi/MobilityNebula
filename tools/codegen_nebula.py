...
def _trgeo_phys_cpp_trgeometry_geo_nad(nebula_name, meos_call, ctor_args):
    """Layout B: trgeometry_geo nad-style — no VariableSizedData cast, trgeoinst_make, nad registrar."""
    n = len(ctor_args)
    ctor = ", ".join(f"PhysicalFunction {a}" for a in ctor_args)
    pushes = "\n".join(f"    paramFns.push_back(std::move({a}));" for a in ctor_args)
    return (
...
def _trgeo_phys_cpp_geo_trgeometry_eq(nebula_name, meos_call, ctor_args):
    """Layout D: geo_trgeometry eq-style — trgeoinst_make, gs_tgt-first build order, nad registrar."""
    n = len(ctor_args)
    ctor = ", ".join(f"PhysicalFunction {a}" for a in ctor_args)
    pushes = "\n".join(f"    paramFns.push_back(std::move({a}));" for a in ctor_args)
    return (
...
def _trgeo_phys_cpp_trgeometry_trgeometry_nad(nebula_name, meos_call, ctor_args):
    """Layout G: trgeometry_trgeometry nad-style — trgeoinst_make, no cast, nad registrar."""
    n = len(ctor_args)
    ctor = ", ".join(f"PhysicalFunction {a}" for a in ctor_args)
    pushes = "\n".join(f"    paramFns.push_back(std::move({a}));" for a in ctor_args)
    return (
...
def _trgeo_phys_cpp_nad_trgeometry_geo(nebula_name, meos_call, ctor_args):
    """Layout I: nad_trgeometry_geo — double return, trgeoinst_make, nad registrar."""
    n = len(ctor_args)
    ctor = ", ".join(f"PhysicalFunction {a}" for a in ctor_args)
    pushes = "\n".join(f"    paramFns.push_back(std::move({a}));" for a in ctor_args)
    return (
...
def _trgeo_phys_cpp_nad_trgeometry_trgeometry(nebula_name, meos_call, ctor_args):
    """Layout J: nad_trgeometry_trgeometry — 10 args, double return, trgeoinst_make."""
    n = len(ctor_args)
    ctor = ", ".join(f"PhysicalFunction {a}" for a in ctor_args)
    pushes = "\n".join(f"    paramFns.push_back(std::move({a}));" for a in ctor_args)
    return (
...
def generic_fields(op):
    ...
    the input type's fields followed by one (or more) per extra arg (reversed for scalar_first).
    New multi-field extra_arg kinds expand to their constituent fields here so the
    logical HPP / physical HPP ctor signatures match the physical CPP."""
    _SCALAR_KINDS = {"scalar", "bigint_scalar", "bool_scalar", "int_scalar"}
    # primary_fields on a static_scalar op overrides the (empty) inp["fields"].
    pf = op.get("primary_fields")
    if pf and op.get("input_type") == "static_scalar":
        return [(str(n), str(t)) for n, t in pf]
    inp_fields = list(GENERIC_INPUTS[op["input_type"]]["fields"])
    extra_fields = []
    for i, ex in enumerate(op.get("extra_args", [])):
        kind = ex["kind"]
        if kind in _SCALAR_KINDS:
            cpp = ex.get("cpp", "double")
            extra_fields.append((f"arg{i}", cpp))
        elif kind in ("h3index", "quadbin", "uint64_scalar", "th3index_scalar"):
            extra_fields.append((f"arg{i}", "uint64_t"))
        elif kind == "npoint":
            extra_fields.append((f"rid{i}", "uint64_t"))
            extra_fields.append((f"frac{i}", "double"))
        elif kind == "pose":
            extra_fields.append((f"px{i}", "double"))
            extra_fields.append((f"py{i}", "double"))
            extra_fields.append((f"ptheta{i}", "double"))
        elif kind == "text_varsized":
            extra_fields.append((f"arg{i}", "VariableSizedData"))
        elif kind == "jsonb_varsized":
            extra_fields.append((f"arg{i}", "VariableSizedData"))
        elif kind == "tjsonb":
            extra_fields.append((f"json{i}", "VariableSizedData"))
            extra_fields.append((f"ts{i}", "uint64_t"))
        elif kind == "ttext":
            extra_fields.append((f"tval{i}", "VariableSizedData"))
            extra_fields.append((f"ts{i}", "uint64_t"))
        elif kind == "th3index":
            extra_fields.append((f"cell{i}", "uint64_t"))
            extra_fields.append((f"ts{i}", "uint64_t"))
        elif kind == "tquadbin":
            extra_fields.append((f"cell{i}", "uint64_t"))
            extra_fields.append((f"ts{i}", "uint64_t"))
            ...
            extra_fields.append((f"ptheta{i}", "double"))
            extra_fields.append((f"ts{i}", "uint64_t"))
        elif kind in ("geom_wkt", "geog_wkt"):
            # Second geometry arg as VARSIZED WKT
            extra_fields.append((f"arg{i}", "VariableSizedData"))
        elif kind == "raw_varsized":
            # Raw VARSIZED arg passed as ptr+size; single VARSIZED field.
            extra_fields.append((f"arg{i}", "VariableSizedData"))
        else:
            # Fallback for geom, box, wkb_temporal: single VARSIZED field
            extra_fields.append((f"arg{i}", "VariableSizedData"))
    if op.get("scalar_first"):
        return extra_fields + inp_fields
    return inp_fields + extra_fields
...
def emit_operator(op, output_root: Path):
    ...
    # If the op descriptor specifies ctor_sep_indent, use that many spaces for the
    # continuation indent (to match the committed file's alignment style); otherwise
    # fall back to the legacy fixed 58-space separator used by other shapes.
    physical_args = [{"name": a["name"] + "Function"} for a in op["args"]]
    _phys_sep_indent = op.get("ctor_sep_indent", 58)
    _phys_sep = ",\n" + " " * _phys_sep_indent
    ctor_physical_args = _phys_sep.join(
    ...
    common = {
        ...
        "zero_return": {"bool": "false", "int": "0", "double": "0.0"}.get(op.get("return_type", "int"), "0"),
        ...
        # tnumber-shape extras (only consumed by the tnumber templates).
        ...
        # Cast expressions for tint operators: double-typed lambda args are cast to int
        # before being passed to fmt::format or the MEOS call (matches committed style).
        # For tfloat operators these are just the bare variable names (no cast).
        "value_fmt_expr":     op.get("value_fmt_expr", "value"),
        "threshold_call_expr": op.get("threshold_call_expr", "threshold"),
        "scalar_call_expr":   op.get("scalar_call_expr", "d"),
        "value1_fmt_expr":    op.get("value1_fmt_expr", "v1"),
        "value2_fmt_expr":    op.get("value2_fmt_expr", "v2"),
    }
    ...
    elif op.get("build_stbox_stbox"):
        physical_cpp_path.write_text(PHYSICAL_CPP_TEMPLATE_STBOX_STBOX.format(**physical_common))
    elif op.get("build_stbox_tspatial"):
        physical_cpp_path.write_text(PHYSICAL_CPP_TEMPLATE_STBOX_TSPATIAL.format(**physical_common))
    elif op.get("build_tspatial_stbox"):
        physical_cpp_path.write_text(PHYSICAL_CPP_TEMPLATE_TSPATIAL_STBOX.format(**physical_common))
    elif op.get("build_tbox_tnumber"):
        physical_cpp_path.write_text(PHYSICAL_CPP_TEMPLATE_TBOX_TNUMBER.format(**physical_common))
    elif op.get("build_tnumber_tbox"):
        physical_cpp_path.write_text(PHYSICAL_CPP_TEMPLATE_TNUMBER_TBOX.format(**physical_common))
    elif op.get("build_tbool_point"):
        physical_cpp_path.write_text(PHYSICAL_CPP_TEMPLATE_TBOOL_POINT.format(**physical_common))
    elif op.get("build_tnumber_tnumber_wkb"):
        physical_cpp_path.write_text(PHYSICAL_CPP_TEMPLATE_TNUMBER_TNUMBER_WKB.format(**physical_common))
    elif op.get("build_two_tnumber_sep_ts"):
        physical_cpp_path.write_text(PHYSICAL_CPP_TEMPLATE_TWO_TNUMBER_SEP_TS.format(**physical_common))
    else:
        sys.stderr.write(
...
{{
    ...
    const auto result = nautilus::invoke(
        +[](double value, double threshold, uint64_t ts) -> double {{
            try {{
                MEOS::Meos::ensureMeosInitialized();
                std::string tempWkt = fmt::format("{tnumber_wkt_format}", {value_fmt_expr}, MEOS::Meos::convertEpochToTimestamp(ts));
                Temporal* temp = {tnumber_in_fn}(tempWkt.c_str());
                if (!temp) return 0.0;
                int r = {meos_call}(temp, {threshold_call_expr});
                free(temp);
    ...
    const auto result = nautilus::invoke(
        +[](double v1, double v2, uint64_t t) -> double {{
            try {{
                ...
                std::string wkt1 = fmt::format("{tnumber_wkt_format}", {value1_fmt_expr}, ts_str);
                std::string wkt2 = fmt::format("{tnumber_wkt_format}", {value2_fmt_expr}, ts_str);
                Temporal* temp1 = {tnumber_in_fn}(wkt1.c_str());
                if (!temp1) return 0.0;
                Temporal* temp2 = {tnumber_in_fn}(wkt2.c_str());
                if (!temp2) {{ free(temp1); return 0.0; }}
                int r = {meos_call}(temp1, temp2);
                free(temp1);
                free(temp2);
                return static_cast<double>(r);
            }} catch (const std::exception&) {{ return 0.0; }}
    ...
    const auto result = nautilus::invoke(
        +[](double d, double v, uint64_t t) -> double {{
            try {{
                MEOS::Meos::ensureMeosInitialized();
                std::string ts_str = MEOS::Meos::convertEpochToTimestamp(t);
                std::string wkt = fmt::format("{tnumber_wkt_format}", {value_fmt_expr}, ts_str);
                Temporal* temp = {tnumber_in_fn}(wkt.c_str());
                if (!temp) return 0.0;
                int r = {meos_call}({scalar_call_expr}, temp);
                free(temp);
                return static_cast<double>(r);
            }} catch (const std::exception&) {{ return 0.0; }}
        }},
...
# Physical .cpp template for STBox vs STBox operators — e.g. adjacent_stbox_stbox,
# nad_stbox_stbox, stbox_cmp. Both operands are VARSIZED text parsed via stbox_in().
# 2 args: box (VariableSizedData), arg0 (VariableSizedData).
PHYSICAL_CPP_TEMPLATE_STBOX_STBOX = """\
/*
    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at
...
{{
    ...
    const auto result = nautilus::invoke(
        +[](const char* boxPtr, uint32_t boxSize,
            const char* arg0Ptr, uint32_t arg0Size) -> {return_type} {{
            try
            {{
                MEOS::Meos::ensureMeosInitialized();
                ...
                if (!arg0B) {{ free(temp); return {zero_return}; }}
                {return_type} r = {meos_call}(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }}
...
# Physical .cpp template for STBox (VARSIZED) vs per-event tgeompoint operators —
# e.g. adjacent_stbox_tspatial, contains_stbox_tspatial. 4 args: lon, lat, ts, arg0
# (the STBox literal). MEOS call order: fn(stbox, temporal) — STBox FIRST.
PHYSICAL_CPP_TEMPLATE_STBOX_TSPATIAL = """\
/*
    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at
...
{{
    ...
    const auto result = nautilus::invoke(
        +[](double lon,
            ...
            {{
                MEOS::Meos::ensureMeosInitialized();
                if (!(lon >= -180.0 && lon <= 180.0 && lat >= -90.0 && lat <= 90.0)) return {zero_return};
                ...
                STBox* arg0B = stbox_in(arg0S.c_str());
                if (!arg0B) {{ free(temp); return {zero_return}; }}
                {return_type} r = {meos_call}(arg0B, temp);
                free(temp);
                free(arg0B);
                return r;
            }}
...
# Physical .cpp template for per-event tgeompoint vs STBox (VARSIZED) operators —
# e.g. adjacent_tspatial_stbox, nad_tgeo_stbox. 4 args: lon, lat, ts, arg0 (STBox).
# MEOS call order: fn(temporal, stbox) — temporal FIRST.
PHYSICAL_CPP_TEMPLATE_TSPATIAL_STBOX = """\
/*
    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at
...
# Physical .cpp template for TBox (VARSIZED) vs per-event tfloat operators —
# e.g. adjacent_tbox_tnumber. 3 args: value (double), ts (uint64_t), arg0 (TBox).
# MEOS call order: fn(tbox, temporal) — TBox FIRST. Returns bool.
PHYSICAL_CPP_TEMPLATE_TBOX_TNUMBER = """\
/*
    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at
...
# Physical .cpp template for per-event tnumber vs TBox (VARSIZED) operators —
# e.g. adjacent_tnumber_tbox, nad_tfloat_tbox, nad_tint_tbox. 3 args: value
# ({tnumber_value_cpp_type}), ts (uint64_t), arg0 (TBox). MEOS call order:
# fn(temporal, tbox) — temporal FIRST. Return type and input fn are parametric.
PHYSICAL_CPP_TEMPLATE_TNUMBER_TBOX = """\
/*
    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at
...
# Physical .cpp template for tbool single-instant operators (e.g. tbool_end_value,
# tbool_start_value). 2 args: value (bool), ts (uint64_t). Returns bool.
PHYSICAL_CPP_TEMPLATE_TBOOL_POINT = """\
/*
    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at
...
# Physical .cpp template for tnumber-vs-tnumber WKB operators — bbox predicates
# over two hex-WKB temporal numbers (e.g. adjacent_tnumber_tnumber). Both args are
# VariableSizedData parsed via temporal_from_hexwkb(). Returns bool.
PHYSICAL_CPP_TEMPLATE_TNUMBER_TNUMBER_WKB = """\
/*
    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at
...
# Physical .cpp template for two-tnumber-point operators with separate timestamps —
# e.g. nad_tfloat_tfloat, nad_tint_tint. 4 args: valueA, tsA, valueB, tsB.
# Uses {tnumber_value_cpp_type} and {tnumber_in_fn} for the per-type variant;
# {return_type}/{zero_return} for nad (double/int) vs predicate (bool) variants.
PHYSICAL_CPP_TEMPLATE_TWO_TNUMBER_SEP_TS = """\
/*
    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at
    ...
    const auto result = nautilus::invoke(
        +[]({tnumber_value_cpp_type} valueAValue, uint64_t tsAValue,
            ...
            {{
                ...
                if (!tempA) return {zero_return};
                Temporal* tempB = {tnumber_in_fn}(wktB.c_str());
                if (!tempB) {{ free(tempA); return {zero_return}; }}
...
GENERIC_INPUTS = {
    ...
    "tint": dict(fields=[("value", "int32_t"), ("ts", "uint64_t")], header="meos_geo.h", build=(
        ...
        '                if (!{var}) return {z};\n')),
    "tbool": dict(fields=[("value", "double"), ("ts", "uint64_t")], header="meos_geo.h", build=(
        '                std::string {var}Wkt = fmt::format("{{}}@{{}}", value != 0.0 ? "t" : "f", MEOS::Meos::convertEpochToTimestamp(ts));\n'
        '                Temporal* {var} = tbool_in({var}Wkt.c_str());\n'
        '                if (!{var}) return {z};\n')),
    "tbigint": dict(fields=[("value", "double"), ("ts", "uint64_t")], header="meos_geo.h", build=(
        '                std::string {var}Wkt = fmt::format("{{}}@{{}}", static_cast<int64_t>(value), MEOS::Meos::convertEpochToTimestamp(ts));\n'
    ...
    "stbox_text": dict(fields=[("box", "VariableSizedData")], header="meos_geo.h", build=(
        ...
        '                if (!{var}) return {z};\n')),
    # th3index temporal instant: 2 fields (cell:uint64, ts:uint64).
    # Constructs a Temporal* via th3indexinst_make. Requires meos_h3.h.
    "th3index": dict(fields=[("cell", "uint64_t"), ("ts", "uint64_t")], header="meos_h3.h", build=(
        '                Temporal* {var} = th3indexinst_make((H3Index)cell, (TimestampTz)ts);\n'
        '                if (!{var}) return {z};\n')),
    # tquadbin temporal instant: 2 fields (cell:uint64, ts:uint64).
    "tquadbin": dict(fields=[("cell", "uint64_t"), ("ts", "uint64_t")], header="meos_quadbin.h", build=(
        '                Temporal* {var} = (Temporal*)tquadbininst_make((Quadbin)cell, (TimestampTz)ts);\n'
        '                if (!{var}) return {z};\n')),
    # ttext temporal instant: 2 fields (value:VARSIZED text string, ts:uint64).
    # Strips outer quotes, builds WKT "'value'@ts", parses via ttext_in.
    "ttext": dict(fields=[("value", "VariableSizedData"), ("ts", "uint64_t")], header="meos.h", build=(
        '                std::string {var}Val(valuePtr, valueSize);\n'
        ...
        '                if (!{var}) return {z};\n')),
    # tjsonb temporal instant: 2 fields (json_str:VARSIZED, ts:uint64).
    # Parses json_str via jsonb_in, then constructs instant via tjsonbinst_make.
    "tjsonb": dict(fields=[("json_str", "VariableSizedData"), ("ts", "uint64_t")], header="meos_json.h", build=(
        '                std::string {var}S(json_strPtr, json_strSize);\n'
        '                Jsonb* {var}Jb = jsonb_in({var}S.c_str());\n'
        '                if (!{var}Jb) return {z};\n'
        '                Temporal* {var} = (Temporal*)tjsonbinst_make({var}Jb, (TimestampTz)ts);\n'
        '                free({var}Jb);\n'
        '                if (!{var}) return {z};\n')),
    # static_scalar: no temporal construction; the "build" step is empty.
    # All meaningful args are passed via extra_args. Primary fields must be
    # declared and the call_terms assembled by the caller via extra_args only.
    # Use primary_fields in descriptor to specify the leading fields, and
    # extra_args for each call argument.  The build string does nothing.
    # (Helper: use "static_scalar" as input_type; override primary_fields.)
    "static_scalar": dict(fields=[], header="meos.h", build=''),
    # geom_wkt: a single geometry carried as a VARSIZED WKT/EWKT field.
    # Parses via geom_in(s.c_str(), -1); result is GSERIALIZED* named "temp".
    # Freed after the call by the free(temp) in call_marshal (like other temporals).
    "geom_wkt": dict(fields=[("wkt", "VariableSizedData")], header="meos_geo.h", build=(
        '                std::string {var}S(wktPtr, wktSize);\n'
        '                GSERIALIZED* {var} = geom_in({var}S.c_str(), -1);\n'
        '                if (!{var}) return {z};\n')),
    # geog_wkt: geography variant (carried as VARSIZED WKT); parsed via geog_in.
    "geog_wkt": dict(fields=[("wkt", "VariableSizedData")], header="meos_geo.h", build=(
        '                std::string {var}S(wktPtr, wktSize);\n'
        '                GSERIALIZED* {var} = geog_in({var}S.c_str(), -1);\n'
        '                if (!{var}) return {z};\n')),
    # jsonb_wkt: a single Jsonb value as a VARSIZED JSON string.
    "jsonb_wkt": dict(fields=[("jb", "VariableSizedData")], header="meos_json.h", build=(
        '                std::string {var}S(jbPtr, jbSize);\n'
        '                Jsonb* {var} = jsonb_in({var}S.c_str());\n'
        '                if (!{var}) return {z};\n')),
    # text_wkt: a single text value as a VARSIZED string (strips quotes).
    "text_wkt": dict(fields=[("str", "VariableSizedData")], header="meos.h", build=(
        '                std::string {var}RawStr(strPtr, strSize);\n'
        ...
        '                if (!{var}) return {z};\n')),
    # intspan_wkt: an intspan as VARSIZED text; parsed via intspan_in.
    "intspan_wkt": dict(fields=[("sp", "VariableSizedData")], header="meos.h", build=(
        '                std::string {var}S(spPtr, spSize);\n'
        '                Span* {var} = intspan_in({var}S.c_str());\n'
        '                if (!{var}) return {z};\n')),
    # floatspan_wkt: a floatspan as VARSIZED text; parsed via floatspan_in.
    "floatspan_wkt": dict(fields=[("sp", "VariableSizedData")], header="meos.h", build=(
        '                std::string {var}S(spPtr, spSize);\n'
...
GENERIC_RETURNS = {
    ...
    "extract_bigint": ("double", "FLOAT64", "0.0", "tbigint_start_value"),
    ...
    # VARSIZED output kinds — these route through assemble_varsized_output,
    # not the scalar GENERIC_PHYSICAL_TEMPLATE. The zero literal "0u" means
    # an empty arena slot (zero actualLen) on error.
    # The meos_call returns char* directly (e.g. text_to_cstring, h3index_out,
    # quadbin_index_to_string, quadbin_cell_to_quadkey); copied to arena, freed.
    "varsized_cstring":  ("VariableSizedData", "VARSIZED", "0u", None),
    # The meos_call returns GSERIALIZED*; serialise via geo_as_text (WKT), copy to arena.
    "varsized_geom":     ("VariableSizedData", "VARSIZED", "0u", None),
    # The meos_call returns text*; extract via text_to_cstring, copy to arena.
    "varsized_text":     ("VariableSizedData", "VARSIZED", "0u", None),
    # The meos_call returns H3Index (uint64_t); serialise via h3index_out, copy.
    "varsized_h3out":    ("VariableSizedData", "VARSIZED", "0u", None),
    # The meos_call returns Quadbin (uint64_t); serialise via quadbin_index_to_string.
    "varsized_quadbin":  ("VariableSizedData", "VARSIZED", "0u", None),
    # The meos_call returns Temporal* (th3index) whose single-instant value is
    # an H3Index; extract via th3index_start_value, serialise via h3index_out.
    "varsized_th3index": ("VariableSizedData", "VARSIZED", "0u", None),
    # The meos_call returns Temporal* (ttext) whose single-instant value is
    # text*; extract via ttext_start_value, then text_to_cstring.
    "varsized_ttext":    ("VariableSizedData", "VARSIZED", "0u", None),
    # The meos_call returns Temporal* (tjsonb) whose single-instant value is
    # Jsonb*; extract via tjsonb_start_value, serialise via jsonb_to_cstring.
    "varsized_tjsonb":   ("VariableSizedData", "VARSIZED", "0u", None),
    # The meos_call returns uint64_t (e.g. H3Index from h3index_in).
    "uint64": ("uint64_t", "UINT64", "0ULL", None),
    # The meos_call returns text* via json_out (json_typeof, json_array_element_text, json_object_field_text).
    "varsized_json_out": ("VariableSizedData", "VARSIZED", "0u", None),
    # The meos_call returns char* from floatspan_out(Span*).
    "varsized_floatspan": ("VariableSizedData", "VARSIZED", "0u", None),
    # The meos_call returns char* from intspan_out(Span*).
    "varsized_intspan": ("VariableSizedData", "VARSIZED", "0u", None),
}
...
def _build_generic_fields_and_parse(op, inp, extras, zero):
    """Shared helper: derive (fields, headers, call_terms, parse_lines, box_frees)
    for both scalar and VARSIZED output generic operators.

    When the descriptor has ``primary_fields`` (a list of [varname, cpptype] pairs)
    on a static_scalar op, those fields are used directly as the lambda parameters
    and ``meos_call`` is treated as a complete expression (not appended with
    ``(callargs)``).  In that case the returned call_terms list is left empty and
    the caller detects ``op.get("primary_fields")`` to emit the call expression
    verbatim."""
    is_static = (op.get("input_type") == "static_scalar")
    pf = op.get("primary_fields")  # list of [varname, cpptype] or None

    if pf and is_static:
        # primary_fields defines the lambda args directly; meos_call is a verbatim
        # expression (already contains casts + varnames).  We add the fields to the
        # parameter list so the ctor/HPP/registrar signatures are correct, but we
        # set call_terms to None to signal "emit meos_call as-is".
        fields = [(str(name), str(ctype)) for name, ctype in pf]
        headers = {"meos.h", inp["header"]}
        for h in op.get("extra_headers", []):
            headers.add(h)
        call_terms = None  # sentinel: caller emits op['meos_call'] verbatim
        parse_lines = []
        box_frees = []
        return fields, headers, call_terms, parse_lines, box_frees
    ...
    # For static_scalar input, the primary call term is just the extra args;
    # there is no "temp" temporal object.
    call_terms = [] if is_static else ["temp"]
    parse_lines = []
    box_frees = []
    ...
    for i, ex in enumerate(extras):
        ...
        if kind == "scalar":
            ...
            free_parent = "temp" if not is_static else None
            fp = f"free({free_parent}); " if free_parent else ""
            parse_lines.append(
        ...
        elif kind == "bigint_scalar":
            fields.append((f"arg{i}", "double"))
            call_terms.append(f"static_cast<int64_t>(arg{i})")
        elif kind == "bool_scalar":
            fields.append((f"arg{i}", "double"))
            call_terms.append(f"static_cast<bool>(arg{i} != 0.0)")
        elif kind == "int_scalar":
            fields.append((f"arg{i}", "double"))
            call_terms.append(f"static_cast<int>(arg{i})")
        elif kind == "uint64_scalar":
            # uint64_t argument exposed as uint64_t at Nautilus level (e.g. H3Index, Quadbin)
            fields.append((f"arg{i}", "uint64_t"))
            cast = ex.get("cast", "")
            call_terms.append(f"({cast})arg{i}" if cast else f"arg{i}")
        elif kind == "h3index":
            # H3Index extra argument: uint64_t field, cast to (H3Index) in call
            fields.append((f"arg{i}", "uint64_t"))
            call_terms.append(f"(H3Index)arg{i}")
            headers.add("meos_h3.h")
        elif kind == "quadbin":
            # Quadbin extra argument: uint64_t field, cast to (Quadbin) in call
            fields.append((f"arg{i}", "uint64_t"))
            call_terms.append(f"(Quadbin)arg{i}")
            headers.add("meos_quadbin.h")
        elif kind == "npoint":
            # Static Npoint constructed from (rid:uint64, frac:double) extra fields
            fields.append((f"rid{i}", "uint64_t"))
            fields.append((f"frac{i}", "double"))
            free_parent = "temp" if not is_static else None
            fp = f"free({free_parent}); " if free_parent else ""
            headers.add("meos_npoint.h")
            parse_lines.append(
                f'                Npoint* np{i} = npoint_make((int64_t)rid{i}, frac{i});\n'
                f'                if (!np{i}) {{ {fp}return {zero}; }}\n')
            call_terms.append(f"np{i}")
            box_frees.append(f"free(np{i});")
        elif kind == "pose":
            # Static Pose constructed from (x:double, y:double, theta:double) extra fields
            fields.append((f"px{i}", "double"))
            fields.append((f"py{i}", "double"))
            fields.append((f"ptheta{i}", "double"))
            free_parent = "temp" if not is_static else None
            fp = f"free({free_parent}); " if free_parent else ""
            headers.add("meos_pose.h")
            parse_lines.append(
                f'                Pose* pose{i} = pose_make_2d(px{i}, py{i}, ptheta{i}, false, 0);\n'
                f'                if (!pose{i}) {{ {fp}return {zero}; }}\n')
            call_terms.append(f"pose{i}")
            box_frees.append(f"free(pose{i});")
        elif kind == "text_varsized":
            # text* constructed from a VARSIZED cstring (strips outer quotes)
            fields.append((f"arg{i}", "VariableSizedData"))
            free_parent = "temp" if not is_static else None
            fp = f"free({free_parent}); " if free_parent else ""
            parse_lines.append(
            ...
            call_terms.append(f"txt{i}")
            box_frees.append(f"free(txt{i});")
        elif kind == "jsonb_varsized":
            # Jsonb* constructed from a VARSIZED JSON string
            fields.append((f"arg{i}", "VariableSizedData"))
            free_parent = "temp" if not is_static else None
            fp = f"free({free_parent}); " if free_parent else ""
            headers.add("meos_json.h")
            parse_lines.append(
                f'                std::string arg{i}S(arg{i}Ptr, arg{i}Size);\n'
                f'                Jsonb* jb{i} = jsonb_in(arg{i}S.c_str());\n'
                f'                if (!jb{i}) {{ {fp}return {zero}; }}\n')
            call_terms.append(f"jb{i}")
            box_frees.append(f"free(jb{i});")
        elif kind == "tjsonb":
            # Second tjsonb temporal: (json:VARSIZED, ts:uint64) extra fields
            fields.append((f"json{i}", "VariableSizedData"))
            fields.append((f"ts{i}", "uint64_t"))
            free_parent = "temp" if not is_static else None
            fp = f"free({free_parent}); " if free_parent else ""
            headers.add("meos_json.h")
            parse_lines.append(
                f'                std::string json{i}S(json{i}Ptr, json{i}Size);\n'
            ...
            call_terms.append(f"inst{i}")
            box_frees.append(f"free(inst{i});")
        elif kind == "ttext":
            # Second ttext temporal: (value:VARSIZED, ts:uint64) extra fields
            fields.append((f"tval{i}", "VariableSizedData"))
            fields.append((f"ts{i}", "uint64_t"))
            free_parent = "temp" if not is_static else None
            fp = f"free({free_parent}); " if free_parent else ""
            parse_lines.append(
                f'                std::string tval{i}S(tval{i}Ptr, tval{i}Size);\n'
            ...
            # Second th3index temporal: (cell:uint64, ts:uint64) extra fields
            fields.append((f"cell{i}", "uint64_t"))
            fields.append((f"ts{i}", "uint64_t"))
            free_parent = "temp" if not is_static else None
            fp = f"free({free_parent}); " if free_parent else ""
            headers.add("meos_h3.h")
            parse_lines.append(
                f'                Temporal* inst{i} = th3indexinst_make((H3Index)cell{i}, (TimestampTz)ts{i});\n'
                f'                if (!inst{i}) {{ {fp}return {zero}; }}\n')
            call_terms.append(f"inst{i}")
            box_frees.append(f"free(inst{i});")
        elif kind == "tquadbin":
            # Second tquadbin temporal: (cell:uint64, ts:uint64) extra fields
            fields.append((f"cell{i}", "uint64_t"))
            fields.append((f"ts{i}", "uint64_t"))
            free_parent = "temp" if not is_static else None
            fp = f"free({free_parent}); " if free_parent else ""
            headers.add("meos_quadbin.h")
            parse_lines.append(
                f'                Temporal* inst{i} = (Temporal*)tquadbininst_make((Quadbin)cell{i}, (TimestampTz)ts{i});\n'
                f'                if (!inst{i}) {{ {fp}return {zero}; }}\n')
            call_terms.append(f"inst{i}")
            box_frees.append(f"free(inst{i});")
        elif kind == "tnpoint":
            # Second tnpoint temporal: (rid:uint64, frac:double, ts:uint64) extra fields
            fields.append((f"rid{i}", "uint64_t"))
            fields.append((f"frac{i}", "double"))
            fields.append((f"ts{i}", "uint64_t"))
            free_parent = "temp" if not is_static else None
            ...
            # Second tpose temporal: (x:double, y:double, theta:double, ts:uint64)
            fields.append((f"px{i}", "double"))
            fields.append((f"py{i}", "double"))
            fields.append((f"ptheta{i}", "double"))
            fields.append((f"ts{i}", "uint64_t"))
            ...
            headers.add("meos_pose.h")
            parse_lines.append(
                f'                Pose* pose{i}A = pose_make_2d(px{i}, py{i}, ptheta{i}, false, 0);\n'
            ...
            # Extra geometry arg: VARSIZED WKT parsed via geom_in
            fields.append((f"arg{i}", "VariableSizedData"))
            headers.add("meos_geo.h")
            free_parent = "temp" if not is_static else None
            fp = f"free({free_parent}); " if free_parent else ""
            ...
            call_terms.append(f"gs{i}")
            box_frees.append(f"free(gs{i});")
        elif kind == "geog_wkt":
            # Extra geography arg: VARSIZED WKT parsed via geog_in
            fields.append((f"arg{i}", "VariableSizedData"))
            headers.add("meos_geo.h")
            free_parent = "temp" if not is_static else None
            fp = f"free({free_parent}); " if free_parent else ""
            ...
            # Raw VARSIZED arg: passed as (const char* ptr, uint32_t size) to the lambda.
            # No parsing — the meos_call uses the ptr/size directly via std::string or c_str().
            fields.append((f"arg{i}", "VariableSizedData"))
            call_terms.append(f"arg{i}Ptr, arg{i}Size")

    return fields, headers, call_terms, parse_lines, box_frees


def _build_casts_lparams_invoke(fields):
    """Build (casts, lparams, invoke) lists from ordered (name, cpp_type) field pairs."""
    casts, lparams, invoke = [], [], []
    for idx, (fn, cpp) in enumerate(fields):
        if cpp == "VariableSizedData":
            ...
            invoke.append(fn)
    return casts, lparams, invoke


def assemble_varsized_output(op, inp, fields, headers, call_terms, parse_lines, box_frees):
    """Build physical .cpp for a build_generic op that returns a VARSIZED arena value.
    The nautilus::invoke lambda receives an extra (char* buf, uint32_t bufMax) pair
    and returns uint32_t actualLen; the arena write uses VarVal(actualLen).writeToMemory."""
    name = op["nebula_name"]
    is_static = (op.get("input_type") == "static_scalar")
    zero = "0u"
    rk = op["return_kind"]

    casts, lparams, invoke = _build_casts_lparams_invoke(fields)
    n_args = len(fields)

    # Add the arena buf/bufMax pair to lambda params and invoke args.
    lparams_with_buf = lparams + ["char* buf", "uint32_t bufMax"]
    invoke_with_buf = invoke + ["outBuf.getContent()", "nautilus::val<uint32_t>(MAX_LEN)"]

    # When meos_call already contains '(' it is a verbatim expression; emit directly.
    # call_terms == None also signals the primary_fields verbatim path.
    meos_call_str = op['meos_call']
    if call_terms is None or '(' in meos_call_str:
        callexpr = meos_call_str
    else:
        callargs = ", ".join(call_terms)
        callexpr = f"{meos_call_str}({callargs})"
    bf = "".join(f"                {x}\n" for x in box_frees)
    build_str = inp["build"].format(var="temp", z=zero) if not is_static else ""
    build_str += "".join(parse_lines)

    # Free the primary temporal (if any) before and after the MEOS call.
    free_temp = "" if is_static else "                free(temp);\n"

    if rk == "varsized_cstring":
        # callexpr → char* out; copy to buf, free out
        call_marshal = (
            f"                char* out = {callexpr};\n"
            f"{free_temp}"
            f"{bf}"
            f"                if (!out) return 0u;\n"
            f"                uint32_t len = static_cast<uint32_t>(strlen(out));\n"
            f"                if (len > bufMax) len = bufMax;\n"
            f"                memcpy(buf, out, len);\n"
            f"                free(out);\n"
            f"                return len;")
    elif rk == "varsized_text":
        # callexpr → text*; text_to_cstring → char* → copy → free
        call_marshal = (
            f"                text* tres = {callexpr};\n"
            f"{free_temp}"
            f"{bf}"
            f"                if (!tres) return 0u;\n"
            ...
            f"                free(out);\n"
            f"                return len;")
    elif rk == "varsized_h3out":
        # callexpr → H3Index (uint64_t); h3index_out → char* hex → copy
        call_marshal = (
            f"                H3Index hcell = {callexpr};\n"
            f"{free_temp}"
            f"{bf}"
            f"                if (hcell == 0) return 0u;\n"
            f"                char* hex = h3index_out(hcell);\n"
            f"                if (!hex) return 0u;\n"
            f"                uint32_t len = static_cast<uint32_t>(strlen(hex));\n"
            f"                if (len > bufMax) len = bufMax;\n"
            f"                memcpy(buf, hex, len);\n"
            f"                free(hex);\n"
            f"                return len;")
    elif rk == "varsized_quadbin":
        # callexpr → Quadbin (uint64_t); quadbin_index_to_string → char* → copy
        call_marshal = (
            f"                Quadbin qcell = {callexpr};\n"
            f"{free_temp}"
            f"{bf}"
            f"                char* qstr = quadbin_index_to_string(qcell);\n"
            f"                if (!qstr) return 0u;\n"
            f"                uint32_t len = static_cast<uint32_t>(strlen(qstr));\n"
            f"                if (len > bufMax) len = bufMax;\n"
            f"                memcpy(buf, qstr, len);\n"
            f"                free(qstr);\n"
            f"                return len;")
    elif rk == "varsized_th3index":
        # callexpr → Temporal*(th3index); th3index_start_value → H3Index; h3index_out → char*
        call_marshal = (
            f"                Temporal* tres = {callexpr};\n"
            f"{free_temp}"
            f"{bf}"
            f"                if (!tres) return 0u;\n"
            ...
            f"                free(hex);\n"
            f"                return len;")
    elif rk == "varsized_ttext":
        # callexpr → Temporal*(ttext); ttext_start_value → text*; text_to_cstring → char*
        call_marshal = (
            f"                Temporal* tres = {callexpr};\n"
            f"{free_temp}"
            f"{bf}"
            ...
            f"                free(out);\n"
            f"                return len;")
    elif rk == "varsized_tjsonb":
        # callexpr → Temporal*(tjsonb); tjsonb_start_value → Jsonb*; jsonb_to_cstring → char*
        call_marshal = (
            f"                Temporal* tres = {callexpr};\n"
            f"{free_temp}"
            f"{bf}"
            ...
            f"                free(out);\n"
            f"                return len;")
    elif rk == "varsized_geom":
        # callexpr → GSERIALIZED*; geo_as_text → char* WKT → copy to arena
        call_marshal = (
            f"                GSERIALIZED* gres = {callexpr};\n"
            f"{free_temp}"
            f"{bf}"
            f"                if (!gres) return 0u;\n"
            ...
            f"                free(out);\n"
            f"                return len;")
    elif rk == "varsized_json_out":
        # callexpr → text* (from json_typeof etc.); json_out → char* cstring → copy
        call_marshal = (
            f"                text* tres = {callexpr};\n"
            f"{free_temp}"
            f"{bf}"
            ...
            f"                free(out);\n"
            f"                return len;")
    elif rk == "varsized_floatspan":
        # callexpr → char* from floatspan_out(Span*) → copy to arena
        call_marshal = (
            f"                char* out = {callexpr};\n"
            f"{free_temp}"
            f"{bf}"
            ...
            f"                free(out);\n"
            f"                return len;")
    elif rk == "varsized_intspan":
        # callexpr → char* from intspan_out(Span*) → copy to arena
        call_marshal = (
            f"                char* out = {callexpr};\n"
            f"{free_temp}"
            f"{bf}"
        ...
        raise ValueError(f"Unknown varsized return_kind: {rk}")
    ...
    return GENERIC_PHYSICAL_VARSIZED_TEMPLATE.format(
        nebula_name=name, includes=inc,
        ctor_physical_args=physical_args, n_args=n_args, ctor_physical_pushes=pushes,
        casts="\n".join(casts),
        lambda_params=",\n            ".join(lparams_with_buf),
        build=build_str, call_marshal=call_marshal,
        invoke_args=", ".join(invoke_with_buf),
        registrar_pushes=registrar)
...
def assemble_generic_physical(op):
    ...
    fields, headers, call_terms, parse_lines, box_frees = _build_generic_fields_and_parse(
        op, inp, extras, zero)

    # VARSIZED output path — arena allocation instead of VarVal(scalar).
    if op["return_kind"].startswith("varsized_"):
        # scalar_first/box_first reordering not currently needed for varsized ops
        return assemble_varsized_output(op, inp, fields, headers, call_terms, parse_lines, box_frees)

    casts, lparams, invoke = _build_casts_lparams_invoke(fields)
    n_args = len(fields)

    build = inp["build"].format(var="temp", z=zero) + "".join(parse_lines)
    # static_scalar: build string is empty; call_terms has no "temp" sentinel.
    is_static = (op.get("input_type") == "static_scalar")
    if is_static:
        build = "".join(parse_lines)
    ...
    if op.get("scalar_first"):
        # For f(scalar, Temporal*) MEOS calls: the SQL args put scalar first,
        # then temporal (value+ts). Reorder so extra arg fields precede the temporal fields.
        n_temp_fields = len(inp["fields"])
        extra_fields = fields[n_temp_fields:]
        temp_fields = fields[:n_temp_fields]
        fields = extra_fields + temp_fields
        # call_terms = ["temp", extra_call_terms...] -> [extra_call_terms..., "temp"]
        if not is_static:
            call_terms = call_terms[1:] + ["temp"]
        # Rebuild casts, lparams, invoke from the reordered fields.
        casts, lparams, invoke = _build_casts_lparams_invoke(fields)
        n_args = len(fields)

    # When meos_call already contains '(' it is a verbatim expression (casts and
    # varnames already embedded); emit it directly without appending (callargs).
    # call_terms == None also signals the primary_fields verbatim path.
    meos_call_str = op['meos_call']
    if call_terms is None or '(' in meos_call_str:
        callexpr = meos_call_str
    ...
    free_temp = "" if is_static else f"                free(temp);\n"

    if extract_fn is None:
        if is_static:
            call_marshal = (f"                {ret_cpp} r = {callexpr};\n"
                            f"{bf}"
                            f"                return r;")
        else:
            call_marshal = (f"                {ret_cpp} r = {callexpr};\n"
        ...
        call_marshal = (f"                Temporal* res = {callexpr};\n"
                        f"{free_temp}"
                        f"{bf}"
...
GENERIC_PHYSICAL_VARSIZED_TEMPLATE = """\
/*
    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at
...
# 2-arg shape: two VariableSizedData blobs (stbox+stbox, traj+traj, etc.)
DISPATCH_CASE_TWO_VARIABLESIZED = """\
        /* BEGIN CODEGEN PARSER GLUE: {sql_token} */
        case AntlrSQLLexer::{sql_token}:
        {{
            const auto argCount = context->expression().size();
            if (argCount != 2)
                throw InvalidQuerySyntax("{sql_token} requires exactly 2 arguments (arg0, arg1), but got {{}}", argCount);

            /* Lift constant blobs into function builder */
            while (!helpers.top().constantBuilder.empty())
            {{
                auto v = std::move(helpers.top().constantBuilder.back());
                helpers.top().constantBuilder.pop_back();
            ...
            auto arg1 = helpers.top().functionBuilder.back(); helpers.top().functionBuilder.pop_back();
            auto arg0 = helpers.top().functionBuilder.back(); helpers.top().functionBuilder.pop_back();

            helpers.top().functionBuilder.emplace_back(
                {nebula_name}LogicalFunction(arg0, arg1));
        }}
        break;
        /* END CODEGEN PARSER GLUE: {sql_token} */
"""

# 2-arg shape: bool value + uint64_t timestamp (tbool instant).
DISPATCH_CASE_TBOOL_POINT = """\
        /* BEGIN CODEGEN PARSER GLUE: {sql_token} */
        case AntlrSQLLexer::{sql_token}:
        {{
            const auto argCount = context->expression().size();
            if (argCount != 2)
                throw InvalidQuerySyntax("{sql_token} requires exactly 2 arguments (value, timestamp), but got {{}}", argCount);

            auto timestamp = helpers.top().functionBuilder.back(); helpers.top().functionBuilder.pop_back();
            auto value     = helpers.top().functionBuilder.back(); helpers.top().functionBuilder.pop_back();

            helpers.top().functionBuilder.emplace_back(
                {nebula_name}LogicalFunction(value, timestamp));
        }}
        break;
        /* END CODEGEN PARSER GLUE: {sql_token} */
"""
...
def dispatch_case_for(op):
    ...
    if op.get("build_two_tnumber_points") or op.get("build_two_tnumber_sep_ts"):
        # sep-ts variant uses separate tsA/tsB — same 4-arg SQL dispatch shape.
        return DISPATCH_CASE_TWO_TNUMBER_POINTS
    # stbox/tspatial and tspatial/stbox: 4 args (lon, lat, ts, box_blob) —
    # same arity as ONE_TEMPORAL_POINT (lon, lat, timestamp, geometry).
    if op.get("build_stbox_tspatial") or op.get("build_tspatial_stbox"):
        return DISPATCH_CASE_ONE_TEMPORAL_POINT
    # tbox/tnumber and tnumber/tbox: 3 args (value, ts, box_blob) —
    # same arity as TNUMBER_POINT_WITH_SCALAR (value, timestamp, scalar).
    if op.get("build_tbox_tnumber") or op.get("build_tnumber_tbox"):
        return DISPATCH_CASE_TNUMBER_POINT_WITH_SCALAR
    # stbox+stbox and tnumber+tnumber (hexwkb): 2 VariableSizedData blobs.
    if op.get("build_stbox_stbox") or op.get("build_tnumber_tnumber_wkb"):
        return DISPATCH_CASE_TWO_VARIABLESIZED
    # tbool instant: 2 args (bool value, uint64_t timestamp).
    if op.get("build_tbool_point"):
        return DISPATCH_CASE_TBOOL_POINT
    return None
...
def inject_parser_cpp(operators, cpp_path: Path) -> int:
    ...
    for op in operators:
        ...
        if re.search(rf"case\s+AntlrSQL(?:Lexer|Parser)::{re.escape(op['sql_token'])}\s*:", body):
            sys.stderr.write(
