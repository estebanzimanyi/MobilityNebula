...
def assemble_wkb_output_stbox_border(op):
    """Physical .cpp for a per-event op f(Temporal*, STBox*, bool) -> Temporal* over one
    hex-WKB VARSIZED temporal operand, a static STBox (WKT/text VARSIZED literal), and a
    border-inclusion flag, emitting the temporal result as a hex-WKB VARSIZED field — the
    temporal-restriction-to-an-stbox round-trip. The STBox is parsed with stbox_in
    (leading/trailing quotes stripped) and freed after the call, matching the box extra
    of the scalar-return generic path."""
    name = op["nebula_name"]
    headers = {"meos.h", "meos_geo.h"}
    for h in op.get("extra_headers", []):
        headers.add(h)
    inc = "\n".join(f"#include <{h}>" for h in
                    ["meos.h"] + sorted(h for h in headers if h != "meos.h"))
    return GENERIC_PHYSICAL_WKB_STBOX_BORDER_TEMPLATE.format(
        nebula_name=name, includes=inc, meos_call=op["meos_call"])
...
def assemble_generic_physical(op):
    ...
    if op["return_kind"] == "wkb":
        if op.get("wkb_stbox_border"):
            return assemble_wkb_output_stbox_border(op)
...
GENERIC_PHYSICAL_WKB_STBOX_BORDER_TEMPLATE = """\
/*
    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at
    ...
    auto hexStr = nautilus::invoke(
        ...
        {{
            ...
            {{
                ...
                Temporal* res = {meos_call}(a, box, borderInc);
                free(a);
                free(box);
...
{{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "{nebula_name}PhysicalFunction requires 3 children but got {{}}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return {nebula_name}PhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}}
