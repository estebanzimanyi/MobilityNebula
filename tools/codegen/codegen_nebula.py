...
def assemble_wkb_output_geom(op):
    """Physical .cpp for a per-event op f(Temporal*, GSERIALIZED*) -> Temporal* over one
    hex-WKB VARSIZED temporal operand and one static geometry (WKT/text VARSIZED literal),
    emitting the temporal result as a hex-WKB VARSIZED field — the temporal-vs-geometry
    transform round-trip (restrict / difference / project a temporal spatial value to a
    geometry). The geometry is parsed with the StaticGeometry RAII wrapper (leading/
    trailing quotes stripped), matching the scalar-return generic path."""
    name = op["nebula_name"]
    headers = {"meos.h", "meos_geo.h"}
    for h in op.get("extra_headers", []):
        headers.add(h)
    inc = "\n".join(f"#include <{h}>" for h in
                    ["meos.h"] + sorted(h for h in headers if h != "meos.h"))
    return GENERIC_PHYSICAL_WKB_GEOM_TEMPLATE.format(
        nebula_name=name, includes=inc, meos_call=op["meos_call"])
...
def assemble_generic_physical(op):
    ...
    if op["return_kind"] == "wkb":
        ...
        if len(extras) == 1 and extras[0]["kind"] == "geom":
            return assemble_wkb_output_geom(op)
...
GENERIC_PHYSICAL_WKB_GEOM_TEMPLATE = """\
/*
    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at
    ...
    auto hexStr = nautilus::invoke(
        ...
        {{
            ...
            {{
                ...
                Temporal* res = {meos_call}(a, g.getGeometry());
                free(a);
