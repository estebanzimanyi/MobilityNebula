...
def cmp_scalar_tempfirst(fn, ret, args):
    ...
    name = pascal(fn)
    # CPP ctor continuation aligns 'P' of PhysicalFunction with the first arg:
    # 'NamePhysicalFunction::NamePhysicalFunction(' has len L, cont = L+1.
    cpp_ctor_prefix = f"{name}PhysicalFunction::{name}PhysicalFunction("
    ctor_sep_indent = len(cpp_ctor_prefix) + 1
    return {
        "nebula_name": name,
        "sql_token": fn.upper(),
        "meos_call": fn,
        "args": [
            ...
            {"name": "threshold", "nautilus_type": vcpp,       "cpp_type": vcpp},
            {"name": "ts",        "nautilus_type": "uint64_t", "cpp_type": "uint64_t"},
        ],
        "return_type": "int",
        ...
        "tnumber_in_fn": in_fn,
        "ctor_sep_indent": ctor_sep_indent,
        # tint lambda args are typed double; cast to int for fmt and MEOS call.
        **({"value_fmt_expr": "static_cast<int>(value)",
            "threshold_call_expr": "static_cast<int>(threshold)"} if base == "tint" else {}),
        "comment_one_liner": (
...
def cmp_scalar_scalarfirst(fn, ret, args):
    ...
    return {
        ...
        "tnumber_wkt_format": wkt,
        "tnumber_in_fn": in_fn,
        # CPP ctor continuation indent is fixed per base type (matches committed W2-W145 style).
        "ctor_sep_indent": 70 if base == "tfloat" else 61,
        # Lambda params are named (d, v); value_fmt_expr uses those names.
        # For tint: cast to int for fmt and MEOS call; for tfloat: use bare lambda param names.
        "value_fmt_expr":  "static_cast<int>(v)" if base == "tint" else "v",
        "scalar_call_expr": "static_cast<int>(d)" if base == "tint" else "d",
        "comment_one_liner": (
...
def cmp_two_temporal(fn, ret, args):
    ...
    (value1,value2,ts) — both instants share the same timestamp — and reuses
    build_two_tnumber_points."""
    ...
    return {
        ...
        "meos_call": fn,
        "args": [
            {"name": "value1", "nautilus_type": vcpp,       "cpp_type": vcpp},
            {"name": "value2", "nautilus_type": vcpp,       "cpp_type": vcpp},
            {"name": "ts",     "nautilus_type": "uint64_t", "cpp_type": "uint64_t"},
        ],
        "return_type": "int",
        "nautilus_return": "INT32",
        ...
        "tnumber_wkt_format": wkt,
        "tnumber_in_fn": in_fn,
        # CPP ctor continuation indent is fixed at 80 for all two-temporal tfloat ops
        # (matches committed W2-W145 style).
        "ctor_sep_indent": 80,
        "comment_one_liner": (
            f"Per-event {fn.split('_')[0]} comparison between two single-instant "
            f"temporals (built from value1/value2 at a shared timestamp ts)."),
    }
