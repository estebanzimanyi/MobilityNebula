...
TpointGetXAggregationLogicalFunction::TpointGetXAggregationLogicalFunction(
...
TpointGetXAggregationLogicalFunction::create(
    const FieldAccessLogicalFunction& lonField,
    const FieldAccessLogicalFunction& latField,
    const FieldAccessLogicalFunction& timestampField)
{
    return std::make_shared<TpointGetXAggregationLogicalFunction>(lonField, latField, timestampField, lonField);
}

std::string_view TpointGetXAggregationLogicalFunction::getName() const noexcept
{
    return NAME;
}

void TpointGetXAggregationLogicalFunction::inferStamp(const Schema& schema)
{
    ...
    {
        throw CannotInferSchema("TpointGetXAggregationLogicalFunction: lon, lat, and timestamp fields must be numeric.");
    }
...
NES::SerializableAggregationFunction TpointGetXAggregationLogicalFunction::serialize() const
{
...
AggregationLogicalFunctionRegistryReturnType AggregationLogicalFunctionGeneratedRegistrar::RegisterTpointGetXAggregationLogicalFunction(
    AggregationLogicalFunctionRegistryArguments arguments)
{
    if (arguments.fields.size() == 4)
    {
        auto ptr = std::make_shared<TpointGetXAggregationLogicalFunction>(
            arguments.fields[0], arguments.fields[1], arguments.fields[2], arguments.fields[3]);
        return ptr;
    }
    throw CannotDeserialize(
        "TpointGetXAggregationLogicalFunction requires lon, lat, timestamp, and alias fields but got {}",
