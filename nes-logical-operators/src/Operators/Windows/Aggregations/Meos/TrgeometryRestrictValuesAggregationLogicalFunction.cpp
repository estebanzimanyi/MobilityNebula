...
TrgeometryRestrictValuesAggregationLogicalFunction::TrgeometryRestrictValuesAggregationLogicalFunction(
...
TrgeometryRestrictValuesAggregationLogicalFunction::create(
    const FieldAccessLogicalFunction& xField,
    const FieldAccessLogicalFunction& yField,
    const FieldAccessLogicalFunction& thetaField,
    const FieldAccessLogicalFunction& timestampField,
    std::vector<std::string> constArgs)
{
    return std::make_shared<TrgeometryRestrictValuesAggregationLogicalFunction>(xField, yField, thetaField, timestampField, xField, std::move(constArgs));
}

std::string_view TrgeometryRestrictValuesAggregationLogicalFunction::getName() const noexcept
{
    return NAME;
}

void TrgeometryRestrictValuesAggregationLogicalFunction::inferStamp(const Schema& schema)
{
    ...
    {
        throw CannotInferSchema("TrgeometryRestrictValuesAggregationLogicalFunction: x, y, theta, and timestamp fields must be numeric.");
    }
...
NES::SerializableAggregationFunction TrgeometryRestrictValuesAggregationLogicalFunction::serialize() const
{
...
AggregationLogicalFunctionRegistryReturnType AggregationLogicalFunctionGeneratedRegistrar::RegisterTrgeometryRestrictValuesAggregationLogicalFunction(
    AggregationLogicalFunctionRegistryArguments arguments)
{
    if (arguments.fields.size() == 5)
    {
        auto ptr = std::make_shared<TrgeometryRestrictValuesAggregationLogicalFunction>(
            arguments.fields[0], arguments.fields[1], arguments.fields[2], arguments.fields[3], arguments.fields[4],
            arguments.literals);
        return ptr;
    }
    throw CannotDeserialize(
        "TrgeometryRestrictValuesAggregationLogicalFunction requires x, y, theta, timestamp, and alias fields but got {}",
