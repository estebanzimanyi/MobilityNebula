...
TpointAngularDifferenceExpAggregationLogicalFunction::TpointAngularDifferenceExpAggregationLogicalFunction(
...
TpointAngularDifferenceExpAggregationLogicalFunction::create(
    const FieldAccessLogicalFunction& lonField,
    const FieldAccessLogicalFunction& latField,
    const FieldAccessLogicalFunction& timestampField)
{
    return std::make_shared<TpointAngularDifferenceExpAggregationLogicalFunction>(lonField, latField, timestampField, lonField);
}

std::string_view TpointAngularDifferenceExpAggregationLogicalFunction::getName() const noexcept
{
    return NAME;
}

void TpointAngularDifferenceExpAggregationLogicalFunction::inferStamp(const Schema& schema)
{
    ...
    {
        throw CannotInferSchema("TpointAngularDifferenceExpAggregationLogicalFunction: lon, lat, and timestamp fields must be numeric.");
    }
...
NES::SerializableAggregationFunction TpointAngularDifferenceExpAggregationLogicalFunction::serialize() const
{
...
AggregationLogicalFunctionRegistryReturnType AggregationLogicalFunctionGeneratedRegistrar::RegisterTpointAngularDifferenceExpAggregationLogicalFunction(
    AggregationLogicalFunctionRegistryArguments arguments)
{
    if (arguments.fields.size() == 4)
    {
        auto ptr = std::make_shared<TpointAngularDifferenceExpAggregationLogicalFunction>(
            arguments.fields[0], arguments.fields[1], arguments.fields[2], arguments.fields[3]);
        return ptr;
    }
    throw CannotDeserialize(
        "TpointAngularDifferenceExpAggregationLogicalFunction requires lon, lat, timestamp, and alias fields but got {}",
