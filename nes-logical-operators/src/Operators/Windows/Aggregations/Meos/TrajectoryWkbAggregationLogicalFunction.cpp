...
TrajectoryWkbAggregationLogicalFunction::TrajectoryWkbAggregationLogicalFunction(
...
TrajectoryWkbAggregationLogicalFunction::create(
    const FieldAccessLogicalFunction& lonField,
    const FieldAccessLogicalFunction& latField,
    const FieldAccessLogicalFunction& timestampField)
{
    return std::make_shared<TrajectoryWkbAggregationLogicalFunction>(lonField, latField, timestampField, lonField);
}

std::string_view TrajectoryWkbAggregationLogicalFunction::getName() const noexcept
{
    return NAME;
}

void TrajectoryWkbAggregationLogicalFunction::inferStamp(const Schema& schema)
{
    ...
    {
        throw CannotInferSchema("TrajectoryWkbAggregationLogicalFunction: lon, lat, and timestamp fields must be numeric.");
    }
...
NES::SerializableAggregationFunction TrajectoryWkbAggregationLogicalFunction::serialize() const
{
...
AggregationLogicalFunctionRegistryReturnType AggregationLogicalFunctionGeneratedRegistrar::RegisterTrajectoryWkbAggregationLogicalFunction(
    AggregationLogicalFunctionRegistryArguments arguments)
{
    if (arguments.fields.size() == 4)
    {
        auto ptr = std::make_shared<TrajectoryWkbAggregationLogicalFunction>(
            arguments.fields[0], arguments.fields[1], arguments.fields[2], arguments.fields[3]);
        return ptr;
    }
    throw CannotDeserialize(
        "TrajectoryWkbAggregationLogicalFunction requires lon, lat, timestamp, and alias fields but got {}",
