...
TnumberAngularDifferenceExpAggregationLogicalFunction::TnumberAngularDifferenceExpAggregationLogicalFunction(
...
TnumberAngularDifferenceExpAggregationLogicalFunction::create(
    const FieldAccessLogicalFunction& valueField,
    const FieldAccessLogicalFunction& timestampField)
{
    return std::make_shared<TnumberAngularDifferenceExpAggregationLogicalFunction>(valueField, timestampField, valueField);
}

std::string_view TnumberAngularDifferenceExpAggregationLogicalFunction::getName() const noexcept
{
    return NAME;
}

void TnumberAngularDifferenceExpAggregationLogicalFunction::inferStamp(const Schema& schema)
{
    ...
    {
        throw CannotInferSchema("TnumberAngularDifferenceExpAggregationLogicalFunction: value and timestamp fields must be numeric.");
    }
...
NES::SerializableAggregationFunction TnumberAngularDifferenceExpAggregationLogicalFunction::serialize() const
{
...
AggregationLogicalFunctionRegistryReturnType AggregationLogicalFunctionGeneratedRegistrar::RegisterTnumberAngularDifferenceExpAggregationLogicalFunction(
    AggregationLogicalFunctionRegistryArguments arguments)
{
    if (arguments.fields.size() == 3)
    {
        auto ptr = std::make_shared<TnumberAngularDifferenceExpAggregationLogicalFunction>(
            arguments.fields[0], arguments.fields[1], arguments.fields[2]);
        return ptr;
    }
    throw CannotDeserialize(
        "TnumberAngularDifferenceExpAggregationLogicalFunction requires value, timestamp, and alias fields but got {}",
