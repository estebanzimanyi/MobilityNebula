...
namespace NES::LogicalFunctionGeneratedRegistrar { LogicalFunctionRegistryReturnType RegisterGeoNumGeosLogicalFunction(LogicalFunctionRegistryArguments); }

namespace NES
{

GeoNumGeosLogicalFunction::GeoNumGeosLogicalFunction(LogicalFunction lit)
...
DataType GeoNumGeosLogicalFunction::getDataType() const
{
    return dataType;
}
...
std::vector<LogicalFunction> GeoNumGeosLogicalFunction::getChildren() const
{
    return parameters;
}
...
{
    PRECONDITION(children.size() == 1, "GeoNumGeosLogicalFunction requires 1 children, but got {}", children.size());
    auto copy = *this;
    copy.parameters = children;
    return copy;
}

std::string_view GeoNumGeosLogicalFunction::getType() const
{
    return NAME;
}
    ...
    LogicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.children.size() == 1,
                 "GeoNumGeosLogicalFunction requires 1 children but got {}",
                 arguments.children.size());
    auto arg0 = std::move(arguments.children[0]);
    return GeoNumGeosLogicalFunction(std::move(arg0));
}
