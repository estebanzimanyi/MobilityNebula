...
namespace NES::LogicalFunctionGeneratedRegistrar { LogicalFunctionRegistryReturnType RegisterEverNeTnpointTnpointLogicalFunction(LogicalFunctionRegistryArguments); }

namespace NES
{

EverNeTnpointTnpointLogicalFunction::EverNeTnpointTnpointLogicalFunction(LogicalFunction traj,
...
DataType EverNeTnpointTnpointLogicalFunction::getDataType() const
{
    return dataType;
}
...
std::vector<LogicalFunction> EverNeTnpointTnpointLogicalFunction::getChildren() const
{
    return parameters;
}
...
{
    PRECONDITION(children.size() == 2, "EverNeTnpointTnpointLogicalFunction requires 2 children, but got {}", children.size());
    auto copy = *this;
    copy.parameters = children;
    return copy;
}

std::string_view EverNeTnpointTnpointLogicalFunction::getType() const
{
    return NAME;
}
    ...
    LogicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.children.size() == 2,
                 "EverNeTnpointTnpointLogicalFunction requires 2 children but got {}",
                 arguments.children.size());
    auto arg0 = std::move(arguments.children[0]);
    auto arg1 = std::move(arguments.children[1]);
    return EverNeTnpointTnpointLogicalFunction(std::move(arg0), std::move(arg1));
}
