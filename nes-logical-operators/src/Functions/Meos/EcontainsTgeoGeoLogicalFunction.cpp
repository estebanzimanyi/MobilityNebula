...
namespace NES::LogicalFunctionGeneratedRegistrar { LogicalFunctionRegistryReturnType RegisterEcontainsTgeoGeoLogicalFunction(LogicalFunctionRegistryArguments); }

namespace NES {

/* ─────────── constructors ─────────── */

EcontainsTgeoGeoLogicalFunction::
EcontainsTgeoGeoLogicalFunction(LogicalFunction param1,
...
EcontainsTgeoGeoLogicalFunction::
EcontainsTgeoGeoLogicalFunction(LogicalFunction lon1,
...
DataType EcontainsTgeoGeoLogicalFunction::getDataType() const { return dataType; }

LogicalFunction
EcontainsTgeoGeoLogicalFunction::withDataType(const DataType& dt) const {
    auto c = *this; c.dataType = dt; return c;
}

std::vector<LogicalFunction>
EcontainsTgeoGeoLogicalFunction::getChildren() const { return parameters; }

LogicalFunction
EcontainsTgeoGeoLogicalFunction::withChildren(const std::vector<LogicalFunction>& ch) const {
    PRECONDITION(ch.size()==4 || ch.size()==6,
                 "EcontainsTgeoGeo expects 4 or 6 params, got {}", ch.size());
    auto c=*this; c.parameters=ch;
    return c;
}

std::string_view EcontainsTgeoGeoLogicalFunction::getType() const { return NAME; }

bool EcontainsTgeoGeoLogicalFunction::operator==(const LogicalFunctionConcept& rhs) const {
    const auto* o = dynamic_cast<const EcontainsTgeoGeoLogicalFunction*>(&rhs);
    return o && parameters==o->parameters;
}

std::string
EcontainsTgeoGeoLogicalFunction::explain(ExplainVerbosity v) const {
    std::string a;
...
LogicalFunction
EcontainsTgeoGeoLogicalFunction::withInferredDataType(const Schema& s) const {
    ...
    if(ch.size() == 6) {
        ...
        PRECONDITION(false, "EcontainsTgeoGeo expects 4 or 6 parameters, got {}", ch.size());
    }
    return withChildren(ch);
}

SerializableFunction
EcontainsTgeoGeoLogicalFunction::serialize() const {
...
LogicalFunctionGeneratedRegistrar::RegisterEcontainsTgeoGeoLogicalFunction(
        LogicalFunctionRegistryArguments arguments){
    if(arguments.children.size()==6)
        return EcontainsTgeoGeoLogicalFunction(arguments.children[0],arguments.children[1],arguments.children[2],
                                                        arguments.children[3],arguments.children[4],arguments.children[5]);
    PRECONDITION(arguments.children.size()==4,
                 "EcontainsTgeoGeo expects 4 or 6 params, got {}", arguments.children.size());

    // decide 4-param layout by inspecting first child’s type
    // decide 4-param layout by inspecting first child's type
    return EcontainsTgeoGeoLogicalFunction(arguments.children[0],arguments.children[1],
