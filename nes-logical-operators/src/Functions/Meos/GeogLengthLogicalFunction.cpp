...
namespace NES::LogicalFunctionGeneratedRegistrar { LogicalFunctionRegistryReturnType RegisterGeogLengthLogicalFunction(LogicalFunctionRegistryArguments); }

namespace NES
{

GeogLengthLogicalFunction::GeogLengthLogicalFunction(LogicalFunction lit)
...
DataType GeogLengthLogicalFunction::getDataType() const
{
    return dataType;
}
...
std::vector<LogicalFunction> GeogLengthLogicalFunction::getChildren() const
{
    return parameters;
}
...
{
    PRECONDITION(children.size() == 1, "GeogLengthLogicalFunction requires 1 children, but got {}", children.size());
    auto copy = *this;
    copy.parameters = children;
    return copy;
}

std::string_view GeogLengthLogicalFunction::getType() const
{
    return NAME;
}
    ...
    LogicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.children.size() == 1,
                 "GeogLengthLogicalFunction requires 1 children but got {}",
                 arguments.children.size());
    auto arg0 = std::move(arguments.children[0]);
    return GeogLengthLogicalFunction(std::move(arg0));
}
