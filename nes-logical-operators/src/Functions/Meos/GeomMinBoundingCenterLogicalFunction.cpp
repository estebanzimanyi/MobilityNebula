...
DataType GeomMinBoundingCenterLogicalFunction::getDataType() const
{
    return dataType;
}
...
std::vector<LogicalFunction> GeomMinBoundingCenterLogicalFunction::getChildren() const
{
    return parameters;
}
...
{
    PRECONDITION(children.size() == 1, "GeomMinBoundingCenterLogicalFunction requires 1 children, but got {}", children.size());
    auto copy = *this;
    copy.parameters = children;
    return copy;
}

std::string_view GeomMinBoundingCenterLogicalFunction::getType() const
{
    return NAME;
}
    ...
    LogicalFunctionRegistryArguments arguments)
{
    ...
    return GeomMinBoundingCenterLogicalFunction(std::move(arg0));
}
