...
TleTextTtextLogicalFunction::TleTextTtextLogicalFunction(
...
DataType TleTextTtextLogicalFunction::getDataType() const { return dataType; }

LogicalFunction TleTextTtextLogicalFunction::withDataType(const DataType& dt) const {
    auto c = *this; c.dataType = dt; return c;
}

std::vector<LogicalFunction> TleTextTtextLogicalFunction::getChildren() const { return parameters; }

LogicalFunction TleTextTtextLogicalFunction::withChildren(const std::vector<LogicalFunction>& ch) const {
    PRECONDITION(ch.size() == 3, "TleTextTtext expects 3 params, got {}", ch.size());
    auto c = *this; c.parameters = ch; return c;
}

std::string_view TleTextTtextLogicalFunction::getType() const { return NAME; }

bool TleTextTtextLogicalFunction::operator==(const LogicalFunctionConcept& rhs) const {
    const auto* o = dynamic_cast<const TleTextTtextLogicalFunction*>(&rhs);
    return o && parameters == o->parameters;
}

std::string TleTextTtextLogicalFunction::explain(ExplainVerbosity v) const {
    return fmt::format("TleTextTtext({}, {}, {})",
        parameters[0].explain(v), parameters[1].explain(v), parameters[2].explain(v));
}

LogicalFunction TleTextTtextLogicalFunction::withInferredDataType(const Schema& s) const {
    ...
    INVARIANT(isStr(ch[0].getDataType()) && isStr(ch[1].getDataType()) && isTime(ch[2].getDataType()),
              "TleTextTtext: expects (VARCHAR, VARCHAR, UINT64)");
...
LogicalFunctionGeneratedRegistrar::RegisterTleTextTtextLogicalFunction(
    LogicalFunctionRegistryArguments arguments) {
    PRECONDITION(arguments.children.size() == 3,
                 "TleTextTtext expects 3 params, got {}", arguments.children.size());
    return TleTextTtextLogicalFunction(
