...
DataType JsonbPrettyLogicalFunction::getDataType() const
{
    return dataType;
}

LogicalFunction JsonbPrettyLogicalFunction::withDataType(const DataType& newDataType) const
{
    auto copy = *this;
...
std::vector<LogicalFunction> JsonbPrettyLogicalFunction::getChildren() const
{
    return parameters;
}
...
std::string_view JsonbPrettyLogicalFunction::getType() const
{
    return NAME;
}
...
{
    if (const auto* other = dynamic_cast<const JsonbPrettyLogicalFunction*>(&rhs))
    {
...
std::string JsonbPrettyLogicalFunction::explain(ExplainVerbosity verbosity) const
{
    std::string args;
    ...
    LogicalFunctionRegistryArguments arguments)
{
    ...
    return JsonbPrettyLogicalFunction(std::move(arg0));
}
