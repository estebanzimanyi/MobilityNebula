...
JsonTypeofLogicalFunction::JsonTypeofLogicalFunction(LogicalFunction json)
...
DataType JsonTypeofLogicalFunction::getDataType() const
{
    return dataType;
}
...
std::vector<LogicalFunction> JsonTypeofLogicalFunction::getChildren() const
{
    return parameters;
}
...
{
    PRECONDITION(children.size() == 1, "JsonTypeofLogicalFunction requires 1 children, but got {}", children.size());
    auto copy = *this;
    copy.parameters = children;
    return copy;
}

std::string_view JsonTypeofLogicalFunction::getType() const
{
    return NAME;
}
    ...
    LogicalFunctionRegistryArguments arguments)
{
    ...
    return JsonTypeofLogicalFunction(std::move(arg0));
}
