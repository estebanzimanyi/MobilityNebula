...
namespace NES::LogicalFunctionGeneratedRegistrar { LogicalFunctionRegistryReturnType RegisterFloatspanMakeLogicalFunction(LogicalFunctionRegistryArguments); }

namespace NES
{

FloatspanMakeLogicalFunction::FloatspanMakeLogicalFunction(LogicalFunction value,
...
DataType FloatspanMakeLogicalFunction::getDataType() const
{
    return dataType;
}

LogicalFunction FloatspanMakeLogicalFunction::withDataType(const DataType& newDataType) const
{
    auto copy = *this;
...
std::vector<LogicalFunction> FloatspanMakeLogicalFunction::getChildren() const
{
    return parameters;
}
...
std::string_view FloatspanMakeLogicalFunction::getType() const
{
    return NAME;
}
...
{
    if (const auto* other = dynamic_cast<const FloatspanMakeLogicalFunction*>(&rhs))
    {
...
std::string FloatspanMakeLogicalFunction::explain(ExplainVerbosity verbosity) const
{
    std::string args;
    ...
    LogicalFunctionRegistryArguments arguments)
{
    ...
    return FloatspanMakeLogicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3));
}
