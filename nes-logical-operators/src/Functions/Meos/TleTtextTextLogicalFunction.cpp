...
TleTtextTextLogicalFunction::TleTtextTextLogicalFunction(
...
DataType TleTtextTextLogicalFunction::getDataType() const { return dataType; }

LogicalFunction TleTtextTextLogicalFunction::withDataType(const DataType& dt) const {
    auto c = *this; c.dataType = dt; return c;
}

std::vector<LogicalFunction> TleTtextTextLogicalFunction::getChildren() const { return parameters; }

LogicalFunction TleTtextTextLogicalFunction::withChildren(const std::vector<LogicalFunction>& ch) const {
    PRECONDITION(ch.size() == 3, "TleTtextText expects 3 params, got {}", ch.size());
    auto c = *this; c.parameters = ch; return c;
}

std::string_view TleTtextTextLogicalFunction::getType() const { return NAME; }

bool TleTtextTextLogicalFunction::operator==(const LogicalFunctionConcept& rhs) const {
    const auto* o = dynamic_cast<const TleTtextTextLogicalFunction*>(&rhs);
    return o && parameters == o->parameters;
}

std::string TleTtextTextLogicalFunction::explain(ExplainVerbosity v) const {
    return fmt::format("TleTtextText({}, {}, {})",
        parameters[0].explain(v), parameters[1].explain(v), parameters[2].explain(v));
}

LogicalFunction TleTtextTextLogicalFunction::withInferredDataType(const Schema& s) const {
    ...
    INVARIANT(isStr(ch[0].getDataType()) && isStr(ch[1].getDataType()) && isTime(ch[2].getDataType()),
              "TleTtextText: expects (VARCHAR, VARCHAR, UINT64)");
...
LogicalFunctionGeneratedRegistrar::RegisterTleTtextTextLogicalFunction(
    LogicalFunctionRegistryArguments arguments) {
    PRECONDITION(arguments.children.size() == 3,
                 "TleTtextText expects 3 params, got {}", arguments.children.size());
    return TleTtextTextLogicalFunction(
