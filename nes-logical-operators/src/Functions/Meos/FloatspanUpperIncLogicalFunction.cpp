...
DataType FloatspanUpperIncLogicalFunction::getDataType() const
{
    return dataType;
}

LogicalFunction FloatspanUpperIncLogicalFunction::withDataType(const DataType& newDataType) const
{
    auto copy = *this;
...
std::vector<LogicalFunction> FloatspanUpperIncLogicalFunction::getChildren() const
{
    return parameters;
}
...
std::string_view FloatspanUpperIncLogicalFunction::getType() const
{
    return NAME;
}
...
{
    if (const auto* other = dynamic_cast<const FloatspanUpperIncLogicalFunction*>(&rhs))
    {
...
std::string FloatspanUpperIncLogicalFunction::explain(ExplainVerbosity verbosity) const
{
    std::string args;
    ...
    LogicalFunctionRegistryArguments arguments)
{
    ...
    return FloatspanUpperIncLogicalFunction(std::move(arg0));
}
