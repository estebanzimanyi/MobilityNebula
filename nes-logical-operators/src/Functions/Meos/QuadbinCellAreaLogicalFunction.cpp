...
DataType QuadbinCellAreaLogicalFunction::getDataType() const
{
    return dataType;
}
...
std::vector<LogicalFunction> QuadbinCellAreaLogicalFunction::getChildren() const
{
    return parameters;
}
...
{
    PRECONDITION(children.size() == 1, "QuadbinCellAreaLogicalFunction requires 1 children, but got {}", children.size());
    auto copy = *this;
    copy.parameters = children;
    return copy;
}

std::string_view QuadbinCellAreaLogicalFunction::getType() const
{
    return NAME;
}
    ...
    LogicalFunctionRegistryArguments arguments)
{
    ...
    return QuadbinCellAreaLogicalFunction(std::move(arg0));
}
