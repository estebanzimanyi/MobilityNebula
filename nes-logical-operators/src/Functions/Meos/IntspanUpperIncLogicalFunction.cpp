...
DataType IntspanUpperIncLogicalFunction::getDataType() const
{
    return dataType;
}

LogicalFunction IntspanUpperIncLogicalFunction::withDataType(const DataType& newDataType) const
{
    auto copy = *this;
...
std::vector<LogicalFunction> IntspanUpperIncLogicalFunction::getChildren() const
{
    return parameters;
}
...
std::string_view IntspanUpperIncLogicalFunction::getType() const
{
    return NAME;
}
...
{
    if (const auto* other = dynamic_cast<const IntspanUpperIncLogicalFunction*>(&rhs))
    {
...
std::string IntspanUpperIncLogicalFunction::explain(ExplainVerbosity verbosity) const
{
    std::string args;
    ...
    LogicalFunctionRegistryArguments arguments)
{
    ...
    return IntspanUpperIncLogicalFunction(std::move(arg0));
}
