...
namespace NES::LogicalFunctionGeneratedRegistrar { LogicalFunctionRegistryReturnType RegisterNadTposePoseLogicalFunction(LogicalFunctionRegistryArguments); }

namespace NES
{

NadTposePoseLogicalFunction::NadTposePoseLogicalFunction(LogicalFunction x,
                                          LogicalFunction y,
...
DataType NadTposePoseLogicalFunction::getDataType() const
{
    return dataType;
}
...
std::vector<LogicalFunction> NadTposePoseLogicalFunction::getChildren() const
{
    return parameters;
}
...
{
    PRECONDITION(children.size() == 5, "NadTposePoseLogicalFunction requires 5 children, but got {}", children.size());
    auto copy = *this;
    copy.parameters = children;
    return copy;
}

std::string_view NadTposePoseLogicalFunction::getType() const
{
    return NAME;
}
    ...
    LogicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.children.size() == 5,
                 "NadTposePoseLogicalFunction requires 5 children but got {}",
    ...
    return NadTposePoseLogicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4));
}
