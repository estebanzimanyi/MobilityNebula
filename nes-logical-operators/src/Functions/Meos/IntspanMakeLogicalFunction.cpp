...
namespace NES::LogicalFunctionGeneratedRegistrar { LogicalFunctionRegistryReturnType RegisterIntspanMakeLogicalFunction(LogicalFunctionRegistryArguments); }

namespace NES
{

IntspanMakeLogicalFunction::IntspanMakeLogicalFunction(LogicalFunction value,
...
DataType IntspanMakeLogicalFunction::getDataType() const
{
    return dataType;
}

LogicalFunction IntspanMakeLogicalFunction::withDataType(const DataType& newDataType) const
{
    auto copy = *this;
...
std::vector<LogicalFunction> IntspanMakeLogicalFunction::getChildren() const
{
    return parameters;
}
...
std::string_view IntspanMakeLogicalFunction::getType() const
{
    return NAME;
}
...
{
    if (const auto* other = dynamic_cast<const IntspanMakeLogicalFunction*>(&rhs))
    {
...
std::string IntspanMakeLogicalFunction::explain(ExplainVerbosity verbosity) const
{
    std::string args;
    ...
    LogicalFunctionRegistryArguments arguments)
{
    ...
    return IntspanMakeLogicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3));
}
