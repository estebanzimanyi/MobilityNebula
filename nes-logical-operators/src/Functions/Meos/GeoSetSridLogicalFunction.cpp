...
namespace NES::LogicalFunctionGeneratedRegistrar { LogicalFunctionRegistryReturnType RegisterGeoSetSridLogicalFunction(LogicalFunctionRegistryArguments); }

namespace NES
{

GeoSetSridLogicalFunction::GeoSetSridLogicalFunction(LogicalFunction lit)
...
DataType GeoSetSridLogicalFunction::getDataType() const
{
    return dataType;
}
...
std::vector<LogicalFunction> GeoSetSridLogicalFunction::getChildren() const
{
    return parameters;
}
...
{
    PRECONDITION(children.size() == 1, "GeoSetSridLogicalFunction requires 1 children, but got {}", children.size());
    auto copy = *this;
    copy.parameters = children;
    return copy;
}

std::string_view GeoSetSridLogicalFunction::getType() const
{
    return NAME;
}
    ...
    LogicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.children.size() == 1,
                 "GeoSetSridLogicalFunction requires 1 children but got {}",
                 arguments.children.size());
    auto arg0 = std::move(arguments.children[0]);
    return GeoSetSridLogicalFunction(std::move(arg0));
}
