...
namespace NES::LogicalFunctionGeneratedRegistrar { LogicalFunctionRegistryReturnType RegisterGeoSridLogicalFunction(LogicalFunctionRegistryArguments); }

namespace NES
{

GeoSridLogicalFunction::GeoSridLogicalFunction(LogicalFunction lit)
...
DataType GeoSridLogicalFunction::getDataType() const
{
    return dataType;
}
...
std::vector<LogicalFunction> GeoSridLogicalFunction::getChildren() const
{
    return parameters;
}
...
{
    PRECONDITION(children.size() == 1, "GeoSridLogicalFunction requires 1 children, but got {}", children.size());
    auto copy = *this;
    copy.parameters = children;
    return copy;
}

std::string_view GeoSridLogicalFunction::getType() const
{
    return NAME;
}
    ...
    LogicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.children.size() == 1,
                 "GeoSridLogicalFunction requires 1 children but got {}",
                 arguments.children.size());
    auto arg0 = std::move(arguments.children[0]);
    return GeoSridLogicalFunction(std::move(arg0));
}
