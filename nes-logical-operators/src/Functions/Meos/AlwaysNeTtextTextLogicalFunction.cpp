...
AlwaysNeTtextTextLogicalFunction::AlwaysNeTtextTextLogicalFunction(
...
DataType AlwaysNeTtextTextLogicalFunction::getDataType() const { return dataType; }

LogicalFunction AlwaysNeTtextTextLogicalFunction::withDataType(const DataType& dt) const {
    auto c = *this; c.dataType = dt; return c;
}

std::vector<LogicalFunction> AlwaysNeTtextTextLogicalFunction::getChildren() const { return parameters; }

LogicalFunction AlwaysNeTtextTextLogicalFunction::withChildren(const std::vector<LogicalFunction>& ch) const {
    PRECONDITION(ch.size() == 3, "AlwaysNeTtextText expects 3 params, got {}", ch.size());
    auto c = *this; c.parameters = ch; return c;
}

std::string_view AlwaysNeTtextTextLogicalFunction::getType() const { return NAME; }

bool AlwaysNeTtextTextLogicalFunction::operator==(const LogicalFunctionConcept& rhs) const {
    const auto* o = dynamic_cast<const AlwaysNeTtextTextLogicalFunction*>(&rhs);
    return o && parameters == o->parameters;
}

std::string AlwaysNeTtextTextLogicalFunction::explain(ExplainVerbosity v) const {
    return fmt::format("AlwaysNeTtextText({}, {}, {})",
        parameters[0].explain(v), parameters[1].explain(v), parameters[2].explain(v));
}

LogicalFunction AlwaysNeTtextTextLogicalFunction::withInferredDataType(const Schema& s) const {
    ...
    INVARIANT(isStr(ch[0].getDataType()) && isStr(ch[1].getDataType()) && isTime(ch[2].getDataType()),
              "AlwaysNeTtextText: expects (VARCHAR, VARCHAR, UINT64)");
...
LogicalFunctionGeneratedRegistrar::RegisterAlwaysNeTtextTextLogicalFunction(
    LogicalFunctionRegistryArguments arguments) {
    PRECONDITION(arguments.children.size() == 3,
                 "AlwaysNeTtextText expects 3 params, got {}", arguments.children.size());
    return AlwaysNeTtextTextLogicalFunction(
