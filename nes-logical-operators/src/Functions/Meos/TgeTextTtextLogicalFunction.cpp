...
TgeTextTtextLogicalFunction::TgeTextTtextLogicalFunction(
...
DataType TgeTextTtextLogicalFunction::getDataType() const { return dataType; }

LogicalFunction TgeTextTtextLogicalFunction::withDataType(const DataType& dt) const {
    auto c = *this; c.dataType = dt; return c;
}

std::vector<LogicalFunction> TgeTextTtextLogicalFunction::getChildren() const { return parameters; }

LogicalFunction TgeTextTtextLogicalFunction::withChildren(const std::vector<LogicalFunction>& ch) const {
    PRECONDITION(ch.size() == 3, "TgeTextTtext expects 3 params, got {}", ch.size());
    auto c = *this; c.parameters = ch; return c;
}

std::string_view TgeTextTtextLogicalFunction::getType() const { return NAME; }

bool TgeTextTtextLogicalFunction::operator==(const LogicalFunctionConcept& rhs) const {
    const auto* o = dynamic_cast<const TgeTextTtextLogicalFunction*>(&rhs);
    return o && parameters == o->parameters;
}

std::string TgeTextTtextLogicalFunction::explain(ExplainVerbosity v) const {
    return fmt::format("TgeTextTtext({}, {}, {})",
        parameters[0].explain(v), parameters[1].explain(v), parameters[2].explain(v));
}

LogicalFunction TgeTextTtextLogicalFunction::withInferredDataType(const Schema& s) const {
    ...
    INVARIANT(isStr(ch[0].getDataType()) && isStr(ch[1].getDataType()) && isTime(ch[2].getDataType()),
              "TgeTextTtext: expects (VARCHAR, VARCHAR, UINT64)");
...
LogicalFunctionGeneratedRegistrar::RegisterTgeTextTtextLogicalFunction(
    LogicalFunctionRegistryArguments arguments) {
    PRECONDITION(arguments.children.size() == 3,
                 "TgeTextTtext expects 3 params, got {}", arguments.children.size());
    return TgeTextTtextLogicalFunction(
