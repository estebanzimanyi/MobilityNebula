...
/**
 * @brief Encode geometry as hex-EWKB.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `({std::string _s(wktPtr,wktSize),_es(endianPtr,endianSize);GSERIALIZED* _gs=geom_in(_s.c_str(),-1);if(!_gs)return 0u;char* _out=geo_as_hexewkb(_gs,_es.c_str());free(_gs);_out;})`. Per-event scalar operator following the
...
public:
    ...
    GeoAsHexewkbLogicalFunction(LogicalFunction wkt,
