...
/**
 * @brief tgeodetic_type (bool) — type-dispatch predicate on a value's temptype.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `tgeodetic_type`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TgeodeticTypeLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TgeodeticType";

    TgeodeticTypeLogicalFunction(LogicalFunction value,
                                          LogicalFunction ts);
