...
/**
 * @brief temporal_type (bool) — type-dispatch predicate on a value's temptype.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `temporal_type`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TemporalTypeLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TemporalType";

    TemporalTypeLogicalFunction(LogicalFunction value,
                                          LogicalFunction ts);
