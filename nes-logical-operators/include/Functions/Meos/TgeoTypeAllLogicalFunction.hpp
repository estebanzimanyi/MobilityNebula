...
/**
 * @brief tgeo_type_all (bool) — type-dispatch predicate on a value's temptype.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `tgeo_type_all`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TgeoTypeAllLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TgeoTypeAll";

    TgeoTypeAllLogicalFunction(LogicalFunction value,
                                          LogicalFunction ts);
