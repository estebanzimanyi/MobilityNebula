...
/**
 * @brief Returns 1.0 if the temporal geometry is always not equal to the static geometry.
 *
 * Args: (lon:FLOAT64, lat:FLOAT64, ts:UINT64, wkt:VARCHAR) -> FLOAT64.
 */
class AlwaysNeTgeoGeoLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "AlwaysNeTgeoGeo";

    AlwaysNeTgeoGeoLogicalFunction(LogicalFunction lon, LogicalFunction lat,
