...
/**
 * @brief tspatial_type (bool) — type-dispatch predicate on a value's temptype.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `tspatial_type`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TspatialTypeLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TspatialType";

    TspatialTypeLogicalFunction(LogicalFunction value,
                                          LogicalFunction ts);
