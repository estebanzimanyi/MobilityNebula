...
/**
 * @brief Per-event always-dwithin between a tgeompoint resolved from a tnpoint (rid,fraction,ts) via tnpoint_to_tgeompoint and a static geometry (network SRID), within a distance.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `adwithin_tgeo_geo`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TemporalADWithinTNpointGeometryLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TemporalADWithinTNpointGeometry";

    TemporalADWithinTNpointGeometryLogicalFunction(LogicalFunction rid,
