...
/**
 * @brief Per-event tnumber_abs: single-instant tnumber transform, value extracted -> double.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `tnumber_abs`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TnumberAbsLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TnumberAbs";

    TnumberAbsLogicalFunction(LogicalFunction value,
                              LogicalFunction ts);
