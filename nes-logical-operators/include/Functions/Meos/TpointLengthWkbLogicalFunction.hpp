...
/**
 * @brief Function-library-over-MEOS-value: tpoint_length applied to an upstream hex-WKB trajectory value (the efficient compose-on-materialized-trajectory mechanism).
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `tpoint_length`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TpointLengthWkbLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TpointLengthWkb";

    TpointLengthWkbLogicalFunction(LogicalFunction traj);
