...
/**
 * @brief Compute the buffer zone around a geometry.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `({std::string _s(wktPtr,wktSize),_ps(paramsPtr,paramsSize);GSERIALIZED* _gs=geom_in(_s.c_str(),-1);if(!_gs)return 0u;GSERIALIZED* _res=geom_buffer(_gs,size,_ps.c_str());free(_gs);_res;})`. Per-event scalar operator following the
...
public:
    ...
    GeomBufferLogicalFunction(LogicalFunction wkt,
