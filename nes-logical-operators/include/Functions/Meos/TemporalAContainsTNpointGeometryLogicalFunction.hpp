...
/**
 * @brief Per-event always-contains between a single-instant tgeompoint resolved from a tnpoint (rid,fraction,ts) via tnpoint_to_tgeompoint and a static geometry (network SRID).
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `acontains_tgeo_geo`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TemporalAContainsTNpointGeometryLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TemporalAContainsTNpointGeometry";

    TemporalAContainsTNpointGeometryLogicalFunction(LogicalFunction rid,
