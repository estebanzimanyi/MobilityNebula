...
/**
 * @brief Per-event ever-touches between two single-instant tgeompoints, each lifted from a tpose (x,y,theta,ts) via tpose_to_tpoint composition.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `etouches_tgeo_tgeo`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TemporalETouchesTPoseTPoseLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TemporalETouchesTPoseTPose";

    TemporalETouchesTPoseTPoseLogicalFunction(LogicalFunction xA,
                                          LogicalFunction yA,
