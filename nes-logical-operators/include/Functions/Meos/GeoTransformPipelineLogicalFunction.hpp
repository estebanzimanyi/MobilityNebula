...
/**
 * @brief Transform geometry through a coordinate pipeline.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `({std::string _s(wktPtr,wktSize),_ps(pipelinePtr,pipelineSize);GSERIALIZED* _gs=geom_in(_s.c_str(),-1);if(!_gs)return 0u;GSERIALIZED* _res=geo_transform_pipeline(_gs,const_cast<char*>(_ps.c_str()),(int32_t)srid_to,(bool)is_forward);free(_gs);_res;})`. Per-event scalar operator following the
...
public:
    ...
    GeoTransformPipelineLogicalFunction(LogicalFunction wkt,
