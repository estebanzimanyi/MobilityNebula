...
/**
 * @brief Returns 1.0 if temporal geometry ever touches the static geometry.
 *
 * Args: (lon:FLOAT64, lat:FLOAT64, ts:UINT64, wkt:VARCHAR) -> FLOAT64.
 */
class EtouchesTgeoGeoLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "EtouchesTgeoGeo";

    EtouchesTgeoGeoLogicalFunction(LogicalFunction lon, LogicalFunction lat,
