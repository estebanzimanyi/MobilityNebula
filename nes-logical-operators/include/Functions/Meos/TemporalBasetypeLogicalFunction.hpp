...
/**
 * @brief temporal_basetype (bool) — type-dispatch predicate on a value's temptype.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `temporal_basetype`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TemporalBasetypeLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TemporalBasetype";

    TemporalBasetypeLogicalFunction(LogicalFunction value,
                                          LogicalFunction ts);
