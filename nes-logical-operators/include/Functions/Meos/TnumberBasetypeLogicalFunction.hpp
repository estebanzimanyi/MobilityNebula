...
/**
 * @brief tnumber_basetype (bool) — type-dispatch predicate on a value's temptype.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `tnumber_basetype`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TnumberBasetypeLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TnumberBasetype";

    TnumberBasetypeLogicalFunction(LogicalFunction value,
                                          LogicalFunction ts);
