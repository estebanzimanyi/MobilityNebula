...
/**
 * @brief Per-event nearest-approach distance between a single-instant tcbuffer (lon,lat,radius,ts) and a static geometry.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `nad_tcbuffer_geo`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TemporalNADTCbufferLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TemporalNADTCbuffer";

    TemporalNADTCbufferLogicalFunction(LogicalFunction lon,
                                          LogicalFunction lat,
