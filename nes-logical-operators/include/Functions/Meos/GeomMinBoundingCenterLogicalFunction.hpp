...
/**
 * @brief Centre of the minimum bounding circle of a geometry.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `({std::string _s(wktPtr,wktSize);GSERIALIZED* _gs=geom_in(_s.c_str(),-1);if(!_gs)return 0u;MinBoundingCircle _mbc=geom_min_bounding_radius(_gs);free(_gs);_mbc.center;})`. Per-event scalar operator following the
