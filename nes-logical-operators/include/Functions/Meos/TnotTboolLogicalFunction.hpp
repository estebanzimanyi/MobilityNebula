...
/**
 * @brief Per-event tnot_tbool: single-instant tbool negation, result extracted -> double.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `tnot_tbool`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TnotTboolLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TnotTbool";

    TnotTboolLogicalFunction(LogicalFunction value,
                                          LogicalFunction ts);
