...
/**
 * @brief Number of elements in a JSON array.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `({std::string _s(jsonPtr,jsonSize);text* _js=json_in(_s.c_str());if(!_js)return 0.0;int _n=json_array_length(_js);free(_js);(double)_n;})`. Per-event scalar operator following the
...
public:
    ...
    JsonArrayLengthLogicalFunction(LogicalFunction json);
