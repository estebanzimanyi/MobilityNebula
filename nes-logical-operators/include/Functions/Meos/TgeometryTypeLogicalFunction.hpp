...
/**
 * @brief tgeometry_type (bool) — type-dispatch predicate on a value's temptype.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `tgeometry_type`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TgeometryTypeLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TgeometryType";

    TgeometryTypeLogicalFunction(LogicalFunction value,
                                          LogicalFunction ts);
