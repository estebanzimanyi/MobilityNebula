...
/**
 * @brief Returns 1.0 if temporal geometry is always disjoint from the static geometry.
 *
 * Args: (lon:FLOAT64, lat:FLOAT64, ts:UINT64, wkt:VARCHAR) -> FLOAT64.
 */
class AdisjointTgeoGeoLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "AdisjointTgeoGeo";

    AdisjointTgeoGeoLogicalFunction(LogicalFunction lon, LogicalFunction lat,
