...
/**
 * @brief tnumber_type (bool) — type-dispatch predicate on a value's temptype.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `tnumber_type`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TnumberTypeLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TnumberType";

    TnumberTypeLogicalFunction(LogicalFunction value,
                                          LogicalFunction ts);
