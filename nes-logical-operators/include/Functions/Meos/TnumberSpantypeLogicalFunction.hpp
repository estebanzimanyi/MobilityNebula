...
/**
 * @brief tnumber_spantype (bool) — type-dispatch predicate on a value's temptype.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `tnumber_spantype`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TnumberSpantypeLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TnumberSpantype";

    TnumberSpantypeLogicalFunction(LogicalFunction value,
                                          LogicalFunction ts);
