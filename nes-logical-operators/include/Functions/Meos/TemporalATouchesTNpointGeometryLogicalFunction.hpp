...
/**
 * @brief Per-event always-touches between a single-instant tgeompoint resolved from a tnpoint (rid,fraction,ts) via tnpoint_to_tgeompoint and a static geometry (network SRID).
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `atouches_tgeo_geo`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TemporalATouchesTNpointGeometryLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TemporalATouchesTNpointGeometry";

    TemporalATouchesTNpointGeometryLogicalFunction(LogicalFunction rid,
