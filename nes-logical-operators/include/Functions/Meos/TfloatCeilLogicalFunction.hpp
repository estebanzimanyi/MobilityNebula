...
/**
 * @brief Per-event tfloat_ceil: single-instant tfloat transform, value extracted -> double.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `tfloat_ceil`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TfloatCeilLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TfloatCeil";

    TfloatCeilLogicalFunction(LogicalFunction value,
                                          LogicalFunction ts);
