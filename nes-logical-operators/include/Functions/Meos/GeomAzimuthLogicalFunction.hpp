...
/**
 * @brief Static geometry azimuth geom_azimuth: (wkt1:VARCHAR, wkt2:VARCHAR) -> float64 (radians).
 *
 * Generated from the MEOS function `geom_azimuth`.
 * Returns the azimuth angle from the first point to the second, in radians;
 * returns 0.0 when the azimuth is undefined (identical points).
 */
class GeomAzimuthLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "GeomAzimuth";

    GeomAzimuthLogicalFunction(LogicalFunction wkt1, LogicalFunction wkt2);
