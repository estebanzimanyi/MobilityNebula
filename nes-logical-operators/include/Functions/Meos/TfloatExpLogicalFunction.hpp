...
/**
 * @brief Per-event tfloat_exp: single-instant tfloat transform, value extracted -> double.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `tfloat_exp`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TfloatExpLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TfloatExp";

    TfloatExpLogicalFunction(LogicalFunction value,
                                          LogicalFunction ts);
