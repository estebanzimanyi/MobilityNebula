...
/**
 * @brief Map (lon,lat) point to H3 cell at given resolution.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `({std::string _wkt=fmt::format("SRID=4326;POINT({} {})",lon,lat);GSERIALIZED* _gs=geom_in(_wkt.c_str(),-1);if(!_gs)return 0u;H3Index _cell=h3_gs_point_to_cell(_gs,(int32_t)res);free(_gs);_cell;})`. Per-event scalar operator following the
...
public:
    ...
    H3GsPointToCellLogicalFunction(LogicalFunction lon,
                                          LogicalFunction lat,
                                          LogicalFunction res);
