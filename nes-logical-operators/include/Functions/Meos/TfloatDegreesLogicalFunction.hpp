...
/**
 * @brief Per-event tfloat_degrees: single-instant tfloat transform, value extracted -> double.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `tfloat_degrees` (normalize=false). Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TfloatDegreesLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TfloatDegrees";

    TfloatDegreesLogicalFunction(LogicalFunction value,
                                          LogicalFunction ts);
