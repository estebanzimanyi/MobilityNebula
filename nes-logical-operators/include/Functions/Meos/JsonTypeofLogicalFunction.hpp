...
/**
 * @brief Data type of the outermost JSON element.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `({std::string _s(jsonPtr,jsonSize);text* _js=json_in(_s.c_str());if(!_js)return 0u;text* _res=json_typeof(_js);free(_js);_res;})`. Per-event scalar operator following the
...
public:
    ...
    JsonTypeofLogicalFunction(LogicalFunction json);
