...
/**
 * @brief Construct a float span from bounds and inclusivity flags.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `({Span* _sp=floatspan_make(lower,upper,lower_inc!=0.0,upper_inc!=0.0);if(!_sp)return 0u;char* _out=floatspan_out(_sp,-1);free(_sp);_out;})`. Per-event scalar operator following the
...
public:
    ...
    FloatspanMakeLogicalFunction(LogicalFunction lower,
