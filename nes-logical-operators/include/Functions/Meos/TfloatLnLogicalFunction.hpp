...
/**
 * @brief Per-event tfloat_ln: single-instant tfloat transform, value extracted -> double.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `tfloat_ln`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TfloatLnLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TfloatLn";

    TfloatLnLogicalFunction(LogicalFunction value,
                                          LogicalFunction ts);
