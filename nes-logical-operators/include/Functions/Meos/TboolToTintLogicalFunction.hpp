...
/**
 * @brief Per-event tbool_to_tint: single-instant tbool cast to tint, result extracted -> double.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `tbool_to_tint`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TboolToTintLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TboolToTint";

    TboolToTintLogicalFunction(LogicalFunction value,
                               LogicalFunction ts);
