...
/**
 * @brief talphanum_type (bool) — type-dispatch predicate on a value's temptype.
 *
 * Generated by tools/codegen/codegen_nebula.py from the MEOS function
 * `talphanum_type`. Per-event scalar operator following the
 * TemporalEDWithinGeometry pattern.
 */
class TalphanumTypeLogicalFunction : public LogicalFunctionConcept {
public:
    static constexpr std::string_view NAME = "TalphanumType";

    TalphanumTypeLogicalFunction(LogicalFunction value,
                                          LogicalFunction ts);
