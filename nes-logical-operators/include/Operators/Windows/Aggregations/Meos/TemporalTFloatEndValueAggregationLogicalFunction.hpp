...
/**
 * @brief Value at the last instant of the per-(window, group) tfloat sequence.
 *
 ...
 * sequence and calls MEOS `tfloat_end_value` to fold it to a single scalar.
 */
class TemporalTFloatEndValueAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TemporalTFloatEndValueAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~TemporalTFloatEndValueAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TemporalTFloatEndValue";
