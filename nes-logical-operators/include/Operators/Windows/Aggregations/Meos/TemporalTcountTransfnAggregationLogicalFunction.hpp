...
/**
 * @brief Windowed TEMPORAL_TCOUNT_TRANSFN -> aggregate tint count (hex-WKB) via temporal_tcount_transfn/temporal_tagg_finalfn.
 *
 ...
 * sequence and calls MEOS `temporal_tcount_transfn` to fold it to a single scalar.
 */
class TemporalTcountTransfnAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TemporalTcountTransfnAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~TemporalTcountTransfnAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TemporalTcountTransfn";
