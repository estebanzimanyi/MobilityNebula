...
/**
 * @brief Windowed SPANSET_EXTENT_TRANSFN -> bounding Span (text) via spanset_extent_transfn.
 *
 ...
 * sequence and calls MEOS `spanset_extent_transfn` to fold it to a single scalar.
 */
class SpansetExtentTransfnAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    SpansetExtentTransfnAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~SpansetExtentTransfnAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "SpansetExtentTransfn";
