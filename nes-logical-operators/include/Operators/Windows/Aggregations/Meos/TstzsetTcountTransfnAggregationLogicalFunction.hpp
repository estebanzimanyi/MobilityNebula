...
/**
 * @brief Windowed TSTZSET_TCOUNT_TRANSFN -> aggregate Temporal (hex-WKB) via tstzset_tcount_transfn/temporal_tagg_finalfn.
 *
 ...
 * sequence and calls MEOS `tstzset_tcount_transfn` to fold it to a single scalar.
 */
class TstzsetTcountTransfnAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TstzsetTcountTransfnAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~TstzsetTcountTransfnAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TstzsetTcountTransfn";
