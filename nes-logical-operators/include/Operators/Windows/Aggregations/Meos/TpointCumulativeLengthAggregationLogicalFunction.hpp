...
/**
 * @brief Windowed tgeompoint mini-trip grown by appendInstant; tpoint_cumulative_length emits the running planar length as a tfloat (hex-WKB).
 *
 ...
 * and calls MEOS `tpoint_cumulative_length` to fold it to a single scalar.
 */
class TpointCumulativeLengthAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TpointCumulativeLengthAggregationLogicalFunction(
    ...
    ~TpointCumulativeLengthAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TpointCumulativeLength";
