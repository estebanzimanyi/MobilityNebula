...
/**
 * @brief tsequenceset_make_gaps (TSequenceSet *) - the gap-aware sequence set built from a window of temporal-instant values (hex-WKB).
 *
 ...
 * sequence and calls MEOS `` to fold it to a single scalar.
 */
class TsequencesetMakeGapsAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TsequencesetMakeGapsAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~TsequencesetMakeGapsAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TsequencesetMakeGaps";
