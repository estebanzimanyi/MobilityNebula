...
/**
 * @brief Windowed trajectory length over an expandable Temporal* grown by appendInstant — the MEOS-native streaming aggregation (no string build, no WKB).
 *
 ...
 * and calls MEOS `tpoint_length` to fold it to a single scalar.
 */
class TLengthExpAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    ~TLengthExpAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TLENGTH_EXP";
