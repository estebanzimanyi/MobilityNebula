...
/**
 * @brief Arithmetic mean of all instant values in the per-(window, group) tfloat sequence.
 *
 ...
 * sequence and calls MEOS `tfloat_avg_value` to fold it to a single scalar.
 */
class TemporalTFloatAvgValueAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TemporalTFloatAvgValueAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~TemporalTFloatAvgValueAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TemporalTFloatAvgValue";
