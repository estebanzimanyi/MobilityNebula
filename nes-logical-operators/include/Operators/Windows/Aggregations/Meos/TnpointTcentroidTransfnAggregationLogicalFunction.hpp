...
/**
 * @brief Windowed TNPOINT_TCENTROID_TRANSFN -> aggregate Temporal (hex-WKB) via tnpoint_tcentroid_transfn/tpoint_tcentroid_finalfn.
 *
 ...
 * sequence and calls MEOS `tnpoint_tcentroid_transfn` to fold it to a single scalar.
 */
class TnpointTcentroidTransfnAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TnpointTcentroidTransfnAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~TnpointTcentroidTransfnAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TnpointTcentroidTransfn";
