...
/**
 * @brief temporal_stops (Temporal* -> hex-WKB) - the windowed trajectory's stop subsequences (within maxdist for at least minduration).
 *
 ...
 * and calls MEOS `temporal_stops` to fold it to a single scalar.
 */
class TemporalStopsAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TemporalStopsAggregationLogicalFunction(
    ...
    ~TemporalStopsAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TemporalStops";
