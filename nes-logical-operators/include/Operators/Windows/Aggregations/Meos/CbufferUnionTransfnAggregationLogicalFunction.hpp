...
/**
 * @brief Windowed CBUFFER_UNION_TRANSFN -> unioned container (text) via cbuffer_union_transfn/set_union_finalfn.
 *
 ...
 * sequence and calls MEOS `cbuffer_union_transfn` to fold it to a single scalar.
 */
class CbufferUnionTransfnAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    CbufferUnionTransfnAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~CbufferUnionTransfnAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "CbufferUnionTransfn";
