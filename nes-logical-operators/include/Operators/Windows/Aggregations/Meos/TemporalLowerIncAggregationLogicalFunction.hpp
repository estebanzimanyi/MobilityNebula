...
/**
 * @brief True if the per-(window, group) tgeo trajectory's lower period bound is inclusive.
 *
 ...
 * and calls MEOS `temporal_lower_inc` to fold it to a single scalar.
 */
class TemporalLowerIncAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    ~TemporalLowerIncAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TemporalLowerInc";
