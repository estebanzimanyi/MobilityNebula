...
/**
 * @brief trgeometry_instants (TInstant **) - the array of instants of the windowed continuous trgeometry, each as EWKB hex.
 *
 ...
 * MEOS sequence accessor `trgeometry_instants(static_cast<Temporal*>(temp), &_cnt)` to fold it.
 */
class TrgeometryInstantsAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TrgeometryInstantsAggregationLogicalFunction(
    ...
    ~TrgeometryInstantsAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TrgeometryInstants";
