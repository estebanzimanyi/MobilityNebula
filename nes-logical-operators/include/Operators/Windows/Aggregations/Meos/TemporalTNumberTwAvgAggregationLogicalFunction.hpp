...
/**
 * @brief Time-weighted average of values across the per-(window, group) tfloat sequence.
 *
 ...
 * sequence and calls MEOS `tnumber_twavg` to fold it to a single scalar.
 */
class TemporalTNumberTwAvgAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TemporalTNumberTwAvgAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~TemporalTNumberTwAvgAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TemporalTNumberTwAvg";
