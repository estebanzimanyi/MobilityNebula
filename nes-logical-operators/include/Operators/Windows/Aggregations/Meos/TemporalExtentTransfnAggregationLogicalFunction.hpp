...
/**
 * @brief Windowed TEMPORAL_EXTENT_TRANSFN -> bounding Span (text) via temporal_extent_transfn.
 *
 ...
 * sequence and calls MEOS `temporal_extent_transfn` to fold it to a single scalar.
 */
class TemporalExtentTransfnAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TemporalExtentTransfnAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~TemporalExtentTransfnAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TemporalExtentTransfn";
