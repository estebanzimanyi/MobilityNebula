...
/**
 * @brief Windowed TIMESTAMPTZ_TCOUNT_TRANSFN -> aggregate Temporal (hex-WKB) via timestamptz_tcount_transfn/temporal_tagg_finalfn.
 *
 ...
 * sequence and calls MEOS `timestamptz_tcount_transfn` to fold it to a single scalar.
 */
class TimestamptzTcountTransfnAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TimestamptzTcountTransfnAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~TimestamptzTcountTransfnAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TimestamptzTcountTransfn";
