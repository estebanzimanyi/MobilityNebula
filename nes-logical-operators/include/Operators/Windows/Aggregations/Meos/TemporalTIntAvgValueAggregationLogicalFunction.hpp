...
/**
 * @brief Arithmetic mean (as double) of all instant values in the per-(window, group) tint sequence.
 *
 ...
 * sequence and calls MEOS `tnumber_avg_value` to fold it to a single scalar.
 */
class TemporalTIntAvgValueAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TemporalTIntAvgValueAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~TemporalTIntAvgValueAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TemporalTIntAvgValue";
