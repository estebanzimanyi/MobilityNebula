...
/**
 * @brief Maximum value across instants of the per-(window, group) tint sequence.
 *
 ...
 * sequence and calls MEOS `tint_max_value` to fold it to a single scalar.
 */
class TemporalTIntMaxValueAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TemporalTIntMaxValueAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~TemporalTIntMaxValueAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TemporalTIntMaxValue";
