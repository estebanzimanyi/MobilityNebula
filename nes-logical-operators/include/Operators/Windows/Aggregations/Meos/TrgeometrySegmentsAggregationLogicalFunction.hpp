...
/**
 * @brief trgeometry_segments (TSequence **) - the array of maximal segments of the windowed continuous trgeometry, each as EWKB hex.
 *
 ...
 * MEOS sequence accessor `trgeometry_segments(static_cast<Temporal*>(temp), &_cnt)` to fold it.
 */
class TrgeometrySegmentsAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TrgeometrySegmentsAggregationLogicalFunction(
    ...
    ~TrgeometrySegmentsAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TrgeometrySegments";
