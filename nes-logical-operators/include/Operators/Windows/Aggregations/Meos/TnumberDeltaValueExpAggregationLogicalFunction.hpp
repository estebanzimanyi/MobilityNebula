...
/**
 * @brief Windowed tnumber_delta_value over the expandable tfloat mini-series, emitted as hex-WKB.
 *
 ...
 * sequence and calls MEOS `tnumber_delta_value` to fold it to a single scalar.
 */
class TnumberDeltaValueExpAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TnumberDeltaValueExpAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~TnumberDeltaValueExpAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TNUMBER_DELTA_VALUE_EXP";
