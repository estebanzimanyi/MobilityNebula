...
/**
 * @brief Windowed TEMPORAL_MERGE_TRANSFN -> aggregate Temporal (hex-WKB) via temporal_merge_transfn/temporal_tagg_finalfn.
 *
 ...
 * sequence and calls MEOS `temporal_merge_transfn` to fold it to a single scalar.
 */
class TemporalMergeTransfnAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TemporalMergeTransfnAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~TemporalMergeTransfnAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TemporalMergeTransfn";
