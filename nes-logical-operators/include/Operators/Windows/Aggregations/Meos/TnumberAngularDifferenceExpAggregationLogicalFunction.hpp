...
/**
 * @brief Windowed tnumber_angular_difference over the expandable tfloat mini-series, emitted as hex-WKB.
 *
 ...
 * sequence and calls MEOS `tnumber_angular_difference` to fold it to a single scalar.
 */
class TnumberAngularDifferenceExpAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TnumberAngularDifferenceExpAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~TnumberAngularDifferenceExpAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TNUMBER_ANGULAR_DIFFERENCE_EXP";
