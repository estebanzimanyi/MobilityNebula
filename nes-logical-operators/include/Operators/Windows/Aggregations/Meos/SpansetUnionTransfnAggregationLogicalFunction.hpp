...
/**
 * @brief Windowed SPANSET_UNION_TRANSFN -> unioned container (text) via spanset_union_transfn/spanset_union_finalfn.
 *
 ...
 * sequence and calls MEOS `spanset_union_transfn` to fold it to a single scalar.
 */
class SpansetUnionTransfnAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    SpansetUnionTransfnAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~SpansetUnionTransfnAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "SpansetUnionTransfn";
