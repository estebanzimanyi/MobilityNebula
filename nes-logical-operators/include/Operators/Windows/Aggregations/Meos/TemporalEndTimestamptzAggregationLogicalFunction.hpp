...
/**
 * @brief TimestampTz (MEOS μs-since-2000) of the last instant in the per-(window, group) tgeo trajectory.
 *
 ...
 * and calls MEOS `temporal_end_timestamptz` to fold it to a single scalar.
 */
class TemporalEndTimestamptzAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TemporalEndTimestamptzAggregationLogicalFunction(
    ...
    ~TemporalEndTimestamptzAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TemporalEndTimestamptz";
