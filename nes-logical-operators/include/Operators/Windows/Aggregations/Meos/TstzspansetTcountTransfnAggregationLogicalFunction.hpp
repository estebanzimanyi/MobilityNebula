...
/**
 * @brief Windowed TSTZSPANSET_TCOUNT_TRANSFN -> aggregate Temporal (hex-WKB) via tstzspanset_tcount_transfn/temporal_tagg_finalfn.
 *
 ...
 * sequence and calls MEOS `tstzspanset_tcount_transfn` to fold it to a single scalar.
 */
class TstzspansetTcountTransfnAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TstzspansetTcountTransfnAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~TstzspansetTcountTransfnAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TstzspansetTcountTransfn";
