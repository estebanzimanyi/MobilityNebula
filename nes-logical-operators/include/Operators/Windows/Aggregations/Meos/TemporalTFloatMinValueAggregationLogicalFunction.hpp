...
/**
 * @brief Minimum value across instants of the per-(window, group) tfloat sequence.
 *
 ...
 * sequence and calls MEOS `tfloat_min_value` to fold it to a single scalar.
 */
class TemporalTFloatMinValueAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TemporalTFloatMinValueAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~TemporalTFloatMinValueAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TemporalTFloatMinValue";
