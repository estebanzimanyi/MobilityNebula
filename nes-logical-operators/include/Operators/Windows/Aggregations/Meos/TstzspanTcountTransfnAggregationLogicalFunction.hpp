...
/**
 * @brief Windowed TSTZSPAN_TCOUNT_TRANSFN -> aggregate Temporal (hex-WKB) via tstzspan_tcount_transfn/temporal_tagg_finalfn.
 *
 ...
 * sequence and calls MEOS `tstzspan_tcount_transfn` to fold it to a single scalar.
 */
class TstzspanTcountTransfnAggregationLogicalFunction : public WindowAggregationLogicalFunction
{
public:
    ...
    TstzspanTcountTransfnAggregationLogicalFunction(
        const FieldAccessLogicalFunction& valueField,
        const FieldAccessLogicalFunction& timestampField,
        const FieldAccessLogicalFunction& asField);

    void inferStamp(const Schema& schema) override;
    ~TstzspanTcountTransfnAggregationLogicalFunction() override = default;
...
private:
    static constexpr std::string_view NAME = "TstzspanTcountTransfn";
