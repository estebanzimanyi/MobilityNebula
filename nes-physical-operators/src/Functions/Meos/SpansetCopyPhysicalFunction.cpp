...
#include <Functions/Meos/SpansetCopyPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterSpansetCopyPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

SpansetCopyPhysicalFunction::SpansetCopyPhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal SpansetCopyPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterSpansetCopyPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "SpansetCopyPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return SpansetCopyPhysicalFunction(std::move(arg0));
}
