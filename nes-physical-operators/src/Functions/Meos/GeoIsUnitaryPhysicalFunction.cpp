...
GeoIsUnitaryPhysicalFunction::GeoIsUnitaryPhysicalFunction(PhysicalFunction wkt)
{
    paramFns.reserve(1);
    ...
    const auto result = nautilus::invoke(
        +[](const char* w1, uint32_t w1sz) -> double {
            try {
                ...
                if (!gs1) return 0.0;
                double r = geo_is_unitary(gs1) ? 1.0 : 0.0;
                free(gs1);
                return r;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "GeoIsUnitaryPhysicalFunction requires 1 child but got {}",
                 arguments.childFunctions.size());
    return GeoIsUnitaryPhysicalFunction(std::move(arguments.childFunctions[0]));
}
