...
#include <Functions/Meos/TemporalInterpPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTemporalInterpPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TemporalInterpPhysicalFunction::TemporalInterpPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TemporalInterpPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                const char* res = temporal_interp(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTemporalInterpPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TemporalInterpPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TemporalInterpPhysicalFunction(std::move(arg0), std::move(arg1));
}
