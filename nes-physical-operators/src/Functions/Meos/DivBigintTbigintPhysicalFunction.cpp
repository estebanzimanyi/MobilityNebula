...
#include <Functions/Meos/DivBigintTbigintPhysicalFunction.hpp>
...
DivBigintTbigintPhysicalFunction::DivBigintTbigintPhysicalFunction(PhysicalFunction trajFunction,
...
VarVal DivBigintTbigintPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto hexStr = nautilus::invoke(
        +[](const char* aPtr, uint32_t aSize, int64_t s) -> char*
        {
            try
            {
                ...
                Temporal* res = div_bigint_tbigint(s, a);
                free(a);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterDivBigintTbigintPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "DivBigintTbigintPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return DivBigintTbigintPhysicalFunction(std::move(arg0), std::move(arg1));
}
