...
#include <Functions/Meos/TspatialToStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTspatialToStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TspatialToStboxPhysicalFunction::TspatialToStboxPhysicalFunction(PhysicalFunction lonFunction,
...
VarVal TspatialToStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                STBox* res = (STBox*) tspatial_to_stbox(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTspatialToStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TspatialToStboxPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TspatialToStboxPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
