...
#include <Functions/Meos/TemporalNADTIntPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTemporalNADTIntPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TemporalNADTIntPhysicalFunction::TemporalNADTIntPhysicalFunction(PhysicalFunction valueAFunction,
...
VarVal TemporalNADTIntPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto valueA = parameterValues[0].cast<nautilus::val<int32_t>>();
    auto tsA    = parameterValues[1].cast<nautilus::val<uint64_t>>();
    auto valueB = parameterValues[2].cast<nautilus::val<int32_t>>();
    auto tsB    = parameterValues[3].cast<nautilus::val<uint64_t>>();

    const auto result = nautilus::invoke(
        +[](int32_t valueAValue, uint64_t tsAValue,
            int32_t valueBValue, uint64_t tsBValue) -> int {
            try
            {
                ...
                Temporal* tempA = tint_in(wktA.c_str());
                if (!tempA) return 0;
                Temporal* tempB = tint_in(wktB.c_str());
                if (!tempB) { free(tempA); return 0; }
                int r = nad_tint_tint(tempA, tempB);
                free(tempA);
                free(tempB);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTemporalNADTIntPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 4,
                 "TemporalNADTIntPhysicalFunction requires 4 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    auto arg3 = std::move(arguments.childFunctions[3]);
    return TemporalNADTIntPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3));
}
