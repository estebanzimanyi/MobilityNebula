...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterGeoSetSridPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

GeoSetSridPhysicalFunction::GeoSetSridPhysicalFunction(PhysicalFunction litFunction)
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                GSERIALIZED* res = (GSERIALIZED*) geo_set_srid(temp, 4326);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterGeoSetSridPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "GeoSetSridPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return GeoSetSridPhysicalFunction(std::move(arg0));
}
