...
#include <Functions/Meos/StboxGetSpaceTilePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterStboxGetSpaceTilePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

StboxGetSpaceTilePhysicalFunction::StboxGetSpaceTilePhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal StboxGetSpaceTilePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                STBox* res = (STBox*) stbox_get_space_tile(temp, 5.0, 5.0, 5.0, geom_in((char*)"Point(0 0 0)", -1));
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterStboxGetSpaceTilePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "StboxGetSpaceTilePhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return StboxGetSpaceTilePhysicalFunction(std::move(arg0));
}
