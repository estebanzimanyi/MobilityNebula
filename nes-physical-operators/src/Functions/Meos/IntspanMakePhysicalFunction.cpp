...
IntspanMakePhysicalFunction::IntspanMakePhysicalFunction(PhysicalFunction lowerFunction,
    ...
    const auto actualLen = nautilus::invoke(
        +[](double lower,
            ...
            try
            {
                ...
                char* out = ({Span* _sp=intspan_make((int)lower,(int)upper,lower_inc!=0.0,upper_inc!=0.0);if(!_sp)return 0u;char* _out=intspan_out(_sp);free(_sp);_out;});
                if (!out) return 0u;
