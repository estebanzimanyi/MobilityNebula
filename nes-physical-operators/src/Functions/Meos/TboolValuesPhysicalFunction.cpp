...
#include <Functions/Meos/TboolValuesPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTboolValuesPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TboolValuesPhysicalFunction::TboolValuesPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TboolValuesPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                int _cnt = 0;
                bool* arr = (bool*) tbool_values(temp, &_cnt);
                free(temp);
                if (!arr || _cnt <= 0) return (char*) nullptr;
                std::string _s = "{";
                for (int _i = 0; _i < _cnt; _i++) { if (_i) _s += ", "; _s += (arr[_i] ? "t" : "f"); }
                _s += "}";
                free(arr);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTboolValuesPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TboolValuesPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TboolValuesPhysicalFunction(std::move(arg0), std::move(arg1));
}
