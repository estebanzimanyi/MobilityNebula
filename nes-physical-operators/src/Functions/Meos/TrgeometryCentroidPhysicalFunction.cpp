...
#include <Functions/Meos/TrgeometryCentroidPhysicalFunction.hpp>
...
TrgeometryCentroidPhysicalFunction::TrgeometryCentroidPhysicalFunction(PhysicalFunction trajFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(trajFunction));
}

VarVal TrgeometryCentroidPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto hexStr = nautilus::invoke(
        +[](const char* aPtr, uint32_t aSize) -> char*
        {
            try
            {
                ...
                Temporal* res = trgeometry_centroid(a);
                free(a);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTrgeometryCentroidPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "TrgeometryCentroidPhysicalFunction requires 1 child but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return TrgeometryCentroidPhysicalFunction(std::move(arg0));
}
