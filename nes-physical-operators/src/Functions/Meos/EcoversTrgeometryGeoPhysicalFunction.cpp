...
#include <Functions/Meos/EcoversTrgeometryGeoPhysicalFunction.hpp>
...
EcoversTrgeometryGeoPhysicalFunction::EcoversTrgeometryGeoPhysicalFunction(PhysicalFunction ref_wkt, PhysicalFunction x1, PhysicalFunction y1, PhysicalFunction theta1, PhysicalFunction ts1, PhysicalFunction tgt_wkt) {
    paramFns.reserve(6);
...
VarVal EcoversTrgeometryGeoPhysicalFunction::execute(const Record& record, ArenaRef& arena) const {
    ...
    const auto result = nautilus::invoke(
        +[](const char* ref_wkt, uint32_t ref_wktsz, double x1, double y1, double theta1, uint64_t ts1, const char* tgt_wkt, uint32_t tgt_wktsz) -> double {
            try {
                ...
                int r = ecovers_trgeometry_geo((Temporal*)inst1, gs_tgt);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterEcoversTrgeometryGeoPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size()==6,
                 "EcoversTrgeometryGeoPhysicalFunction requires 6 children but got {}",
                 arguments.childFunctions.size());
    return EcoversTrgeometryGeoPhysicalFunction(
