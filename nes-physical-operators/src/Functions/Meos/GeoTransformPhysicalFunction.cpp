...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterGeoTransformPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

GeoTransformPhysicalFunction::GeoTransformPhysicalFunction(PhysicalFunction litFunction)
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                GSERIALIZED* res = (GSERIALIZED*) geo_transform(temp, 3857);
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "GeoTransformPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return GeoTransformPhysicalFunction(std::move(arg0));
}
