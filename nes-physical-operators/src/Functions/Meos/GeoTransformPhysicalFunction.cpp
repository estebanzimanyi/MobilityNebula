...
GeoTransformPhysicalFunction::GeoTransformPhysicalFunction(PhysicalFunction wktFunction,
    ...
    const auto actualLen = nautilus::invoke(
        +[](const char* wktPtr, uint32_t wktSize,
            ...
            try
            {
                ...
                GSERIALIZED* gres = geo_transform(temp, (int32_t)srid);
                free(temp);
                if (!gres) return 0u;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return GeoTransformPhysicalFunction(std::move(arg0), std::move(arg1));
}
