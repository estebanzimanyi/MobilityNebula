...
#include <Functions/Meos/TgeoValuesPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTgeoValuesPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TgeoValuesPhysicalFunction::TgeoValuesPhysicalFunction(PhysicalFunction lonFunction,
...
VarVal TgeoValuesPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                int _cnt = 0;
                GSERIALIZED ** arr = (GSERIALIZED **) tgeo_values(temp, &_cnt);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTgeoValuesPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TgeoValuesPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TgeoValuesPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
