...
#include <Functions/Meos/UnionSpanSpansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterUnionSpanSpansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

UnionSpanSpansetPhysicalFunction::UnionSpanSpansetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal UnionSpanSpansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                SpanSet* res = union_span_spanset(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterUnionSpanSpansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "UnionSpanSpansetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return UnionSpanSpansetPhysicalFunction(std::move(arg0), std::move(arg1));
}
