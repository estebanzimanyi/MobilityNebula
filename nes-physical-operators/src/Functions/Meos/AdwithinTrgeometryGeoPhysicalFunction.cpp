...
#include <Functions/Meos/AdwithinTrgeometryGeoPhysicalFunction.hpp>
...
AdwithinTrgeometryGeoPhysicalFunction::AdwithinTrgeometryGeoPhysicalFunction(PhysicalFunction ref_wkt, PhysicalFunction x1, PhysicalFunction y1, PhysicalFunction theta1, PhysicalFunction ts1, PhysicalFunction tgt_wkt, PhysicalFunction dist) {
    paramFns.reserve(7);
...
VarVal AdwithinTrgeometryGeoPhysicalFunction::execute(const Record& record, ArenaRef& arena) const {
    ...
    const auto result = nautilus::invoke(
        +[](const char* ref_wkt, uint32_t ref_wktsz, double x1, double y1, double theta1, uint64_t ts1, const char* tgt_wkt, uint32_t tgt_wktsz, double dist) -> double {
            try {
                ...
                int r = adwithin_trgeometry_geo((Temporal*)inst1, gs_tgt, dist);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterAdwithinTrgeometryGeoPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size()==7,
                 "AdwithinTrgeometryGeoPhysicalFunction requires 7 children but got {}",
                 arguments.childFunctions.size());
    return AdwithinTrgeometryGeoPhysicalFunction(
