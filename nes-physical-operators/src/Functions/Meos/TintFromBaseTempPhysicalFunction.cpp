...
#include <Functions/Meos/TintFromBaseTempPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTintFromBaseTempPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TintFromBaseTempPhysicalFunction::TintFromBaseTempPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TintFromBaseTempPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int32_t arg0) -> char*
        {
            try
            {
                ...
                Temporal* res = tint_from_base_temp(arg0, temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTintFromBaseTempPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TintFromBaseTempPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TintFromBaseTempPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
