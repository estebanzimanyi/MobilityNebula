...
#include <Functions/Meos/GeoTstzspanToStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterGeoTstzspanToStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

GeoTstzspanToStboxPhysicalFunction::GeoTstzspanToStboxPhysicalFunction(PhysicalFunction litFunction,
...
VarVal GeoTstzspanToStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                STBox* res = (STBox*) geo_tstzspan_to_stbox(temp, arg0B);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterGeoTstzspanToStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "GeoTstzspanToStboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return GeoTstzspanToStboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
