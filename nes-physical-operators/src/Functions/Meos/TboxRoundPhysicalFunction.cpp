...
#include <Functions/Meos/TboxRoundPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTboxRoundPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TboxRoundPhysicalFunction::TboxRoundPhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal TboxRoundPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTboxRoundPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "TboxRoundPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return TboxRoundPhysicalFunction(std::move(arg0));
}
