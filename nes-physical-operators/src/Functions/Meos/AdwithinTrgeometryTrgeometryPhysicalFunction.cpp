...
#include <Functions/Meos/AdwithinTrgeometryTrgeometryPhysicalFunction.hpp>
...
VarVal AdwithinTrgeometryTrgeometryPhysicalFunction::execute(const Record& record, ArenaRef& arena) const {
    ...
    const auto result = nautilus::invoke(
        +[](const char* ref1_wkt, uint32_t ref1_wktsz, double x1, double y1, double theta1, uint64_t ts1, const char* ref2_wkt, uint32_t ref2_wktsz, double x2, double y2, double theta2, uint64_t ts2, double dist) -> double {
            try {
                ...
                int r = adwithin_trgeometry_trgeometry((Temporal*)inst1, (Temporal*)inst2, dist);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterAdwithinTrgeometryTrgeometryPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size()==11,
                 "AdwithinTrgeometryTrgeometryPhysicalFunction requires 11 children but got {}",
                 arguments.childFunctions.size());
    return AdwithinTrgeometryTrgeometryPhysicalFunction(
