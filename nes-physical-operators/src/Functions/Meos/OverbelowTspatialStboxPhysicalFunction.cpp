...
#include <Functions/Meos/OverbelowTspatialStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterOverbelowTspatialStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

OverbelowTspatialStboxPhysicalFunction::OverbelowTspatialStboxPhysicalFunction(PhysicalFunction lonFunction,
...
VarVal OverbelowTspatialStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](double lon,
            ...
            try
            {
                ...
                bool r = overbelow_tspatial_stbox(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterOverbelowTspatialStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 4,
                 "OverbelowTspatialStboxPhysicalFunction requires 4 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    auto arg3 = std::move(arguments.childFunctions[3]);
    return OverbelowTspatialStboxPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3));
}
