...
#include <Functions/Meos/BigintspanBinsPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterBigintspanBinsPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

BigintspanBinsPhysicalFunction::BigintspanBinsPhysicalFunction(PhysicalFunction litFunction,
...
VarVal BigintspanBinsPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int64_t arg1) -> char*
        {
            try
            {
                ...
                int _cnt = 0;
                Span* arr = (Span*) bigintspan_bins(temp, arg0, arg1, &_cnt);
                free(temp);
                if (!arr || _cnt <= 0) return (char*) nullptr;
                std::string _s = "{";
                for (int _i = 0; _i < _cnt; _i++) { if (_i) _s += ", "; char* _e = bigintspan_out(&arr[_i]); if (_e) { _s += _e; free(_e); } }
                _s += "}";
                free(arr);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterBigintspanBinsPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "BigintspanBinsPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return BigintspanBinsPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
