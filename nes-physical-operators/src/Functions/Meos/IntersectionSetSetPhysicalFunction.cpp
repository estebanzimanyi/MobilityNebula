...
#include <Functions/Meos/IntersectionSetSetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterIntersectionSetSetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

IntersectionSetSetPhysicalFunction::IntersectionSetSetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal IntersectionSetSetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Set* arg0B = intset_in(arg0S.c_str());
                if (!arg0B) { free(temp); return (char*) nullptr; }

                Set* res = intersection_set_set(temp, arg0B);
                free(temp);
                free(arg0B);
                if (!res) return (char*) nullptr;
                char* outStr = intset_out(res);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterIntersectionSetSetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "IntersectionSetSetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return IntersectionSetSetPhysicalFunction(std::move(arg0), std::move(arg1));
}
