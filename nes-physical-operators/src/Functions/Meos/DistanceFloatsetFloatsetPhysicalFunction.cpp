...
#include <Functions/Meos/DistanceFloatsetFloatsetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterDistanceFloatsetFloatsetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

DistanceFloatsetFloatsetPhysicalFunction::DistanceFloatsetFloatsetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal DistanceFloatsetFloatsetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                Set* arg0B = floatset_in(arg0S.c_str());
                if (!arg0B) { free(temp); return 0.0; }

                double r = distance_floatset_floatset(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterDistanceFloatsetFloatsetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "DistanceFloatsetFloatsetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return DistanceFloatsetFloatsetPhysicalFunction(std::move(arg0), std::move(arg1));
}
