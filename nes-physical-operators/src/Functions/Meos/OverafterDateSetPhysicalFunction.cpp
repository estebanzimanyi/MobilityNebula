...
#include <Functions/Meos/OverafterDateSetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterOverafterDateSetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

OverafterDateSetPhysicalFunction::OverafterDateSetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal OverafterDateSetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = overafter_date_set(arg0V, temp);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterOverafterDateSetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "OverafterDateSetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return OverafterDateSetPhysicalFunction(std::move(arg0), std::move(arg1));
}
