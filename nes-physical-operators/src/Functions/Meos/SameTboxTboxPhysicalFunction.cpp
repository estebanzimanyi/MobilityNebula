...
#include <Functions/Meos/SameTboxTboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterSameTboxTboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

SameTboxTboxPhysicalFunction::SameTboxTboxPhysicalFunction(PhysicalFunction litFunction,
...
VarVal SameTboxTboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = same_tbox_tbox(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterSameTboxTboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "SameTboxTboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return SameTboxTboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
