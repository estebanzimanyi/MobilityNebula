...
#include <Functions/Meos/EdisjointTrgeometryTrgeometryPhysicalFunction.hpp>
...
VarVal EdisjointTrgeometryTrgeometryPhysicalFunction::execute(const Record& record, ArenaRef& arena) const {
    ...
    const auto result = nautilus::invoke(
        +[](const char* ref1_wkt, uint32_t ref1_wktsz, double x1, double y1, double theta1, uint64_t ts1, const char* ref2_wkt, uint32_t ref2_wktsz, double x2, double y2, double theta2, uint64_t ts2) -> double {
            try {
                ...
                int r = edisjoint_trgeometry_trgeometry((Temporal*)inst1, (Temporal*)inst2);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterEdisjointTrgeometryTrgeometryPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size()==10,
                 "EdisjointTrgeometryTrgeometryPhysicalFunction requires 10 children but got {}",
                 arguments.childFunctions.size());
    return EdisjointTrgeometryTrgeometryPhysicalFunction(
