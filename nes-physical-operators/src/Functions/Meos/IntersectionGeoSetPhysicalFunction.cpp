...
#include <Functions/Meos/IntersectionGeoSetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterIntersectionGeoSetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

IntersectionGeoSetPhysicalFunction::IntersectionGeoSetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal IntersectionGeoSetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Set* res = intersection_geo_set(arg0G.getGeometry(), temp);
                free(temp);
                if (!res) return (char*) nullptr;
                char* outStr = spatialset_as_text(res, 15);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterIntersectionGeoSetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "IntersectionGeoSetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return IntersectionGeoSetPhysicalFunction(std::move(arg0), std::move(arg1));
}
