...
#include <Functions/Meos/TrgeometryCumulativeLengthPhysicalFunction.hpp>
...
TrgeometryCumulativeLengthPhysicalFunction::TrgeometryCumulativeLengthPhysicalFunction(PhysicalFunction trajFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(trajFunction));
}

VarVal TrgeometryCumulativeLengthPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto hexStr = nautilus::invoke(
        +[](const char* aPtr, uint32_t aSize) -> char*
        {
            try
            {
                ...
                Temporal* res = trgeometry_cumulative_length(a);
                free(a);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTrgeometryCumulativeLengthPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "TrgeometryCumulativeLengthPhysicalFunction requires 1 child but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return TrgeometryCumulativeLengthPhysicalFunction(std::move(arg0));
}
