...
GeomUnaryUnionPhysicalFunction::GeomUnaryUnionPhysicalFunction(PhysicalFunction wkt)
{
    paramFns.reserve(1);
    ...
    const auto actualLen = nautilus::invoke(
        +[](const char* wkt, uint32_t wsz, char* buf, uint32_t bufMax) -> uint32_t {
            try {
                ...
                if (!gs) return 0u;
                GSERIALIZED* result = geom_unary_union(gs, 0.0);
                free(gs);
                if (!result) return 0u;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return GeomUnaryUnionPhysicalFunction(
