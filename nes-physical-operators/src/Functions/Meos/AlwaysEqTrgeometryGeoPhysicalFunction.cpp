...
#include <Functions/Meos/AlwaysEqTrgeometryGeoPhysicalFunction.hpp>
...
AlwaysEqTrgeometryGeoPhysicalFunction::AlwaysEqTrgeometryGeoPhysicalFunction(PhysicalFunction ref_wkt, PhysicalFunction x, PhysicalFunction y, PhysicalFunction theta, PhysicalFunction ts, PhysicalFunction tgt_wkt)
{
    paramFns.reserve(6);
    ...
    const auto result = nautilus::invoke(
        +[](const char* ref_wkt, uint32_t ref_len, double x, double y, double theta, uint64_t ts, const char* tgt_wkt, uint32_t tgt_len) -> double {
            try {
                ...
                int r = always_eq_trgeometry_geo((Temporal*)inst, gs_tgt);
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 6,
                 "AlwaysEqTrgeometryGeoPhysicalFunction requires 6 children but got {}",
                 arguments.childFunctions.size());
    return AlwaysEqTrgeometryGeoPhysicalFunction(
