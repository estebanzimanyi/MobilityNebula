...
#include <Functions/Meos/TrgeometrySpeedPhysicalFunction.hpp>
...
TrgeometrySpeedPhysicalFunction::TrgeometrySpeedPhysicalFunction(PhysicalFunction trajFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(trajFunction));
}

VarVal TrgeometrySpeedPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto hexStr = nautilus::invoke(
        +[](const char* aPtr, uint32_t aSize) -> char*
        {
            try
            {
                ...
                Temporal* res = trgeometry_speed(a);
                free(a);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTrgeometrySpeedPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "TrgeometrySpeedPhysicalFunction requires 1 child but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return TrgeometrySpeedPhysicalFunction(std::move(arg0));
}
