...
JsonbObjectFieldTextPhysicalFunction::JsonbObjectFieldTextPhysicalFunction(PhysicalFunction jbFunction,
    ...
    const auto actualLen = nautilus::invoke(
        +[](const char* jbPtr, uint32_t jbSize,
            ...
            try
            {
                ...
                text* tres = jsonb_object_field_text(temp, txt0);
                free(temp);
                free(txt0);
                if (!tres) return 0u;
        ...
        jb.getContent(), jb.getContentSize(), arg0.getContent(), arg0.getContentSize(), outBuf.getContent(), nautilus::val<uint32_t>(MAX_LEN));
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return JsonbObjectFieldTextPhysicalFunction(std::move(arg0), std::move(arg1));
}
