...
GeoFromGeojsonPhysicalFunction::GeoFromGeojsonPhysicalFunction(PhysicalFunction jsonFunction)
{
    ...
    const auto actualLen = nautilus::invoke(
        +[](const char* jsonPtr, uint32_t jsonSize,
            ...
            try
            {
                ...
                GSERIALIZED* gres = geo_from_geojson(std::string(jsonPtr,jsonSize).c_str());
                if (!gres) return 0u;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return GeoFromGeojsonPhysicalFunction(std::move(arg0));
}
