...
EverNeTquadbinTquadbinPhysicalFunction::EverNeTquadbinTquadbinPhysicalFunction(PhysicalFunction cell1, PhysicalFunction ts1, PhysicalFunction cell2, PhysicalFunction ts2) {
    paramFns.reserve(4);
...
VarVal EverNeTquadbinTquadbinPhysicalFunction::execute(const Record& record, ArenaRef& arena) const {
    ...
    const auto result = nautilus::invoke(
        +[](uint64_t c1, uint64_t t1, uint64_t c2, uint64_t t2) -> double {
            ...
            int r = ever_ne_tquadbin_tquadbin((const Temporal*)i1, (const Temporal*)i2);
            free(i1); free(i2);
            return r > 0 ? 1.0 : 0.0;
        },
        cell1, ts1, cell2, ts2);
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return EverNeTquadbinTquadbinPhysicalFunction(
