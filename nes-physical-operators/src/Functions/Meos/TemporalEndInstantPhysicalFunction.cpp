...
#include <Functions/Meos/TemporalEndInstantPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTemporalEndInstantPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TemporalEndInstantPhysicalFunction::TemporalEndInstantPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TemporalEndInstantPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) temporal_end_instant(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTemporalEndInstantPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TemporalEndInstantPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TemporalEndInstantPhysicalFunction(std::move(arg0), std::move(arg1));
}
