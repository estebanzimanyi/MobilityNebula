...
#include <Functions/Meos/BeforeTboxTboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterBeforeTboxTboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

BeforeTboxTboxPhysicalFunction::BeforeTboxTboxPhysicalFunction(PhysicalFunction litFunction,
...
VarVal BeforeTboxTboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = before_tbox_tbox(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterBeforeTboxTboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "BeforeTboxTboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return BeforeTboxTboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
