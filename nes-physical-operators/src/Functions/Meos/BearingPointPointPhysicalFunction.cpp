...
#include <Functions/Meos/BearingPointPointPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterBearingPointPointPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

BearingPointPointPhysicalFunction::BearingPointPointPhysicalFunction(PhysicalFunction litFunction,
...
VarVal BearingPointPointPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                if (!arg0G.getGeometry()) { return 0.0; }

                double res;
                bool ok = bearing_point_point(temp, arg0G.getGeometry(), &res);
                if (!ok) return 0.0;
                return (double) res;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterBearingPointPointPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "BearingPointPointPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return BearingPointPointPhysicalFunction(std::move(arg0), std::move(arg1));
}
