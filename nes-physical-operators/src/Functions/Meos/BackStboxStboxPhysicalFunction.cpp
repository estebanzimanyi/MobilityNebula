...
#include <Functions/Meos/BackStboxStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterBackStboxStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

BackStboxStboxPhysicalFunction::BackStboxStboxPhysicalFunction(PhysicalFunction boxFunction,
...
VarVal BackStboxStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* boxPtr, uint32_t boxSize,
            ...
            try
            {
                ...
                bool r = back_stbox_stbox(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterBackStboxStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "BackStboxStboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return BackStboxStboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
