...
#include <Functions/Meos/TgeoValueNPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTgeoValueNPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TgeoValueNPhysicalFunction::TgeoValueNPhysicalFunction(PhysicalFunction lonFunction,
...
VarVal TgeoValueNPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                bool ok = tgeo_value_n(temp, 1, &res);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTgeoValueNPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TgeoValueNPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TgeoValueNPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
