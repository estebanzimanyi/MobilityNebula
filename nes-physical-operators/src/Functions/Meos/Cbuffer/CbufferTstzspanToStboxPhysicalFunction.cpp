...
#include <Functions/Meos/Cbuffer/CbufferTstzspanToStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterCbufferTstzspanToStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

CbufferTstzspanToStboxPhysicalFunction::CbufferTstzspanToStboxPhysicalFunction(PhysicalFunction litFunction,
...
VarVal CbufferTstzspanToStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                STBox* res = (STBox*) cbuffer_tstzspan_to_stbox(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterCbufferTstzspanToStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "CbufferTstzspanToStboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return CbufferTstzspanToStboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
