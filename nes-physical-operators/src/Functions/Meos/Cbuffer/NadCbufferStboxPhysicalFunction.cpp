...
#include <Functions/Meos/Cbuffer/NadCbufferStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterNadCbufferStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

NadCbufferStboxPhysicalFunction::NadCbufferStboxPhysicalFunction(PhysicalFunction litFunction,
...
VarVal NadCbufferStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                double r = nad_cbuffer_stbox(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterNadCbufferStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "NadCbufferStboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return NadCbufferStboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
