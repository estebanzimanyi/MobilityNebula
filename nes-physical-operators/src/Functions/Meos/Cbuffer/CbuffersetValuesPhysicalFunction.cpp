...
#include <Functions/Meos/Cbuffer/CbuffersetValuesPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterCbuffersetValuesPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

CbuffersetValuesPhysicalFunction::CbuffersetValuesPhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal CbuffersetValuesPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                Set* temp = cbufferset_in(tempS.c_str());
                if (!temp) return (char*) nullptr;

                int _cnt = 0;
                Cbuffer ** arr = (Cbuffer **) cbufferset_values(temp, &_cnt);
                free(temp);
                if (!arr || _cnt <= 0) return (char*) nullptr;
                std::string _s = "{";
                for (int _i = 0; _i < _cnt; _i++) { if (_i) _s += ", "; char* _e = cbuffer_out(arr[_i], 15); if (_e) { _s += _e; free(_e); } free(arr[_i]); }
                _s += "}";
                free(arr);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterCbuffersetValuesPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "CbuffersetValuesPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return CbuffersetValuesPhysicalFunction(std::move(arg0));
}
