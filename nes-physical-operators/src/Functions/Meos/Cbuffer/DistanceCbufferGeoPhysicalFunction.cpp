...
#include <Functions/Meos/Cbuffer/DistanceCbufferGeoPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterDistanceCbufferGeoPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

DistanceCbufferGeoPhysicalFunction::DistanceCbufferGeoPhysicalFunction(PhysicalFunction litFunction,
...
VarVal DistanceCbufferGeoPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                if (!arg0G.getGeometry()) { free(temp); return 0.0; }

                double r = distance_cbuffer_geo(temp, arg0G.getGeometry());
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterDistanceCbufferGeoPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "DistanceCbufferGeoPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return DistanceCbufferGeoPhysicalFunction(std::move(arg0), std::move(arg1));
}
