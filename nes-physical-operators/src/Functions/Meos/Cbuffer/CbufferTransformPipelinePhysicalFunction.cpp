...
#include <Functions/Meos/Cbuffer/CbufferTransformPipelinePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterCbufferTransformPipelinePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

CbufferTransformPipelinePhysicalFunction::CbufferTransformPipelinePhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal CbufferTransformPipelinePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                Cbuffer* temp = cbuffer_in(tempS.c_str());
                if (!temp) return (char*) nullptr;

                Cbuffer* res = (Cbuffer*) cbuffer_transform_pipeline(temp, (char*)"+proj=pipeline +step +proj=unitconvert +xy_in=deg +xy_out=rad +step +proj=webmerc +ellps=WGS84", 3857, true);
                free(temp);
                if (!res) return (char*) nullptr;
                char* outStr = cbuffer_out(res, 15);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterCbufferTransformPipelinePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "CbufferTransformPipelinePhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return CbufferTransformPipelinePhysicalFunction(std::move(arg0));
}
