...
#include <Functions/Meos/Cbuffer/CbuffersetStartValuePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterCbuffersetStartValuePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

CbuffersetStartValuePhysicalFunction::CbuffersetStartValuePhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal CbuffersetStartValuePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                Cbuffer* res = cbufferset_start_value(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterCbuffersetStartValuePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "CbuffersetStartValuePhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return CbuffersetStartValuePhysicalFunction(std::move(arg0));
}
