...
#include <Functions/Meos/Cbuffer/IntersectionSetCbufferPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterIntersectionSetCbufferPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

IntersectionSetCbufferPhysicalFunction::IntersectionSetCbufferPhysicalFunction(PhysicalFunction litFunction,
...
VarVal IntersectionSetCbufferPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Set* res = intersection_set_cbuffer(temp, arg0B);
                free(temp);
                free(arg0B);
                if (!res) return (char*) nullptr;
                char* outStr = cbufferset_out(res, 15);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterIntersectionSetCbufferPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "IntersectionSetCbufferPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return IntersectionSetCbufferPhysicalFunction(std::move(arg0), std::move(arg1));
}
