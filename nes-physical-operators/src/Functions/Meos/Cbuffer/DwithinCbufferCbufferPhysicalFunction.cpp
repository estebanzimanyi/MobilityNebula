...
#include <Functions/Meos/Cbuffer/DwithinCbufferCbufferPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterDwithinCbufferCbufferPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

DwithinCbufferCbufferPhysicalFunction::DwithinCbufferCbufferPhysicalFunction(PhysicalFunction litFunction,
...
VarVal DwithinCbufferCbufferPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = dwithin_cbuffer_cbuffer(temp, arg0B, arg1);
                free(temp);
                free(arg0B);
                return r;
            }
        ...
        lit.getContent(), lit.getContentSize(), arg0.getContent(), arg0.getContentSize(), arg1);

    return VarVal(result);
}

PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterDwithinCbufferCbufferPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "DwithinCbufferCbufferPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return DwithinCbufferCbufferPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
