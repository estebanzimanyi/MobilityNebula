...
#include <Functions/Meos/Cbuffer/CbufferarrRoundPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterCbufferarrRoundPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

CbufferarrRoundPhysicalFunction::CbufferarrRoundPhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal CbufferarrRoundPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                Cbuffer* _ina[1] = { (Cbuffer*) temp };
                Cbuffer** _out = (Cbuffer**) cbufferarr_round((const Cbuffer**) _ina, 1, 6);
                free(temp);
                if (!_out || !_out[0]) return (char*) nullptr;
                char* outStr = cbuffer_out(_out[0], 6);
                free(_out[0]); free(_out);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterCbufferarrRoundPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "CbufferarrRoundPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return CbufferarrRoundPhysicalFunction(std::move(arg0));
}
