...
#include <Functions/Meos/Rgeo/TrgeometryToTposePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTrgeometryToTposePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TrgeometryToTposePhysicalFunction::TrgeometryToTposePhysicalFunction(PhysicalFunction xFunction,
...
VarVal TrgeometryToTposePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) trgeometry_to_tpose(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTrgeometryToTposePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 4,
                 "TrgeometryToTposePhysicalFunction requires 4 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    auto arg3 = std::move(arguments.childFunctions[3]);
    return TrgeometryToTposePhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3));
}
