...
#include <Functions/Meos/Rgeo/TrgeometryAfterTimestamptzPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTrgeometryAfterTimestamptzPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TrgeometryAfterTimestamptzPhysicalFunction::TrgeometryAfterTimestamptzPhysicalFunction(PhysicalFunction xFunction,
...
VarVal TrgeometryAfterTimestamptzPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) trgeometry_after_timestamptz(temp, arg0V, false);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTrgeometryAfterTimestamptzPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 5,
                 "TrgeometryAfterTimestamptzPhysicalFunction requires 5 children but got {}",
    ...
    return TrgeometryAfterTimestamptzPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4));
}
