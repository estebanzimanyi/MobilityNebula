...
#include <Functions/Meos/Rgeo/TrgeometryEndSequencePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTrgeometryEndSequencePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TrgeometryEndSequencePhysicalFunction::TrgeometryEndSequencePhysicalFunction(PhysicalFunction xFunction,
...
VarVal TrgeometryEndSequencePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) trgeometry_end_sequence(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTrgeometryEndSequencePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 4,
                 "TrgeometryEndSequencePhysicalFunction requires 4 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    auto arg3 = std::move(arguments.childFunctions[3]);
    return TrgeometryEndSequencePhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3));
}
