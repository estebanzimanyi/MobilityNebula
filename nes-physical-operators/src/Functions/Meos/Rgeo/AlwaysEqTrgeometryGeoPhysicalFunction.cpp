...
#include <Functions/Meos/Rgeo/AlwaysEqTrgeometryGeoPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterAlwaysEqTrgeometryGeoPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

AlwaysEqTrgeometryGeoPhysicalFunction::AlwaysEqTrgeometryGeoPhysicalFunction(PhysicalFunction xFunction,
...
VarVal AlwaysEqTrgeometryGeoPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](double x,
            ...
            try
            {
                ...
                if (!arg0G.getGeometry()) { free(temp); return 0; }

                int r = always_eq_trgeometry_geo(temp, arg0G.getGeometry());
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterAlwaysEqTrgeometryGeoPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 5,
                 "AlwaysEqTrgeometryGeoPhysicalFunction requires 5 children but got {}",
    ...
    return AlwaysEqTrgeometryGeoPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4));
}
