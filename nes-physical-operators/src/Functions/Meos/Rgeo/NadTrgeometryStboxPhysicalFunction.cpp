...
#include <Functions/Meos/Rgeo/NadTrgeometryStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterNadTrgeometryStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

NadTrgeometryStboxPhysicalFunction::NadTrgeometryStboxPhysicalFunction(PhysicalFunction xFunction,
...
VarVal NadTrgeometryStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](double x,
            ...
            try
            {
                ...
                double r = nad_trgeometry_stbox(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterNadTrgeometryStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 5,
                 "NadTrgeometryStboxPhysicalFunction requires 5 children but got {}",
    ...
    return NadTrgeometryStboxPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4));
}
