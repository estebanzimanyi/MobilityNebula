...
#include <Functions/Meos/Rgeo/TrgeometryRestrictTstzspanPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTrgeometryRestrictTstzspanPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TrgeometryRestrictTstzspanPhysicalFunction::TrgeometryRestrictTstzspanPhysicalFunction(PhysicalFunction xFunction,
...
VarVal TrgeometryRestrictTstzspanPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) trgeometry_restrict_tstzspan(temp, arg0B, true);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTrgeometryRestrictTstzspanPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 5,
                 "TrgeometryRestrictTstzspanPhysicalFunction requires 5 children but got {}",
    ...
    return TrgeometryRestrictTstzspanPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4));
}
