...
#include <Functions/Meos/Rgeo/NadTrgeometryGeoPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterNadTrgeometryGeoPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

NadTrgeometryGeoPhysicalFunction::NadTrgeometryGeoPhysicalFunction(PhysicalFunction xFunction,
...
VarVal NadTrgeometryGeoPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](double x,
            ...
            try
            {
                ...
                double r = nad_trgeometry_geo(temp, arg0G.getGeometry());
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterNadTrgeometryGeoPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 5,
                 "NadTrgeometryGeoPhysicalFunction requires 5 children but got {}",
    ...
    return NadTrgeometryGeoPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4));
}
