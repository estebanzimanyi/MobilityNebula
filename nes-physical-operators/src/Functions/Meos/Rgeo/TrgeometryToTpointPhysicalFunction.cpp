...
#include <Functions/Meos/Rgeo/TrgeometryToTpointPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTrgeometryToTpointPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TrgeometryToTpointPhysicalFunction::TrgeometryToTpointPhysicalFunction(PhysicalFunction xFunction,
...
VarVal TrgeometryToTpointPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) trgeometry_to_tpoint(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTrgeometryToTpointPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 4,
                 "TrgeometryToTpointPhysicalFunction requires 4 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    auto arg3 = std::move(arguments.childFunctions[3]);
    return TrgeometryToTpointPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3));
}
