...
#include <Functions/Meos/Rgeo/NaiTrgeometryGeoPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterNaiTrgeometryGeoPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

NaiTrgeometryGeoPhysicalFunction::NaiTrgeometryGeoPhysicalFunction(PhysicalFunction xFunction,
...
VarVal NaiTrgeometryGeoPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) nai_trgeometry_geo(temp, arg0G.getGeometry());
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterNaiTrgeometryGeoPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 5,
                 "NaiTrgeometryGeoPhysicalFunction requires 5 children but got {}",
    ...
    return NaiTrgeometryGeoPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4));
}
