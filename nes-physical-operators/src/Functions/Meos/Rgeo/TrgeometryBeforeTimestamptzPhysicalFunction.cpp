...
#include <Functions/Meos/Rgeo/TrgeometryBeforeTimestamptzPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTrgeometryBeforeTimestamptzPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TrgeometryBeforeTimestamptzPhysicalFunction::TrgeometryBeforeTimestamptzPhysicalFunction(PhysicalFunction xFunction,
...
VarVal TrgeometryBeforeTimestamptzPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) trgeometry_before_timestamptz(temp, arg0V, false);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTrgeometryBeforeTimestamptzPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 5,
                 "TrgeometryBeforeTimestamptzPhysicalFunction requires 5 children but got {}",
    ...
    return TrgeometryBeforeTimestamptzPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4));
}
