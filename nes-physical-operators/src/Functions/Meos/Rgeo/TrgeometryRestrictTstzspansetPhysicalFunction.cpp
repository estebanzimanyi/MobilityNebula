...
#include <Functions/Meos/Rgeo/TrgeometryRestrictTstzspansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTrgeometryRestrictTstzspansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TrgeometryRestrictTstzspansetPhysicalFunction::TrgeometryRestrictTstzspansetPhysicalFunction(PhysicalFunction xFunction,
...
VarVal TrgeometryRestrictTstzspansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                SpanSet* arg0B = tstzspanset_in(arg0S.c_str());
                if (!arg0B) { free(temp); return (char*) nullptr; }

                Temporal* res = (Temporal*) trgeometry_restrict_tstzspanset(temp, arg0B, true);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTrgeometryRestrictTstzspansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 5,
                 "TrgeometryRestrictTstzspansetPhysicalFunction requires 5 children but got {}",
    ...
    return TrgeometryRestrictTstzspansetPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4));
}
