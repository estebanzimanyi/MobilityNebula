...
#include <Functions/Meos/Rgeo/TrgeometryRestrictTstzsetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTrgeometryRestrictTstzsetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TrgeometryRestrictTstzsetPhysicalFunction::TrgeometryRestrictTstzsetPhysicalFunction(PhysicalFunction xFunction,
...
VarVal TrgeometryRestrictTstzsetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Set* arg0B = tstzset_in(arg0S.c_str());
                if (!arg0B) { free(temp); return (char*) nullptr; }

                Temporal* res = (Temporal*) trgeometry_restrict_tstzset(temp, arg0B, true);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTrgeometryRestrictTstzsetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 5,
                 "TrgeometryRestrictTstzsetPhysicalFunction requires 5 children but got {}",
    ...
    return TrgeometryRestrictTstzsetPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4));
}
