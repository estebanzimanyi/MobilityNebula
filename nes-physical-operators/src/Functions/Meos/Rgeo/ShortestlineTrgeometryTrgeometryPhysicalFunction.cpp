...
#include <Functions/Meos/Rgeo/ShortestlineTrgeometryTrgeometryPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterShortestlineTrgeometryTrgeometryPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

ShortestlineTrgeometryTrgeometryPhysicalFunction::ShortestlineTrgeometryTrgeometryPhysicalFunction(PhysicalFunction xFunction,
...
VarVal ShortestlineTrgeometryTrgeometryPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts2) -> char*
        {
            try
            {
                ...
                if (!arg0tg) { free(temp); return (char*) nullptr; }
                ...
                if (!arg0ttp) { free(arg0tg); free(temp); return (char*) nullptr; }
                Temporal* arg0t = geo_tpose_to_trgeometry(arg0tg, arg0ttp);
                free(arg0tg); free(arg0ttp);
                if (!arg0t) { free(temp); return (char*) nullptr; }

                GSERIALIZED* res = (GSERIALIZED*) shortestline_trgeometry_trgeometry(temp, arg0t);
                free(temp);
                free(arg0t);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterShortestlineTrgeometryTrgeometryPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 8,
                 "ShortestlineTrgeometryTrgeometryPhysicalFunction requires 8 children but got {}",
    ...
    return ShortestlineTrgeometryTrgeometryPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4), std::move(arg5), std::move(arg6), std::move(arg7));
}
