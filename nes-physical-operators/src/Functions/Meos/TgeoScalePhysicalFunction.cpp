...
#include <Functions/Meos/TgeoScalePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTgeoScalePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TgeoScalePhysicalFunction::TgeoScalePhysicalFunction(PhysicalFunction lonFunction,
...
VarVal TgeoScalePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) tgeo_scale(temp, geom_in((char*)"Point(2 2)", -1), geom_in((char*)"Point(0 0)", -1));
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTgeoScalePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TgeoScalePhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TgeoScalePhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
