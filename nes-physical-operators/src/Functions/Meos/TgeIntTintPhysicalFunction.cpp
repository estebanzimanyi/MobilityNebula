...
#include <Functions/Meos/TgeIntTintPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTgeIntTintPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TgeIntTintPhysicalFunction::TgeIntTintPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TgeIntTintPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int32_t arg0) -> char*
        {
            try
            {
                ...
                Temporal* res = tge_int_tint(arg0, temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTgeIntTintPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TgeIntTintPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TgeIntTintPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
