...
#include <Functions/Meos/TgeoStboxesPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTgeoStboxesPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TgeoStboxesPhysicalFunction::TgeoStboxesPhysicalFunction(PhysicalFunction lonFunction,
...
VarVal TgeoStboxesPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                int _cnt = 0;
                STBox* arr = (STBox*) tgeo_stboxes(temp, &_cnt);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTgeoStboxesPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TgeoStboxesPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TgeoStboxesPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
