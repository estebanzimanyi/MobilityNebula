...
AlwaysEqNpointTnpointPhysicalFunction::AlwaysEqNpointTnpointPhysicalFunction(PhysicalFunction rid_np, PhysicalFunction pos_np, PhysicalFunction rid_tp, PhysicalFunction pos_tp, PhysicalFunction ts)
{
    paramFns.reserve(5);
    ...
    const auto result = nautilus::invoke(
        +[](uint64_t rid_np, double pos_np, uint64_t rid_tp, double pos_tp, uint64_t ts) -> double {
            try {
                ...
                int r = always_eq_npoint_tnpoint(np_s, inst);
                free(np_s); free(inst);
                return r > 0 ? 1.0 : 0.0;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 5,
                 "AlwaysEqNpointTnpointPhysicalFunction requires 5 children but got {}",
                 arguments.childFunctions.size());
    return AlwaysEqNpointTnpointPhysicalFunction(
