...
#include <Functions/Meos/TtextValuesPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTtextValuesPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TtextValuesPhysicalFunction::TtextValuesPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TtextValuesPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                int _cnt = 0;
                text ** arr = (text **) ttext_values(temp, &_cnt);
                free(temp);
        ...
        value.getContent(), value.getContentSize(), ts);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTtextValuesPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TtextValuesPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TtextValuesPhysicalFunction(std::move(arg0), std::move(arg1));
}
