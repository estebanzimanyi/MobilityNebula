...
#include <Functions/Meos/TpointFromBaseTempPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTpointFromBaseTempPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TpointFromBaseTempPhysicalFunction::TpointFromBaseTempPhysicalFunction(PhysicalFunction litFunction,
...
VarVal TpointFromBaseTempPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) tpoint_from_base_temp(temp, arg0T);
                free(arg0T);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTpointFromBaseTempPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TpointFromBaseTempPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TpointFromBaseTempPhysicalFunction(std::move(arg0), std::move(arg1));
}
