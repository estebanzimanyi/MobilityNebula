...
#include <Functions/Meos/TstzspansetToDatespansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTstzspansetToDatespansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TstzspansetToDatespansetPhysicalFunction::TstzspansetToDatespansetPhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal TstzspansetToDatespansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                SpanSet* res = tstzspanset_to_datespanset(temp);
                free(temp);
                if (!res) return (char*) nullptr;
                char* outStr = datespanset_out(res);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTstzspansetToDatespansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "TstzspansetToDatespansetPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return TstzspansetToDatespansetPhysicalFunction(std::move(arg0));
}
