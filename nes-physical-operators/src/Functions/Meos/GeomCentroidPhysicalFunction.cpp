...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterGeomCentroidPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

GeomCentroidPhysicalFunction::GeomCentroidPhysicalFunction(PhysicalFunction litFunction)
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                GSERIALIZED* res = geom_centroid(temp);
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return GeomCentroidPhysicalFunction(std::move(arg0));
}
