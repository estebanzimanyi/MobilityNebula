...
#include <Functions/Meos/BeforeSpansetDatePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterBeforeSpansetDatePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

BeforeSpansetDatePhysicalFunction::BeforeSpansetDatePhysicalFunction(PhysicalFunction litFunction,
...
VarVal BeforeSpansetDatePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = before_spanset_date(temp, arg0V);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterBeforeSpansetDatePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "BeforeSpansetDatePhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return BeforeSpansetDatePhysicalFunction(std::move(arg0), std::move(arg1));
}
