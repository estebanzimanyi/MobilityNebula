...
#include <Functions/Meos/Th3indexCellToCenterChildPhysicalFunction.hpp>
...
Th3indexCellToCenterChildPhysicalFunction::Th3indexCellToCenterChildPhysicalFunction(PhysicalFunction cell, PhysicalFunction ts,
...
VarVal Th3indexCellToCenterChildPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto actualLen = nautilus::invoke(
        +[](uint64_t cell, uint64_t ts, uint64_t resolution, char* buf, uint32_t bufMax) -> uint32_t {
            try {
                ...
                if (!inst) return 0u;
                Temporal* res = th3index_cell_to_center_child(inst, (int32_t)resolution);
                free(inst);
                if (!res) return 0u;
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTh3indexCellToCenterChildPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "Th3indexCellToCenterChildPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    return Th3indexCellToCenterChildPhysicalFunction(std::move(arguments.childFunctions[0]),
