...
EverNeTjsonbJsonbPhysicalFunction::EverNeTjsonbJsonbPhysicalFunction(PhysicalFunction json_str, PhysicalFunction ts, PhysicalFunction target_json)
{
    paramFns.reserve(3);
    ...
    const auto result = nautilus::invoke(
        +[](const char* json_str, uint32_t json_len, uint64_t ts, const char* target_json, uint32_t target_len) -> double {
            try {
                ...
                bool r = ever_ne_tjsonb_jsonb((Temporal*)inst, target) > 0;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return EverNeTjsonbJsonbPhysicalFunction(
