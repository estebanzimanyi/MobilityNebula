...
#include <Functions/Meos/MindistanceTcbufferTcbufferPhysicalFunction.hpp>
...
MindistanceTcbufferTcbufferPhysicalFunction::MindistanceTcbufferTcbufferPhysicalFunction(
...
VarVal MindistanceTcbufferTcbufferPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto dist = paramFns[8].execute(record, arena).cast<double>();

    const auto result = nautilus::invoke(
        +[](double lon1, double lat1, double r1, uint64_t ts1,
            double lon2, double lat2, double r2, uint64_t ts2,
            double dist) -> double {
            try {
                ...
                double r = mindistance_tcbuffer_tcbuffer(tcb1, tcb2, dist);
                free(tcb1); free(tcb2);
                return r;
            } catch (const std::exception&) { return -1.0; }
        },
        lon1, lat1, r1, ts1, lon2, lat2, r2, ts2, dist);

    return VarVal(result);
}

PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterMindistanceTcbufferTcbufferPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 9,
                 "MindistanceTcbufferTcbufferPhysicalFunction requires 9 children but got {}",
                 arguments.childFunctions.size());
    return MindistanceTcbufferTcbufferPhysicalFunction(std::move(arguments.childFunctions[0]),
