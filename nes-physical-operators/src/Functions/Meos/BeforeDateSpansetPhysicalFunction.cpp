...
#include <Functions/Meos/BeforeDateSpansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterBeforeDateSpansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

BeforeDateSpansetPhysicalFunction::BeforeDateSpansetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal BeforeDateSpansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = before_date_spanset(arg0V, temp);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterBeforeDateSpansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "BeforeDateSpansetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return BeforeDateSpansetPhysicalFunction(std::move(arg0), std::move(arg1));
}
