...
GeomPointMake3dzPhysicalFunction::GeomPointMake3dzPhysicalFunction(PhysicalFunction srid, PhysicalFunction x, PhysicalFunction y, PhysicalFunction z)
{
    paramFns.reserve(4);
    ...
    const auto actualLen = nautilus::invoke(
        +[](uint64_t srid, double x, double y, double z, char* buf, uint32_t bufMax) -> uint32_t {
            try {
                ...
                GSERIALIZED* result = geompoint_make3dz((int32_t)srid, x, y, z);
                if (!result) return 0u;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return GeomPointMake3dzPhysicalFunction(
