...
#include <Functions/Meos/UnionSpanSpanPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterUnionSpanSpanPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

UnionSpanSpanPhysicalFunction::UnionSpanSpanPhysicalFunction(PhysicalFunction litFunction,
...
VarVal UnionSpanSpanPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                SpanSet* res = union_span_span(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterUnionSpanSpanPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "UnionSpanSpanPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return UnionSpanSpanPhysicalFunction(std::move(arg0), std::move(arg1));
}
