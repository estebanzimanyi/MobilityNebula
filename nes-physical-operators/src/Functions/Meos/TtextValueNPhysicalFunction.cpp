...
#include <Functions/Meos/TtextValueNPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTtextValueNPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TtextValueNPhysicalFunction::TtextValueNPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TtextValueNPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                text* res = nullptr;
                bool ok = ttext_value_n(temp, 1, &res);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTtextValueNPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TtextValueNPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TtextValueNPhysicalFunction(std::move(arg0), std::move(arg1));
}
