...
NadTpcpointGeoPhysicalFunction::NadTpcpointGeoPhysicalFunction(PhysicalFunction pt_hexwkb, PhysicalFunction ts, PhysicalFunction tgt_wkt)
{
    paramFns.reserve(3);
    ...
    const auto result = nautilus::invoke(
        +[](const char* pt_hexwkb, uint32_t pt_len, uint64_t ts, const char* tgt_wkt, uint32_t tgt_len) -> double {
            try {
                ...
                double r = nad_tpcpoint_geo((Temporal*)inst, gs);
                free(inst); free(gs);
                return r;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return NadTpcpointGeoPhysicalFunction(
