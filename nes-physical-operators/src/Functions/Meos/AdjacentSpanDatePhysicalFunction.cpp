...
#include <Functions/Meos/AdjacentSpanDatePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterAdjacentSpanDatePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

AdjacentSpanDatePhysicalFunction::AdjacentSpanDatePhysicalFunction(PhysicalFunction litFunction,
...
VarVal AdjacentSpanDatePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = adjacent_span_date(temp, arg0V);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterAdjacentSpanDatePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "AdjacentSpanDatePhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return AdjacentSpanDatePhysicalFunction(std::move(arg0), std::move(arg1));
}
