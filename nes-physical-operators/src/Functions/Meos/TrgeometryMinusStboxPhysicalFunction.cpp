...
#include <Functions/Meos/TrgeometryMinusStboxPhysicalFunction.hpp>
...
TrgeometryMinusStboxPhysicalFunction::TrgeometryMinusStboxPhysicalFunction(PhysicalFunction trajFunction,
...
VarVal TrgeometryMinusStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto hexStr = nautilus::invoke(
        +[](const char* aPtr, uint32_t aSize, const char* bPtr, uint32_t bSize, bool borderInc) -> char*
        {
            try
            {
                ...
                Temporal* res = trgeometry_minus_stbox(a, box, borderInc);
                free(a);
                free(box);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTrgeometryMinusStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TrgeometryMinusStboxPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TrgeometryMinusStboxPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
