...
#include <Functions/Meos/LeftTboxTboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterLeftTboxTboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

LeftTboxTboxPhysicalFunction::LeftTboxTboxPhysicalFunction(PhysicalFunction litFunction,
...
VarVal LeftTboxTboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = left_tbox_tbox(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterLeftTboxTboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "LeftTboxTboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return LeftTboxTboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
