...
GeomIntersection2dCollPhysicalFunction::GeomIntersection2dCollPhysicalFunction(PhysicalFunction wktFunction,
    ...
    const auto actualLen = nautilus::invoke(
        +[](const char* wktPtr, uint32_t wktSize,
            ...
            try
            {
                ...
                GSERIALIZED* gres = geom_intersection2d_coll(temp, gs0);
                free(temp);
                free(gs0);
                if (!gres) return 0u;
