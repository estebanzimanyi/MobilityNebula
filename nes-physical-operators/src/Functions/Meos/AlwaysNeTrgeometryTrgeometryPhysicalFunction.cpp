...
#include <Functions/Meos/AlwaysNeTrgeometryTrgeometryPhysicalFunction.hpp>
...
AlwaysNeTrgeometryTrgeometryPhysicalFunction::AlwaysNeTrgeometryTrgeometryPhysicalFunction(PhysicalFunction ref1_wkt, PhysicalFunction x1, PhysicalFunction y1, PhysicalFunction theta1, PhysicalFunction ts1, PhysicalFunction ref2_wkt, PhysicalFunction x2, PhysicalFunction y2, PhysicalFunction theta2, PhysicalFunction ts2)
{
    paramFns.reserve(10);
    ...
    const auto result = nautilus::invoke(
        +[](const char* ref1_wkt, uint32_t ref1_len, double x1, double y1, double theta1, uint64_t ts1, const char* ref2_wkt, uint32_t ref2_len, double x2, double y2, double theta2, uint64_t ts2) -> double {
            try {
                ...
                int r = always_ne_trgeometry_trgeometry((Temporal*)inst1, (Temporal*)inst2);
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 10,
                 "AlwaysNeTrgeometryTrgeometryPhysicalFunction requires 10 children but got {}",
                 arguments.childFunctions.size());
    return AlwaysNeTrgeometryTrgeometryPhysicalFunction(
