...
#include <Functions/Meos/AfterDateSpanPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterAfterDateSpanPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

AfterDateSpanPhysicalFunction::AfterDateSpanPhysicalFunction(PhysicalFunction litFunction,
...
VarVal AfterDateSpanPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = after_date_span(arg0V, temp);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterAfterDateSpanPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "AfterDateSpanPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return AfterDateSpanPhysicalFunction(std::move(arg0), std::move(arg1));
}
