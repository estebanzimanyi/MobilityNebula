...
#include <Functions/Meos/AlwaysLtTintIntPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterAlwaysLtTintIntPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

AlwaysLtTintIntPhysicalFunction::AlwaysLtTintIntPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal AlwaysLtTintIntPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](int valueValue,
            ...
            try
            {
                ...
                if (!temp) return 0;
                int r = always_lt_tint_int(temp, scalarValue);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterAlwaysLtTintIntPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "AlwaysLtTintIntPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return AlwaysLtTintIntPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
