...
#include <Functions/Meos/RightSpanSpansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterRightSpanSpansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

RightSpanSpansetPhysicalFunction::RightSpanSpansetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal RightSpanSpansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = right_span_spanset(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterRightSpanSpansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "RightSpanSpansetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return RightSpanSpansetPhysicalFunction(std::move(arg0), std::move(arg1));
}
