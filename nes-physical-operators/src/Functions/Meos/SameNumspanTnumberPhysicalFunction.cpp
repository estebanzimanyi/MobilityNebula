...
#include <Functions/Meos/SameNumspanTnumberPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterSameNumspanTnumberPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

SameNumspanTnumberPhysicalFunction::SameNumspanTnumberPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal SameNumspanTnumberPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](double value,
            ...
            try
            {
                ...
                int r = same_numspan_tnumber(arg0B, temp);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterSameNumspanTnumberPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "SameNumspanTnumberPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return SameNumspanTnumberPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
