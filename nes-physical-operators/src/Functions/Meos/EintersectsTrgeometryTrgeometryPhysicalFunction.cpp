...
#include <Functions/Meos/EintersectsTrgeometryTrgeometryPhysicalFunction.hpp>
...
VarVal EintersectsTrgeometryTrgeometryPhysicalFunction::execute(const Record& record, ArenaRef& arena) const {
    ...
    const auto result = nautilus::invoke(
        +[](const char* ref1_wkt, uint32_t ref1_wktsz, double x1, double y1, double theta1, uint64_t ts1, const char* ref2_wkt, uint32_t ref2_wktsz, double x2, double y2, double theta2, uint64_t ts2) -> double {
            try {
                ...
                int r = eintersects_trgeometry_trgeometry((Temporal*)inst1, (Temporal*)inst2);
                free(inst1); free(inst2);
                return r > 0 ? 1.0 : 0.0;
            } catch (const std::exception&) { return 0.0; }
        },
        ref1_wkt.getContent(), ref1_wkt.getContentSize(), x1, y1, theta1, ts1, ref2_wkt.getContent(), ref2_wkt.getContentSize(), x2, y2, theta2, ts2);
    return VarVal(result);
}

PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterEintersectsTrgeometryTrgeometryPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size()==10,
                 "EintersectsTrgeometryTrgeometryPhysicalFunction requires 10 children but got {}",
                 arguments.childFunctions.size());
    return EintersectsTrgeometryTrgeometryPhysicalFunction(
