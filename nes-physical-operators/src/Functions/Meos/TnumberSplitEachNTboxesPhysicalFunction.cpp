...
#include <Functions/Meos/TnumberSplitEachNTboxesPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTnumberSplitEachNTboxesPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TnumberSplitEachNTboxesPhysicalFunction::TnumberSplitEachNTboxesPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TnumberSplitEachNTboxesPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int32_t arg0) -> char*
        {
            try
            {
                ...
                int _cnt = 0;
                TBox* arr = (TBox*) tnumber_split_each_n_tboxes(temp, arg0, &_cnt);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTnumberSplitEachNTboxesPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TnumberSplitEachNTboxesPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TnumberSplitEachNTboxesPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
