...
#include <Functions/Meos/FloatspanShiftScalePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterFloatspanShiftScalePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

FloatspanShiftScalePhysicalFunction::FloatspanShiftScalePhysicalFunction(PhysicalFunction litFunction,
...
VarVal FloatspanShiftScalePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            double arg1) -> char*
        {
            try
            {
                ...
                Span* res = floatspan_shift_scale(temp, arg0, arg1, true, true);
                free(temp);
                if (!res) return (char*) nullptr;
                char* outStr = floatspan_out(res, 15);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterFloatspanShiftScalePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "FloatspanShiftScalePhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return FloatspanShiftScalePhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
