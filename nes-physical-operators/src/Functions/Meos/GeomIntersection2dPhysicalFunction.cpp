...
GeomIntersection2dPhysicalFunction::GeomIntersection2dPhysicalFunction(PhysicalFunction wkt1, PhysicalFunction wkt2)
{
    paramFns.reserve(2);
    ...
    const auto actualLen = nautilus::invoke(
        +[](const char* w1, uint32_t w1sz, const char* w2, uint32_t w2sz, char* buf, uint32_t bufMax) -> uint32_t {
            try {
                ...
                GSERIALIZED* result = geom_intersection2d(gs1, gs2);
                free(gs1); free(gs2);
                if (!result) return 0u;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return GeomIntersection2dPhysicalFunction(
