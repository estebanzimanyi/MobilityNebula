...
#include <Functions/Meos/Box3dToStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterBox3dToStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

Box3dToStboxPhysicalFunction::Box3dToStboxPhysicalFunction(PhysicalFunction boxFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(boxFunction));
}

VarVal Box3dToStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* boxPtr, uint32_t boxSize) -> char*
        {
            try
            {
                ...
                BOX3D* temp = box3d_in(tempS.c_str());
                if (!temp) return (char*) nullptr;

                STBox* res = (STBox*) box3d_to_stbox(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterBox3dToStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "Box3dToStboxPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return Box3dToStboxPhysicalFunction(std::move(arg0));
}
