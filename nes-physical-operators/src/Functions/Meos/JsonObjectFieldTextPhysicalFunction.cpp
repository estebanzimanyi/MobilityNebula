...
JsonObjectFieldTextPhysicalFunction::JsonObjectFieldTextPhysicalFunction(PhysicalFunction js, PhysicalFunction key)
{
    paramFns.reserve(2);
...
VarVal JsonObjectFieldTextPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto actualLen = nautilus::invoke(
        +[](const char* wj, uint32_t wjsz,
            const char* wk, uint32_t wksz,
            char* buf, uint32_t bufMax) -> uint32_t {
            try {
                MEOS::Meos::ensureMeosInitialized();
                std::string sj(wj, wjsz), sk(wk, wksz);
                text* js  = json_in(sj.c_str());
                if (!js) return 0u;
                text* key = cstring_to_text(sk.c_str());
                text* res = json_object_field_text(js, key);
                free(js); free(key);
                if (!res) return 0u;
        ...
        js, key, outBuf.getContent(), nautilus::val<uint32_t>(MAX_LEN));
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return JsonObjectFieldTextPhysicalFunction(
