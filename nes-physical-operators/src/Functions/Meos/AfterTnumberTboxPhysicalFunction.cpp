...
#include <Functions/Meos/AfterTnumberTboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterAfterTnumberTboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

AfterTnumberTboxPhysicalFunction::AfterTnumberTboxPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal AfterTnumberTboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](double value,
            ...
            try
            {
                ...
                bool r = after_tnumber_tbox(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterAfterTnumberTboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "AfterTnumberTboxPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return AfterTnumberTboxPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
