...
Th3indexCellToCenterChildNextPhysicalFunction::Th3indexCellToCenterChildNextPhysicalFunction(PhysicalFunction cell, PhysicalFunction ts)
{
    paramFns.reserve(2);
    ...
    const auto actualLen = nautilus::invoke(
        +[](uint64_t cell, uint64_t ts, char* buf, uint32_t bufMax) -> uint32_t {
            try {
                ...
                if (!inst) return 0u;
                Temporal* res = th3index_cell_to_center_child_next(inst);
                free(inst);
                if (!res) return 0u;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "Th3indexCellToCenterChildNextPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    return Th3indexCellToCenterChildNextPhysicalFunction(std::move(arguments.childFunctions[0]),
