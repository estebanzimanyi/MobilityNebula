...
#include <Functions/Meos/AfterStboxStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterAfterStboxStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

AfterStboxStboxPhysicalFunction::AfterStboxStboxPhysicalFunction(PhysicalFunction boxFunction,
...
VarVal AfterStboxStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* boxPtr, uint32_t boxSize,
            ...
            try
            {
                ...
                bool r = after_stbox_stbox(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterAfterStboxStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "AfterStboxStboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return AfterStboxStboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
