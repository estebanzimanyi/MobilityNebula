...
#include <Functions/Meos/IntsetToFloatsetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterIntsetToFloatsetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

IntsetToFloatsetPhysicalFunction::IntsetToFloatsetPhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal IntsetToFloatsetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                Set* res = (Set*) intset_to_floatset(temp);
                free(temp);
                if (!res) return (char*) nullptr;
                char* outStr = floatset_out(res, 15);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterIntsetToFloatsetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "IntsetToFloatsetPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return IntsetToFloatsetPhysicalFunction(std::move(arg0));
}
