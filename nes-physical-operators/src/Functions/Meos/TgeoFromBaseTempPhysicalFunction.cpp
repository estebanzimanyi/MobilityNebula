...
#include <Functions/Meos/TgeoFromBaseTempPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTgeoFromBaseTempPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TgeoFromBaseTempPhysicalFunction::TgeoFromBaseTempPhysicalFunction(PhysicalFunction litFunction,
...
VarVal TgeoFromBaseTempPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) tgeo_from_base_temp(temp, arg0T);
                free(arg0T);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTgeoFromBaseTempPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TgeoFromBaseTempPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TgeoFromBaseTempPhysicalFunction(std::move(arg0), std::move(arg1));
}
