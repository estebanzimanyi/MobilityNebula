...
#include <Functions/Meos/TspatialTransformPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTspatialTransformPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TspatialTransformPhysicalFunction::TspatialTransformPhysicalFunction(PhysicalFunction lonFunction,
...
VarVal TspatialTransformPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) tspatial_transform(temp, 3857);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTspatialTransformPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TspatialTransformPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TspatialTransformPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
