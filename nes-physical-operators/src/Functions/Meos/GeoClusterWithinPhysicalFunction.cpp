...
#include <Functions/Meos/GeoClusterWithinPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterGeoClusterWithinPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

GeoClusterWithinPhysicalFunction::GeoClusterWithinPhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal GeoClusterWithinPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                int _cnt = 0;
                GSERIALIZED ** arr = (GSERIALIZED **) geo_cluster_within((const GSERIALIZED**) _ina, 1, 5.0, &_cnt);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterGeoClusterWithinPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "GeoClusterWithinPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return GeoClusterWithinPhysicalFunction(std::move(arg0));
}
