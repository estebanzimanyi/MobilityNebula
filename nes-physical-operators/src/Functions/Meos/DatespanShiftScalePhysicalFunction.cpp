...
#include <Functions/Meos/DatespanShiftScalePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterDatespanShiftScalePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

DatespanShiftScalePhysicalFunction::DatespanShiftScalePhysicalFunction(PhysicalFunction litFunction,
...
VarVal DatespanShiftScalePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int32_t arg1) -> char*
        {
            try
            {
                ...
                Span* res = datespan_shift_scale(temp, arg0, arg1, true, true);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterDatespanShiftScalePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "DatespanShiftScalePhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return DatespanShiftScalePhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
