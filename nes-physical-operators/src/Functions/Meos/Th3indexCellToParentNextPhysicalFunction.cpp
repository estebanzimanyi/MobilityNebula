...
Th3indexCellToParentNextPhysicalFunction::Th3indexCellToParentNextPhysicalFunction(PhysicalFunction cellFunction,
    ...
    const auto actualLen = nautilus::invoke(
        +[](uint64_t cell,
            ...
            try
            {
                ...
                Temporal* tres = th3index_cell_to_parent_next(temp, (int32_t)arg0);
                free(temp);
                if (!tres) return 0u;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "Th3indexCellToParentNextPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return Th3indexCellToParentNextPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
