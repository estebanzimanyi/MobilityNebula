...
#include <Functions/Meos/Npoint/NpointToNsegmentPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterNpointToNsegmentPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

NpointToNsegmentPhysicalFunction::NpointToNsegmentPhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal NpointToNsegmentPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                Nsegment* res = (Nsegment*) npoint_to_nsegment(temp);
                free(temp);
                if (!res) return (char*) nullptr;
                char* outStr = nsegment_out(res, 15);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterNpointToNsegmentPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "NpointToNsegmentPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return NpointToNsegmentPhysicalFunction(std::move(arg0));
}
