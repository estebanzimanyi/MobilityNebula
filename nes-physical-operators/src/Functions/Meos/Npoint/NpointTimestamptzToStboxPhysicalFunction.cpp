...
#include <Functions/Meos/Npoint/NpointTimestamptzToStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterNpointTimestamptzToStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

NpointTimestamptzToStboxPhysicalFunction::NpointTimestamptzToStboxPhysicalFunction(PhysicalFunction litFunction,
...
VarVal NpointTimestamptzToStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Npoint* temp = npoint_in(tempS.c_str());
                ...
                STBox* res = (STBox*) npoint_timestamptz_to_stbox(temp, arg0V);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterNpointTimestamptzToStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "NpointTimestamptzToStboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return NpointTimestamptzToStboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
