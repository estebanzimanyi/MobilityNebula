...
#include <Functions/Meos/Npoint/NpointsetStartValuePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterNpointsetStartValuePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

NpointsetStartValuePhysicalFunction::NpointsetStartValuePhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal NpointsetStartValuePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                Npoint* res = npointset_start_value(temp);
                free(temp);
                if (!res) return (char*) nullptr;
                char* outStr = npoint_out(res, 15);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterNpointsetStartValuePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "NpointsetStartValuePhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return NpointsetStartValuePhysicalFunction(std::move(arg0));
}
