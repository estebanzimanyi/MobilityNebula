...
#include <Functions/Meos/Npoint/TnpointMinusNpointsetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTnpointMinusNpointsetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TnpointMinusNpointsetPhysicalFunction::TnpointMinusNpointsetPhysicalFunction(PhysicalFunction ridFunction,
...
VarVal TnpointMinusNpointsetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Set* arg0B = npointset_in(arg0S.c_str());
                if (!arg0B) { free(temp); return (char*) nullptr; }

                Temporal* res = (Temporal*) tnpoint_minus_npointset(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTnpointMinusNpointsetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 4,
                 "TnpointMinusNpointsetPhysicalFunction requires 4 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    auto arg3 = std::move(arguments.childFunctions[3]);
    return TnpointMinusNpointsetPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3));
}
