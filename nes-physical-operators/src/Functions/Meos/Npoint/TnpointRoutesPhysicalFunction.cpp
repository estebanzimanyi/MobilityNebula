...
#include <Functions/Meos/Npoint/TnpointRoutesPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTnpointRoutesPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TnpointRoutesPhysicalFunction::TnpointRoutesPhysicalFunction(PhysicalFunction ridFunction,
...
VarVal TnpointRoutesPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                Set* res = (Set*) tnpoint_routes(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTnpointRoutesPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TnpointRoutesPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TnpointRoutesPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
