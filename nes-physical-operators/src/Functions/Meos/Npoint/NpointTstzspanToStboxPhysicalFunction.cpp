...
#include <Functions/Meos/Npoint/NpointTstzspanToStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterNpointTstzspanToStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

NpointTstzspanToStboxPhysicalFunction::NpointTstzspanToStboxPhysicalFunction(PhysicalFunction litFunction,
...
VarVal NpointTstzspanToStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                STBox* res = (STBox*) npoint_tstzspan_to_stbox(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterNpointTstzspanToStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "NpointTstzspanToStboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return NpointTstzspanToStboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
