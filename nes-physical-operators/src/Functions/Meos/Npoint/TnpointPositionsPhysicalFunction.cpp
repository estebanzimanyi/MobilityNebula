...
#include <Functions/Meos/Npoint/TnpointPositionsPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTnpointPositionsPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TnpointPositionsPhysicalFunction::TnpointPositionsPhysicalFunction(PhysicalFunction ridFunction,
...
VarVal TnpointPositionsPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                int _cnt = 0;
                Nsegment ** arr = (Nsegment **) tnpoint_positions(temp, &_cnt);
                free(temp);
                if (!arr || _cnt <= 0) return (char*) nullptr;
                std::string _s = "{";
                for (int _i = 0; _i < _cnt; _i++) { if (_i) _s += ", "; char* _e = nsegment_out((Nsegment *) arr[_i], 15); if (_e) { _s += _e; free(_e); } free(arr[_i]); }
                _s += "}";
                free(arr);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTnpointPositionsPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TnpointPositionsPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TnpointPositionsPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
