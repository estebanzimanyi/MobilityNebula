...
#include <Functions/Meos/Npoint/IntersectionSetNpointPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterIntersectionSetNpointPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

IntersectionSetNpointPhysicalFunction::IntersectionSetNpointPhysicalFunction(PhysicalFunction litFunction,
...
VarVal IntersectionSetNpointPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Set* res = intersection_set_npoint(temp, arg0B);
                free(temp);
                free(arg0B);
                if (!res) return (char*) nullptr;
                char* outStr = npointset_out(res, 15);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterIntersectionSetNpointPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "IntersectionSetNpointPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return IntersectionSetNpointPhysicalFunction(std::move(arg0), std::move(arg1));
}
