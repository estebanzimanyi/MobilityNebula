...
#include <Functions/Meos/Npoint/NadTnpointTnpointPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterNadTnpointTnpointPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

NadTnpointTnpointPhysicalFunction::NadTnpointTnpointPhysicalFunction(PhysicalFunction trajFunction,
    ...
    const auto result = nautilus::invoke(
        +[](const char* trajPtr, uint32_t trajSize,
            ...
            try
            {
                ...
                double r = nad_tnpoint_tnpoint(temp, arg0T);
                free(temp);
                free(arg0T);
                return r;
            }
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "NadTnpointTnpointPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return NadTnpointTnpointPhysicalFunction(std::move(arg0), std::move(arg1));
}
