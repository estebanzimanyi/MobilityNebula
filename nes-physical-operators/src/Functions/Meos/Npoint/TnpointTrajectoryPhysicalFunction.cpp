...
#include <Functions/Meos/Npoint/TnpointTrajectoryPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTnpointTrajectoryPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TnpointTrajectoryPhysicalFunction::TnpointTrajectoryPhysicalFunction(PhysicalFunction ridFunction,
...
VarVal TnpointTrajectoryPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                GSERIALIZED* res = (GSERIALIZED*) tnpoint_trajectory(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTnpointTrajectoryPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TnpointTrajectoryPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TnpointTrajectoryPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
