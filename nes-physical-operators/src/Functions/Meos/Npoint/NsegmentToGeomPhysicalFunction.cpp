...
#include <Functions/Meos/Npoint/NsegmentToGeomPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterNsegmentToGeomPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

NsegmentToGeomPhysicalFunction::NsegmentToGeomPhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal NsegmentToGeomPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                GSERIALIZED* res = (GSERIALIZED*) nsegment_to_geom(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterNsegmentToGeomPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "NsegmentToGeomPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return NsegmentToGeomPhysicalFunction(std::move(arg0));
}
