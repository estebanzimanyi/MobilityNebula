...
#include <Functions/Meos/TstzspansetTimestampsPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTstzspansetTimestampsPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TstzspansetTimestampsPhysicalFunction::TstzspansetTimestampsPhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal TstzspansetTimestampsPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                Set* res = (Set*) tstzspanset_timestamps(temp);
                free(temp);
                if (!res) return (char*) nullptr;
                char* outStr = tstzset_out(res);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTstzspansetTimestampsPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "TstzspansetTimestampsPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return TstzspansetTimestampsPhysicalFunction(std::move(arg0));
}
