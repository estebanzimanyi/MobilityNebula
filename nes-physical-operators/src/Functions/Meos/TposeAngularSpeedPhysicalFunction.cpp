...
#include <Functions/Meos/TposeAngularSpeedPhysicalFunction.hpp>
...
TposeAngularSpeedPhysicalFunction::TposeAngularSpeedPhysicalFunction(PhysicalFunction trajFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(trajFunction));
}

VarVal TposeAngularSpeedPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto hexStr = nautilus::invoke(
        +[](const char* aPtr, uint32_t aSize) -> char*
        {
            try
            {
                ...
                Temporal* res = tpose_angular_speed(a);
                free(a);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTposeAngularSpeedPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "TposeAngularSpeedPhysicalFunction requires 1 child but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return TposeAngularSpeedPhysicalFunction(std::move(arg0));
}
