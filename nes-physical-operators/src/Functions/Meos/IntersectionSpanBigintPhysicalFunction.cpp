...
#include <Functions/Meos/IntersectionSpanBigintPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterIntersectionSpanBigintPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

IntersectionSpanBigintPhysicalFunction::IntersectionSpanBigintPhysicalFunction(PhysicalFunction litFunction,
...
VarVal IntersectionSpanBigintPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int64_t arg0) -> char*
        {
            try
            {
                ...
                Span* res = intersection_span_bigint(temp, arg0);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterIntersectionSpanBigintPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "IntersectionSpanBigintPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return IntersectionSpanBigintPhysicalFunction(std::move(arg0), std::move(arg1));
}
