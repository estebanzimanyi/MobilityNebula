...
#include <Functions/Meos/TextCmpPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTextCmpPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TextCmpPhysicalFunction::TextCmpPhysicalFunction(PhysicalFunction litFunction,
...
VarVal TextCmpPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                text* arg0B = text_in(arg0S.c_str());
                if (!arg0B) { free(temp); return 0; }

                int r = text_cmp(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTextCmpPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TextCmpPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TextCmpPhysicalFunction(std::move(arg0), std::move(arg1));
}
