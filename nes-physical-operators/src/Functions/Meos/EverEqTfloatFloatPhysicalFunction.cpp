...
#include <Functions/Meos/EverEqTfloatFloatPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterEverEqTfloatFloatPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

EverEqTfloatFloatPhysicalFunction::EverEqTfloatFloatPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal EverEqTfloatFloatPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](double valueValue,
            ...
            try
            {
                ...
                if (!temp) return 0;
                int r = ever_eq_tfloat_float(temp, scalarValue);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterEverEqTfloatFloatPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "EverEqTfloatFloatPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return EverEqTfloatFloatPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
