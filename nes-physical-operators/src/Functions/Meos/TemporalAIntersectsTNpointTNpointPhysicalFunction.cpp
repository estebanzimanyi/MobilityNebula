...
#include <Functions/Meos/TemporalAIntersectsTNpointTNpointPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTemporalAIntersectsTNpointTNpointPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TemporalAIntersectsTNpointTNpointPhysicalFunction::TemporalAIntersectsTNpointTNpointPhysicalFunction(PhysicalFunction ridAFunction,
...
VarVal TemporalAIntersectsTNpointTNpointPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](uint64_t ridAValue, double fractionAValue, uint64_t tsAValue,
            uint64_t ridBValue, double fractionBValue, uint64_t tsBValue) -> int {
            try
            {
                ...
                int r = aintersects_tgeo_tgeo(tgeoA, tgeoB);
                free(tgeoB);
                free(tnpointB);
                free(tgeoA);
                free(tnpointA);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTemporalAIntersectsTNpointTNpointPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 6,
                 "TemporalAIntersectsTNpointTNpointPhysicalFunction requires 6 children but got {}",
    ...
    return TemporalAIntersectsTNpointTNpointPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4), std::move(arg5));
}
