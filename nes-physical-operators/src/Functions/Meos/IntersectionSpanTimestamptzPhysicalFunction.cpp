...
#include <Functions/Meos/IntersectionSpanTimestamptzPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterIntersectionSpanTimestamptzPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

IntersectionSpanTimestamptzPhysicalFunction::IntersectionSpanTimestamptzPhysicalFunction(PhysicalFunction litFunction,
...
VarVal IntersectionSpanTimestamptzPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int64_t arg0) -> char*
        {
            try
            {
                ...
                Span* temp = tstzspan_in(tempS.c_str());
                if (!temp) return (char*) nullptr;

                Span* res = intersection_span_timestamptz(temp, arg0);
                free(temp);
                if (!res) return (char*) nullptr;
                char* outStr = tstzspan_out(res);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterIntersectionSpanTimestamptzPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "IntersectionSpanTimestamptzPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return IntersectionSpanTimestamptzPhysicalFunction(std::move(arg0), std::move(arg1));
}
