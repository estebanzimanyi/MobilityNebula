...
JsonTypeofPhysicalFunction::JsonTypeofPhysicalFunction(PhysicalFunction jsonFunction)
{
    ...
    const auto actualLen = nautilus::invoke(
        +[](const char* jsonPtr, uint32_t jsonSize,
            ...
            try
            {
                ...
                text* tres = ({std::string _s(jsonPtr,jsonSize);text* _js=json_in(_s.c_str());if(!_js)return 0u;text* _res=json_typeof(_js);free(_js);_res;});
                if (!tres) return 0u;
        ...
        json.getContent(), json.getContentSize(), outBuf.getContent(), nautilus::val<uint32_t>(MAX_LEN));
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return JsonTypeofPhysicalFunction(std::move(arg0));
}
