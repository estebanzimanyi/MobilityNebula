...
#include <Functions/Meos/ContainsSetGeoPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterContainsSetGeoPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

ContainsSetGeoPhysicalFunction::ContainsSetGeoPhysicalFunction(PhysicalFunction litFunction,
...
VarVal ContainsSetGeoPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = contains_set_geo(temp, arg0G.getGeometry());
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterContainsSetGeoPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "ContainsSetGeoPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return ContainsSetGeoPhysicalFunction(std::move(arg0), std::move(arg1));
}
