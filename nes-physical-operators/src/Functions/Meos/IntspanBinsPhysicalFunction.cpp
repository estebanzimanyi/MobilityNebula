...
#include <Functions/Meos/IntspanBinsPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterIntspanBinsPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

IntspanBinsPhysicalFunction::IntspanBinsPhysicalFunction(PhysicalFunction litFunction,
...
VarVal IntspanBinsPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int32_t arg1) -> char*
        {
            try
            {
                ...
                int _cnt = 0;
                Span* arr = (Span*) intspan_bins(temp, arg0, arg1, &_cnt);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterIntspanBinsPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "IntspanBinsPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return IntspanBinsPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
