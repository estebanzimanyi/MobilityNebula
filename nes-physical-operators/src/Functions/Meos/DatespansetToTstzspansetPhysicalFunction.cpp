...
#include <Functions/Meos/DatespansetToTstzspansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterDatespansetToTstzspansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

DatespansetToTstzspansetPhysicalFunction::DatespansetToTstzspansetPhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal DatespansetToTstzspansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                SpanSet* res = datespanset_to_tstzspanset(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterDatespansetToTstzspansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "DatespansetToTstzspansetPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return DatespansetToTstzspansetPhysicalFunction(std::move(arg0));
}
