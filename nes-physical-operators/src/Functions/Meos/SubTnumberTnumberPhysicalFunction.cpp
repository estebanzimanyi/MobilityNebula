...
#include <Functions/Meos/SubTnumberTnumberPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterSubTnumberTnumberPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

SubTnumberTnumberPhysicalFunction::SubTnumberTnumberPhysicalFunction(PhysicalFunction trajFunction,
...
VarVal SubTnumberTnumberPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto hexStr = nautilus::invoke(
        +[](const char* aPtr, uint32_t aSize, const char* bPtr, uint32_t bSize) -> char*
        {
            try
            {
                ...
                Temporal* res = sub_tnumber_tnumber(a, b);
                free(a);
                free(b);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterSubTnumberTnumberPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "SubTnumberTnumberPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return SubTnumberTnumberPhysicalFunction(std::move(arg0), std::move(arg1));
}
