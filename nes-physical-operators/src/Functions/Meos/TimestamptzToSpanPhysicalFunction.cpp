...
#include <Functions/Meos/TimestamptzToSpanPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTimestamptzToSpanPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TimestamptzToSpanPhysicalFunction::TimestamptzToSpanPhysicalFunction(PhysicalFunction valueFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(valueFunction));
}

VarVal TimestamptzToSpanPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](int64_t value) -> char*
        {
            try
            {
                ...
                Span* res = timestamptz_to_span(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTimestamptzToSpanPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "TimestamptzToSpanPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return TimestamptzToSpanPhysicalFunction(std::move(arg0));
}
