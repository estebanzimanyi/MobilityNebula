...
#include <Functions/Meos/OverafterDateSpanPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterOverafterDateSpanPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

OverafterDateSpanPhysicalFunction::OverafterDateSpanPhysicalFunction(PhysicalFunction litFunction,
...
VarVal OverafterDateSpanPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = overafter_date_span(arg0V, temp);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterOverafterDateSpanPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "OverafterDateSpanPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return OverafterDateSpanPhysicalFunction(std::move(arg0), std::move(arg1));
}
