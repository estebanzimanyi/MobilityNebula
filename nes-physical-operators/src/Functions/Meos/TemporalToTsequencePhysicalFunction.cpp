...
#include <Functions/Meos/TemporalToTsequencePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTemporalToTsequencePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TemporalToTsequencePhysicalFunction::TemporalToTsequencePhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TemporalToTsequencePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) temporal_to_tsequence(temp, (interpType) 2);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTemporalToTsequencePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TemporalToTsequencePhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TemporalToTsequencePhysicalFunction(std::move(arg0), std::move(arg1));
}
