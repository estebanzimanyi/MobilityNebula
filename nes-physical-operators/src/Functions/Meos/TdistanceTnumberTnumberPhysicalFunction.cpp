...
#include <Functions/Meos/TdistanceTnumberTnumberPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTdistanceTnumberTnumberPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TdistanceTnumberTnumberPhysicalFunction::TdistanceTnumberTnumberPhysicalFunction(PhysicalFunction trajFunction,
...
VarVal TdistanceTnumberTnumberPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto hexStr = nautilus::invoke(
        +[](const char* aPtr, uint32_t aSize, const char* bPtr, uint32_t bSize) -> char*
        {
            try
            {
                ...
                Temporal* res = tdistance_tnumber_tnumber(a, b);
                free(a);
                free(b);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTdistanceTnumberTnumberPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TdistanceTnumberTnumberPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TdistanceTnumberTnumberPhysicalFunction(std::move(arg0), std::move(arg1));
}
