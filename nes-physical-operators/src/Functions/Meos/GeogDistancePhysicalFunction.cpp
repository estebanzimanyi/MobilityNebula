...
#include <Functions/Meos/GeogDistancePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterGeogDistancePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

GeogDistancePhysicalFunction::GeogDistancePhysicalFunction(PhysicalFunction litFunction,
...
VarVal GeogDistancePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                double r = geog_distance(temp, arg0G.getGeometry());
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterGeogDistancePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "GeogDistancePhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return GeogDistancePhysicalFunction(std::move(arg0), std::move(arg1));
}
