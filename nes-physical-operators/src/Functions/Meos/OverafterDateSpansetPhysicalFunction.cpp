...
#include <Functions/Meos/OverafterDateSpansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterOverafterDateSpansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

OverafterDateSpansetPhysicalFunction::OverafterDateSpansetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal OverafterDateSpansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = overafter_date_spanset(arg0V, temp);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterOverafterDateSpansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "OverafterDateSpansetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return OverafterDateSpansetPhysicalFunction(std::move(arg0), std::move(arg1));
}
