...
#include <Functions/Meos/Pose/TposeAtGeomPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTposeAtGeomPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TposeAtGeomPhysicalFunction::TposeAtGeomPhysicalFunction(PhysicalFunction xFunction,
...
VarVal TposeAtGeomPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) tpose_at_geom(temp, arg0G.getGeometry());
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTposeAtGeomPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 5,
                 "TposeAtGeomPhysicalFunction requires 5 children but got {}",
    ...
    return TposeAtGeomPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4));
}
