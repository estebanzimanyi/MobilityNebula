...
#include <Functions/Meos/Pose/DistancePoseStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterDistancePoseStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

DistancePoseStboxPhysicalFunction::DistancePoseStboxPhysicalFunction(PhysicalFunction litFunction,
...
VarVal DistancePoseStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                double r = distance_pose_stbox(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterDistancePoseStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "DistancePoseStboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return DistancePoseStboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
