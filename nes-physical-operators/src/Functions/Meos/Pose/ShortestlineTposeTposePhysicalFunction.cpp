...
#include <Functions/Meos/Pose/ShortestlineTposeTposePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterShortestlineTposeTposePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

ShortestlineTposeTposePhysicalFunction::ShortestlineTposeTposePhysicalFunction(PhysicalFunction xFunction,
...
VarVal ShortestlineTposeTposePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts2) -> char*
        {
            try
            {
                ...
                std::string arg0tW = fmt::format("Pose(Point({} {}),{})@{}", x2, y2, theta2, MEOS::Meos::convertEpochToTimestamp(ts2));
                Temporal* arg0t = tpose_in(arg0tW.c_str());
                if (!arg0t) { free(temp); return (char*) nullptr; }

                GSERIALIZED* res = (GSERIALIZED*) shortestline_tpose_tpose(temp, arg0t);
                free(temp);
                free(arg0t);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterShortestlineTposeTposePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 8,
                 "ShortestlineTposeTposePhysicalFunction requires 8 children but got {}",
    ...
    return ShortestlineTposeTposePhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4), std::move(arg5), std::move(arg6), std::move(arg7));
}
