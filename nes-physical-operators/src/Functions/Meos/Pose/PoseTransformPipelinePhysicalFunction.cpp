...
#include <Functions/Meos/Pose/PoseTransformPipelinePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterPoseTransformPipelinePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

PoseTransformPipelinePhysicalFunction::PoseTransformPipelinePhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal PoseTransformPipelinePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                Pose* temp = pose_in(tempS.c_str());
                if (!temp) return (char*) nullptr;

                Pose* res = (Pose*) pose_transform_pipeline(temp, (char*)"+proj=pipeline +step +proj=unitconvert +xy_in=deg +xy_out=rad +step +proj=webmerc +ellps=WGS84", 3857, true);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterPoseTransformPipelinePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "PoseTransformPipelinePhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return PoseTransformPipelinePhysicalFunction(std::move(arg0));
}
