...
#include <Functions/Meos/Pose/IntersectionSetPosePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterIntersectionSetPosePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

IntersectionSetPosePhysicalFunction::IntersectionSetPosePhysicalFunction(PhysicalFunction litFunction,
...
VarVal IntersectionSetPosePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Set* res = intersection_set_pose(temp, arg0B);
                free(temp);
                free(arg0B);
                if (!res) return (char*) nullptr;
                char* outStr = poseset_out(res, 15);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterIntersectionSetPosePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "IntersectionSetPosePhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return IntersectionSetPosePhysicalFunction(std::move(arg0), std::move(arg1));
}
