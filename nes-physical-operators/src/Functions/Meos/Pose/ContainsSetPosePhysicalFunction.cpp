...
#include <Functions/Meos/Pose/ContainsSetPosePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterContainsSetPosePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

ContainsSetPosePhysicalFunction::ContainsSetPosePhysicalFunction(PhysicalFunction litFunction,
...
VarVal ContainsSetPosePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = contains_set_pose(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterContainsSetPosePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "ContainsSetPosePhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return ContainsSetPosePhysicalFunction(std::move(arg0), std::move(arg1));
}
