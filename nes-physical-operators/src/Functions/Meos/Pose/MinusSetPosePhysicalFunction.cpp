...
#include <Functions/Meos/Pose/MinusSetPosePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterMinusSetPosePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

MinusSetPosePhysicalFunction::MinusSetPosePhysicalFunction(PhysicalFunction litFunction,
...
VarVal MinusSetPosePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Set* res = minus_set_pose(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterMinusSetPosePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "MinusSetPosePhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return MinusSetPosePhysicalFunction(std::move(arg0), std::move(arg1));
}
