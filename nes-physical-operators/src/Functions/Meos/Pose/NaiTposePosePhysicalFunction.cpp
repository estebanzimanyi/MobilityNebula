...
#include <Functions/Meos/Pose/NaiTposePosePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterNaiTposePosePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

NaiTposePosePhysicalFunction::NaiTposePosePhysicalFunction(PhysicalFunction xFunction,
...
VarVal NaiTposePosePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) nai_tpose_pose(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterNaiTposePosePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 5,
                 "NaiTposePosePhysicalFunction requires 5 children but got {}",
    ...
    return NaiTposePosePhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4));
}
