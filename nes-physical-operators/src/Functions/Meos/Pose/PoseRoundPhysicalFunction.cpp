...
#include <Functions/Meos/Pose/PoseRoundPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterPoseRoundPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

PoseRoundPhysicalFunction::PoseRoundPhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal PoseRoundPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterPoseRoundPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "PoseRoundPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return PoseRoundPhysicalFunction(std::move(arg0));
}
