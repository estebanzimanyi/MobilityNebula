...
#include <Functions/Meos/Pose/TposeValuesPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTposeValuesPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TposeValuesPhysicalFunction::TposeValuesPhysicalFunction(PhysicalFunction xFunction,
...
VarVal TposeValuesPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                int _cnt = 0;
                Pose ** arr = (Pose **) tpose_values(temp, &_cnt);
                free(temp);
                if (!arr || _cnt <= 0) return (char*) nullptr;
                std::string _s = "{";
                for (int _i = 0; _i < _cnt; _i++) { if (_i) _s += ", "; char* _e = pose_out(arr[_i], 15); if (_e) { _s += _e; free(_e); } free(arr[_i]); }
                _s += "}";
                free(arr);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTposeValuesPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 4,
                 "TposeValuesPhysicalFunction requires 4 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    auto arg3 = std::move(arguments.childFunctions[3]);
    return TposeValuesPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3));
}
