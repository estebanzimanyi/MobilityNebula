...
#include <Functions/Meos/Pose/PoseTstzspanToStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterPoseTstzspanToStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

PoseTstzspanToStboxPhysicalFunction::PoseTstzspanToStboxPhysicalFunction(PhysicalFunction litFunction,
...
VarVal PoseTstzspanToStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                STBox* res = (STBox*) pose_tstzspan_to_stbox(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterPoseTstzspanToStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "PoseTstzspanToStboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return PoseTstzspanToStboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
