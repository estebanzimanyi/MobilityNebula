...
#include <Functions/Meos/Pose/UnionSetPosePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterUnionSetPosePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

UnionSetPosePhysicalFunction::UnionSetPosePhysicalFunction(PhysicalFunction litFunction,
...
VarVal UnionSetPosePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Set* res = union_set_pose(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterUnionSetPosePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "UnionSetPosePhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return UnionSetPosePhysicalFunction(std::move(arg0), std::move(arg1));
}
