...
#include <Functions/Meos/Pose/TposeAtStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTposeAtStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TposeAtStboxPhysicalFunction::TposeAtStboxPhysicalFunction(PhysicalFunction xFunction,
...
VarVal TposeAtStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) tpose_at_stbox(temp, arg0B, true);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTposeAtStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 5,
                 "TposeAtStboxPhysicalFunction requires 5 children but got {}",
    ...
    return TposeAtStboxPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4));
}
