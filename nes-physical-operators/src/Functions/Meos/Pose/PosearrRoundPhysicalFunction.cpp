...
#include <Functions/Meos/Pose/PosearrRoundPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterPosearrRoundPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

PosearrRoundPhysicalFunction::PosearrRoundPhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal PosearrRoundPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                Pose* _ina[1] = { (Pose*) temp };
                Pose** _out = (Pose**) posearr_round((const Pose**) _ina, 1, 6);
                free(temp);
                if (!_out || !_out[0]) return (char*) nullptr;
                char* outStr = pose_out(_out[0], 6);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterPosearrRoundPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "PosearrRoundPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return PosearrRoundPhysicalFunction(std::move(arg0));
}
