...
#include <Functions/Meos/Pose/EverEqTposeTposePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterEverEqTposeTposePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

EverEqTposeTposePhysicalFunction::EverEqTposeTposePhysicalFunction(PhysicalFunction trajFunction,
    ...
    const auto result = nautilus::invoke(
        +[](const char* trajPtr, uint32_t trajSize,
            ...
            try
            {
                ...
                int r = ever_eq_tpose_tpose(temp, arg0T);
                free(temp);
                free(arg0T);
                return r;
            }
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "EverEqTposeTposePhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return EverEqTposeTposePhysicalFunction(std::move(arg0), std::move(arg1));
}
