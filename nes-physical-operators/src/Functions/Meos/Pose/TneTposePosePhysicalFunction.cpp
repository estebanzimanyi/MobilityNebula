...
#include <Functions/Meos/Pose/TneTposePosePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTneTposePosePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TneTposePosePhysicalFunction::TneTposePosePhysicalFunction(PhysicalFunction xFunction,
...
VarVal TneTposePosePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Temporal* res = tne_tpose_pose(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTneTposePosePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 5,
                 "TneTposePosePhysicalFunction requires 5 children but got {}",
    ...
    return TneTposePosePhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4));
}
