...
#include <Functions/Meos/Pose/TposeMinusStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTposeMinusStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TposeMinusStboxPhysicalFunction::TposeMinusStboxPhysicalFunction(PhysicalFunction xFunction,
...
VarVal TposeMinusStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) tpose_minus_stbox(temp, arg0B, true);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTposeMinusStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 5,
                 "TposeMinusStboxPhysicalFunction requires 5 children but got {}",
    ...
    return TposeMinusStboxPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4));
}
