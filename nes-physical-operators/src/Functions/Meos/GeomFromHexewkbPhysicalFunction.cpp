...
GeomFromHexewkbPhysicalFunction::GeomFromHexewkbPhysicalFunction(PhysicalFunction hexFunction)
{
...
VarVal GeomFromHexewkbPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto actualLen = nautilus::invoke(
        +[](const char* hexPtr, uint32_t hexSize,
            char* buf,
            ...
            try
            {
                ...
                GSERIALIZED* gres = geom_from_hexewkb(std::string(hexPtr,hexSize).c_str());
                if (!gres) return 0u;
        ...
        hex.getContent(), hex.getContentSize(), outBuf.getContent(), nautilus::val<uint32_t>(MAX_LEN));
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return GeomFromHexewkbPhysicalFunction(std::move(arg0));
}
