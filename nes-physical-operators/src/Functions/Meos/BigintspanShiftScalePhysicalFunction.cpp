...
#include <Functions/Meos/BigintspanShiftScalePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterBigintspanShiftScalePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

BigintspanShiftScalePhysicalFunction::BigintspanShiftScalePhysicalFunction(PhysicalFunction litFunction,
...
VarVal BigintspanShiftScalePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto arg1 = parameterValues[2].cast<nautilus::val<int64_t>>();
    ...
    auto outStr = nautilus::invoke(
            ...
            int64_t arg1) -> char*
        {
            try
            {
                ...
                Span* res = bigintspan_shift_scale(temp, arg0, arg1, true, true);
                free(temp);
                if (!res) return (char*) nullptr;
                char* outStr = bigintspan_out(res);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterBigintspanShiftScalePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "BigintspanShiftScalePhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return BigintspanShiftScalePhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
