...
#include <Functions/Meos/AfterSpansetDatePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterAfterSpansetDatePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

AfterSpansetDatePhysicalFunction::AfterSpansetDatePhysicalFunction(PhysicalFunction litFunction,
...
VarVal AfterSpansetDatePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = after_spanset_date(temp, arg0V);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterAfterSpansetDatePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "AfterSpansetDatePhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return AfterSpansetDatePhysicalFunction(std::move(arg0), std::move(arg1));
}
