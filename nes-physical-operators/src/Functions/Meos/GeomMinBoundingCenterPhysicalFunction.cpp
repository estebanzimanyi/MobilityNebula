...
GeomMinBoundingCenterPhysicalFunction::GeomMinBoundingCenterPhysicalFunction(PhysicalFunction wktFunction)
{
    ...
    const auto actualLen = nautilus::invoke(
        +[](const char* wktPtr, uint32_t wktSize,
            ...
            try
            {
                ...
                GSERIALIZED* gres = ({std::string _s(wktPtr,wktSize);GSERIALIZED* _gs=geom_in(_s.c_str(),-1);if(!_gs)return 0u;MinBoundingCircle _mbc=geom_min_bounding_radius(_gs);free(_gs);_mbc.center;});
                if (!gres) return 0u;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return GeomMinBoundingCenterPhysicalFunction(std::move(arg0));
}
