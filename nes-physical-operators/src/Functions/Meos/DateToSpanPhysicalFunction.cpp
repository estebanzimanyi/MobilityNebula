...
#include <Functions/Meos/DateToSpanPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterDateToSpanPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

DateToSpanPhysicalFunction::DateToSpanPhysicalFunction(PhysicalFunction valueFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(valueFunction));
}

VarVal DateToSpanPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterDateToSpanPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "DateToSpanPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return DateToSpanPhysicalFunction(std::move(arg0));
}
