...
#include <Functions/Meos/RightSpanSpanPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterRightSpanSpanPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

RightSpanSpanPhysicalFunction::RightSpanSpanPhysicalFunction(PhysicalFunction litFunction,
...
VarVal RightSpanSpanPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = right_span_span(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterRightSpanSpanPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "RightSpanSpanPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return RightSpanSpanPhysicalFunction(std::move(arg0), std::move(arg1));
}
