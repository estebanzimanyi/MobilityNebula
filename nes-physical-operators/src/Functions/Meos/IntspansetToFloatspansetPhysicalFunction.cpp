...
#include <Functions/Meos/IntspansetToFloatspansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterIntspansetToFloatspansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

IntspansetToFloatspansetPhysicalFunction::IntspansetToFloatspansetPhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal IntspansetToFloatspansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                SpanSet* res = intspanset_to_floatspanset(temp);
                free(temp);
                if (!res) return (char*) nullptr;
                char* outStr = floatspanset_out(res, 15);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterIntspansetToFloatspansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "IntspansetToFloatspansetPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return IntspansetToFloatspansetPhysicalFunction(std::move(arg0));
}
