...
#include <Functions/Meos/DatesetShiftScalePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterDatesetShiftScalePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

DatesetShiftScalePhysicalFunction::DatesetShiftScalePhysicalFunction(PhysicalFunction litFunction,
...
VarVal DatesetShiftScalePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int32_t arg1) -> char*
        {
            try
            {
                ...
                Set* res = dateset_shift_scale(temp, arg0, arg1, true, true);
                free(temp);
                if (!res) return (char*) nullptr;
                char* outStr = dateset_out(res);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterDatesetShiftScalePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "DatesetShiftScalePhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return DatesetShiftScalePhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
