...
GeoAsHexewkbPhysicalFunction::GeoAsHexewkbPhysicalFunction(PhysicalFunction wkt, PhysicalFunction endian)
{
    paramFns.reserve(2);
    paramFns.push_back(std::move(wkt));
    paramFns.push_back(std::move(endian));
}
...
VarVal GeoAsHexewkbPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto endian = paramFns[1].execute(record, arena).cast<VariableSizedData>();
    ...
    const auto actualLen = nautilus::invoke(
        +[](const char* w, uint32_t wsz, const char* e, uint32_t esz, char* buf, uint32_t bufMax) -> uint32_t {
            try {
                MEOS::Meos::ensureMeosInitialized();
                std::string s(w, wsz), es(e, esz);
                GSERIALIZED* gs = geom_in(s.c_str(), -1);
                if (!gs) return 0u;
                char* out = geo_as_hexewkb(gs, es.c_str());
                free(gs);
                if (!out) return 0u;
        ...
        wkt, endian, outBuf.getContent(), nautilus::val<uint32_t>(MAX_LEN));
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return GeoAsHexewkbPhysicalFunction(
