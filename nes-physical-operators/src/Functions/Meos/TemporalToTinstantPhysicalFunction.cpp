...
#include <Functions/Meos/TemporalToTinstantPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTemporalToTinstantPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TemporalToTinstantPhysicalFunction::TemporalToTinstantPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TemporalToTinstantPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) temporal_to_tinstant(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTemporalToTinstantPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TemporalToTinstantPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TemporalToTinstantPhysicalFunction(std::move(arg0), std::move(arg1));
}
