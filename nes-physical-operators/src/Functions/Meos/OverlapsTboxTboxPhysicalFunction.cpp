...
#include <Functions/Meos/OverlapsTboxTboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterOverlapsTboxTboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

OverlapsTboxTboxPhysicalFunction::OverlapsTboxTboxPhysicalFunction(PhysicalFunction litFunction,
...
VarVal OverlapsTboxTboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = overlaps_tbox_tbox(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterOverlapsTboxTboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "OverlapsTboxTboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return OverlapsTboxTboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
