...
GeomBufferPhysicalFunction::GeomBufferPhysicalFunction(PhysicalFunction wktFunction,
                                                          ...
                                                          PhysicalFunction paramsFunction)
{
    ...
    parameterFunctions.push_back(std::move(sizeFunction));
    parameterFunctions.push_back(std::move(paramsFunction));
}
...
VarVal GeomBufferPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto size = parameterValues[1].cast<nautilus::val<double>>();
    auto params = parameterValues[2].cast<VariableSizedData>();
    ...
    const auto actualLen = nautilus::invoke(
        +[](const char* wktPtr, uint32_t wktSize,
            double size,
            const char* paramsPtr, uint32_t paramsSize,
            char* buf,
            ...
            try
            {
                ...
                GSERIALIZED* gres = ({std::string _s(wktPtr,wktSize),_ps(paramsPtr,paramsSize);GSERIALIZED* _gs=geom_in(_s.c_str(),-1);if(!_gs)return 0u;GSERIALIZED* _res=geom_buffer(_gs,size,_ps.c_str());free(_gs);_res;});
                if (!gres) return 0u;
        ...
        wkt.getContent(), wkt.getContentSize(), size, params.getContent(), params.getContentSize(), outBuf.getContent(), nautilus::val<uint32_t>(MAX_LEN));
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return GeomBufferPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
