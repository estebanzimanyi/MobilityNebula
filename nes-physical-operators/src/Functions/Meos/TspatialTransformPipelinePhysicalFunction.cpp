...
#include <Functions/Meos/TspatialTransformPipelinePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTspatialTransformPipelinePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TspatialTransformPipelinePhysicalFunction::TspatialTransformPipelinePhysicalFunction(PhysicalFunction lonFunction,
...
VarVal TspatialTransformPipelinePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) tspatial_transform_pipeline(temp, (char*)"+proj=pipeline +step +proj=unitconvert +xy_in=deg +xy_out=rad +step +proj=webmerc +ellps=WGS84", 3857, true);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTspatialTransformPipelinePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TspatialTransformPipelinePhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TspatialTransformPipelinePhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
