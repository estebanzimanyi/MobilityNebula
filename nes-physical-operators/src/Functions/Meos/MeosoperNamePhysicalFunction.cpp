...
#include <Functions/Meos/MeosoperNamePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterMeosoperNamePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

MeosoperNamePhysicalFunction::MeosoperNamePhysicalFunction(PhysicalFunction valueFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(valueFunction));
}

VarVal MeosoperNamePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](int32_t value) -> char*
        {
            try
            {
                ...
                MeosOper temp = (MeosOper) value;

                const char* res = meosoper_name(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterMeosoperNamePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "MeosoperNamePhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return MeosoperNamePhysicalFunction(std::move(arg0));
}
