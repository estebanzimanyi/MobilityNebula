...
#include <Functions/Meos/TintValueBinsPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTintValueBinsPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TintValueBinsPhysicalFunction::TintValueBinsPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TintValueBinsPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int32_t arg1) -> char*
        {
            try
            {
                ...
                int _cnt = 0;
                Span* arr = (Span*) tint_value_bins(temp, arg0, arg1, &_cnt);
                free(temp);
                if (!arr || _cnt <= 0) return (char*) nullptr;
                std::string _s = "{";
                for (int _i = 0; _i < _cnt; _i++) { if (_i) _s += ", "; char* _e = intspan_out(&arr[_i]); if (_e) { _s += _e; free(_e); } }
                _s += "}";
                free(arr);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTintValueBinsPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 4,
                 "TintValueBinsPhysicalFunction requires 4 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    auto arg3 = std::move(arguments.childFunctions[3]);
    return TintValueBinsPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3));
}
