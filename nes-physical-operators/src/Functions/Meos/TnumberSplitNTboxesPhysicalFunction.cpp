...
#include <Functions/Meos/TnumberSplitNTboxesPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTnumberSplitNTboxesPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TnumberSplitNTboxesPhysicalFunction::TnumberSplitNTboxesPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TnumberSplitNTboxesPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int32_t arg0) -> char*
        {
            try
            {
                ...
                int _cnt = 0;
                TBox* arr = (TBox*) tnumber_split_n_tboxes(temp, arg0, &_cnt);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTnumberSplitNTboxesPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TnumberSplitNTboxesPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TnumberSplitNTboxesPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
