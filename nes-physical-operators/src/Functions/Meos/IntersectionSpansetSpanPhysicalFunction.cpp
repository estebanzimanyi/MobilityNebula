...
#include <Functions/Meos/IntersectionSpansetSpanPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterIntersectionSpansetSpanPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

IntersectionSpansetSpanPhysicalFunction::IntersectionSpansetSpanPhysicalFunction(PhysicalFunction litFunction,
...
VarVal IntersectionSpansetSpanPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Span* arg0B = intspan_in(arg0S.c_str());
                if (!arg0B) { free(temp); return (char*) nullptr; }

                SpanSet* res = intersection_spanset_span(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterIntersectionSpansetSpanPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "IntersectionSpansetSpanPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return IntersectionSpansetSpanPhysicalFunction(std::move(arg0), std::move(arg1));
}
