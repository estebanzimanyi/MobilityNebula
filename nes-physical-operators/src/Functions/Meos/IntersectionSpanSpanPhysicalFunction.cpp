...
#include <Functions/Meos/IntersectionSpanSpanPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterIntersectionSpanSpanPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

IntersectionSpanSpanPhysicalFunction::IntersectionSpanSpanPhysicalFunction(PhysicalFunction litFunction,
...
VarVal IntersectionSpanSpanPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Span* res = intersection_span_span(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterIntersectionSpanSpanPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "IntersectionSpanSpanPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return IntersectionSpanSpanPhysicalFunction(std::move(arg0), std::move(arg1));
}
