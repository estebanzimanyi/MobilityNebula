...
#include <Functions/Meos/TgeompointToTgeometryPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTgeompointToTgeometryPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TgeompointToTgeometryPhysicalFunction::TgeompointToTgeometryPhysicalFunction(PhysicalFunction lonFunction,
...
VarVal TgeompointToTgeometryPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) tgeompoint_to_tgeometry(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTgeompointToTgeometryPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TgeompointToTgeometryPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TgeompointToTgeometryPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
