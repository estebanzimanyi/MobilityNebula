...
#include <Functions/Meos/TltIntTintPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTltIntTintPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TltIntTintPhysicalFunction::TltIntTintPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TltIntTintPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int32_t arg0) -> char*
        {
            try
            {
                ...
                Temporal* res = tlt_int_tint(arg0, temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTltIntTintPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TltIntTintPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TltIntTintPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
