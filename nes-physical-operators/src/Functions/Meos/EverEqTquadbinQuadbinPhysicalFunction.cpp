...
EverEqTquadbinQuadbinPhysicalFunction::EverEqTquadbinQuadbinPhysicalFunction(PhysicalFunction inst_cell, PhysicalFunction ts, PhysicalFunction cell) {
    paramFns.reserve(3);
...
VarVal EverEqTquadbinQuadbinPhysicalFunction::execute(const Record& record, ArenaRef& arena) const {
    ...
    const auto result = nautilus::invoke(
        +[](uint64_t ic, uint64_t ts, uint64_t cell) -> double {
            ...
            if (!inst) return 0.0;
            int r = ever_eq_tquadbin_quadbin((const Temporal*)inst, (Quadbin)cell);
            free(inst);
            return r > 0 ? 1.0 : 0.0;
        },
        inst_cell, ts, cell);
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return EverEqTquadbinQuadbinPhysicalFunction(
