...
#include <Functions/Meos/NadStboxGeoPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterNadStboxGeoPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

NadStboxGeoPhysicalFunction::NadStboxGeoPhysicalFunction(PhysicalFunction boxFunction,
...
VarVal NadStboxGeoPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* boxPtr, uint32_t boxSize,
            ...
            try
            {
                ...
                double r = nad_stbox_geo(temp, arg0G.getGeometry());
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterNadStboxGeoPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "NadStboxGeoPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return NadStboxGeoPhysicalFunction(std::move(arg0), std::move(arg1));
}
