...
#include <Functions/Meos/AfterDateSpansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterAfterDateSpansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

AfterDateSpansetPhysicalFunction::AfterDateSpansetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal AfterDateSpansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = after_date_spanset(arg0V, temp);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterAfterDateSpansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "AfterDateSpansetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return AfterDateSpansetPhysicalFunction(std::move(arg0), std::move(arg1));
}
