...
#include <Functions/Meos/LeftSpanSpansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterLeftSpanSpansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

LeftSpanSpansetPhysicalFunction::LeftSpanSpansetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal LeftSpanSpansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = left_span_spanset(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterLeftSpanSpansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "LeftSpanSpansetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return LeftSpanSpansetPhysicalFunction(std::move(arg0), std::move(arg1));
}
