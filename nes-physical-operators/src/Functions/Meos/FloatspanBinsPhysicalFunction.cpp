...
#include <Functions/Meos/FloatspanBinsPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterFloatspanBinsPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

FloatspanBinsPhysicalFunction::FloatspanBinsPhysicalFunction(PhysicalFunction litFunction,
...
VarVal FloatspanBinsPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            double arg1) -> char*
        {
            try
            {
                ...
                int _cnt = 0;
                Span* arr = (Span*) floatspan_bins(temp, arg0, arg1, &_cnt);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterFloatspanBinsPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "FloatspanBinsPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return FloatspanBinsPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
