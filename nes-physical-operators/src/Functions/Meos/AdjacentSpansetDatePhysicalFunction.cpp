...
#include <Functions/Meos/AdjacentSpansetDatePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterAdjacentSpansetDatePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

AdjacentSpansetDatePhysicalFunction::AdjacentSpansetDatePhysicalFunction(PhysicalFunction litFunction,
...
VarVal AdjacentSpansetDatePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = adjacent_spanset_date(temp, arg0V);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterAdjacentSpansetDatePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "AdjacentSpansetDatePhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return AdjacentSpansetDatePhysicalFunction(std::move(arg0), std::move(arg1));
}
