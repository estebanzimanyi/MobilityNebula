...
GeoGeoNPhysicalFunction::GeoGeoNPhysicalFunction(PhysicalFunction wkt, PhysicalFunction n)
{
    paramFns.reserve(2);
    ...
    const auto actualLen = nautilus::invoke(
        +[](const char* w, uint32_t wsz, uint64_t n, char* buf, uint32_t bufMax) -> uint32_t {
            try {
                ...
                if (!gs) return 0u;
                GSERIALIZED* result = geo_geo_n(gs, (int)n);
                free(gs);
                if (!result) return 0u;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return GeoGeoNPhysicalFunction(
