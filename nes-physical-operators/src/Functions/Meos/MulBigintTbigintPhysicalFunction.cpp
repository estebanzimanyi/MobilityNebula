...
#include <Functions/Meos/MulBigintTbigintPhysicalFunction.hpp>
...
MulBigintTbigintPhysicalFunction::MulBigintTbigintPhysicalFunction(PhysicalFunction scalarFunction,
...
VarVal MulBigintTbigintPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](double scalar_d, double value_d, uint64_t ts) -> double {
            try {
                ...
                if (!temp) return 0.0;
                Temporal* res = mul_bigint_tbigint(static_cast<int64_t>(scalar_d), temp);
                free(temp);
                if (!res) return 0.0;
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterMulBigintTbigintPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "MulBigintTbigintPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return MulBigintTbigintPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
