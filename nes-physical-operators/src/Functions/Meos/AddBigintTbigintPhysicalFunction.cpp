...
#include <Functions/Meos/AddBigintTbigintPhysicalFunction.hpp>
...
AddBigintTbigintPhysicalFunction::AddBigintTbigintPhysicalFunction(PhysicalFunction scalarFunction,
...
VarVal AddBigintTbigintPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](double scalar_d, double value_d, uint64_t ts) -> double {
            try {
                ...
                if (!temp) return 0.0;
                Temporal* res = add_bigint_tbigint(static_cast<int64_t>(scalar_d), temp);
                free(temp);
                if (!res) return 0.0;
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterAddBigintTbigintPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "AddBigintTbigintPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return AddBigintTbigintPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
