...
#include <Functions/Meos/BeforeDateSpanPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterBeforeDateSpanPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

BeforeDateSpanPhysicalFunction::BeforeDateSpanPhysicalFunction(PhysicalFunction litFunction,
...
VarVal BeforeDateSpanPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = before_date_span(arg0V, temp);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterBeforeDateSpanPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "BeforeDateSpanPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return BeforeDateSpanPhysicalFunction(std::move(arg0), std::move(arg1));
}
