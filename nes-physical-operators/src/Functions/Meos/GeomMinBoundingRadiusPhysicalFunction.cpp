...
GeomMinBoundingRadiusPhysicalFunction::GeomMinBoundingRadiusPhysicalFunction(PhysicalFunction wktFunction)
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* wktPtr, uint32_t wktSize) -> double {
            try
            {
                ...
                double r = ({std::string _s(wktPtr,wktSize);GSERIALIZED* _gs=geom_in(_s.c_str(),-1);if(!_gs)return 0.0;MinBoundingCircle _mbc=geom_min_bounding_radius(_gs);double _r=_mbc.radius;if(_mbc.center)free(_mbc.center);free(_gs);_r;});
                return r;
            }
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return GeomMinBoundingRadiusPhysicalFunction(std::move(arg0));
}
