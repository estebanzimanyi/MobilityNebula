...
#include <Functions/Meos/TpointMinusElevationPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTpointMinusElevationPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TpointMinusElevationPhysicalFunction::TpointMinusElevationPhysicalFunction(PhysicalFunction trajFunction,
...
VarVal TpointMinusElevationPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) tpoint_minus_elevation(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTpointMinusElevationPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TpointMinusElevationPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TpointMinusElevationPhysicalFunction(std::move(arg0), std::move(arg1));
}
