...
JsonbCmpPhysicalFunction::JsonbCmpPhysicalFunction(PhysicalFunction jb1, PhysicalFunction jb2) {
    paramFns.reserve(2);
    ...
    const auto result = nautilus::invoke(
        +[](const char* w1, uint32_t w1sz, const char* w2, uint32_t w2sz) -> double {
            try {
                ...
                double r = (double)jsonb_cmp(jb1, jb2);
                free(jb1); free(jb2);
                return r;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return JsonbCmpPhysicalFunction(
