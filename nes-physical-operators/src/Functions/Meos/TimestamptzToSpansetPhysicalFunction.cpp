...
#include <Functions/Meos/TimestamptzToSpansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTimestamptzToSpansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TimestamptzToSpansetPhysicalFunction::TimestamptzToSpansetPhysicalFunction(PhysicalFunction valueFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(valueFunction));
}

VarVal TimestamptzToSpansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](int64_t value) -> char*
        {
            try
            {
                ...
                SpanSet* res = timestamptz_to_spanset(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTimestamptzToSpansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "TimestamptzToSpansetPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return TimestamptzToSpansetPhysicalFunction(std::move(arg0));
}
