...
#include <Functions/Meos/TempsubtypeNamePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTempsubtypeNamePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TempsubtypeNamePhysicalFunction::TempsubtypeNamePhysicalFunction(PhysicalFunction valueFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(valueFunction));
}

VarVal TempsubtypeNamePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](int32_t value) -> char*
        {
            try
            {
                ...
                tempSubtype temp = (tempSubtype) value;

                const char* res = tempsubtype_name(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTempsubtypeNamePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "TempsubtypeNamePhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return TempsubtypeNamePhysicalFunction(std::move(arg0));
}
