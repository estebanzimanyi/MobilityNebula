...
#include <Functions/Meos/OverlapsTboxTnumberPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterOverlapsTboxTnumberPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

OverlapsTboxTnumberPhysicalFunction::OverlapsTboxTnumberPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal OverlapsTboxTnumberPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](double value,
            ...
            try
            {
                ...
                bool r = overlaps_tbox_tnumber(arg0B, temp);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterOverlapsTboxTnumberPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "OverlapsTboxTnumberPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return OverlapsTboxTnumberPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
