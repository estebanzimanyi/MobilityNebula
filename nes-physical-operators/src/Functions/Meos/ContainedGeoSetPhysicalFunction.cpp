...
#include <Functions/Meos/ContainedGeoSetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterContainedGeoSetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

ContainedGeoSetPhysicalFunction::ContainedGeoSetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal ContainedGeoSetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = contained_geo_set(arg0G.getGeometry(), temp);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterContainedGeoSetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "ContainedGeoSetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return ContainedGeoSetPhysicalFunction(std::move(arg0), std::move(arg1));
}
