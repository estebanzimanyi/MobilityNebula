...
AlwaysEqPoseTposePhysicalFunction::AlwaysEqPoseTposePhysicalFunction(PhysicalFunction x2, PhysicalFunction y2, PhysicalFunction theta2, PhysicalFunction x, PhysicalFunction y, PhysicalFunction theta, PhysicalFunction ts)
{
    paramFns.reserve(7);
    ...
    const auto result = nautilus::invoke(
        +[](double x2, double y2, double theta2, double x, double y, double theta, uint64_t ts) -> double {
            try {
                ...
                int r = always_eq_pose_tpose(pose_s, inst);
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 7,
                 "AlwaysEqPoseTposePhysicalFunction requires 7 children but got {}",
                 arguments.childFunctions.size());
    return AlwaysEqPoseTposePhysicalFunction(
