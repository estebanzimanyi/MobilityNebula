...
LinePointNPhysicalFunction::LinePointNPhysicalFunction(PhysicalFunction wktFunction,
    ...
    const auto actualLen = nautilus::invoke(
        +[](const char* wktPtr, uint32_t wktSize,
            ...
            try
            {
                ...
                GSERIALIZED* gres = line_point_n(temp, (int)arg0);
                free(temp);
                if (!gres) return 0u;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return LinePointNPhysicalFunction(std::move(arg0), std::move(arg1));
}
