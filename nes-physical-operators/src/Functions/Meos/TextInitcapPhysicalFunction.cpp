...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTextInitcapPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TextInitcapPhysicalFunction::TextInitcapPhysicalFunction(PhysicalFunction litFunction)
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                text* res = (text*) text_initcap(temp);
                free(temp);
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "TextInitcapPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return TextInitcapPhysicalFunction(std::move(arg0));
}
