...
#include <Functions/Meos/SetSplitEachNSpansPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterSetSplitEachNSpansPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

SetSplitEachNSpansPhysicalFunction::SetSplitEachNSpansPhysicalFunction(PhysicalFunction litFunction,
...
VarVal SetSplitEachNSpansPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int32_t arg0) -> char*
        {
            try
            {
                ...
                Set* temp = intset_in(tempS.c_str());
                if (!temp) return (char*) nullptr;

                int _cnt = 0;
                Span* arr = (Span*) set_split_each_n_spans(temp, arg0, &_cnt);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterSetSplitEachNSpansPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "SetSplitEachNSpansPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return SetSplitEachNSpansPhysicalFunction(std::move(arg0), std::move(arg1));
}
