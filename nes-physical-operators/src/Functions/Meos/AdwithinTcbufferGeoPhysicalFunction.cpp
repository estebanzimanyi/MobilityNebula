...
AdwithinTcbufferGeoPhysicalFunction::AdwithinTcbufferGeoPhysicalFunction(PhysicalFunction lon, PhysicalFunction lat,
                                              PhysicalFunction radius, PhysicalFunction ts,
                                              PhysicalFunction wkt, PhysicalFunction dist)
{
    paramFns.reserve(6);
...
VarVal AdwithinTcbufferGeoPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto dist   = paramFns[5].execute(record, arena).cast<double>();

    const auto result = nautilus::invoke(
        +[](double lon, double lat, double radius, uint64_t ts,
            const char* wkt, uint32_t wkt_len, double dist) -> double {
            try {
                MEOS::Meos::ensureMeosInitialized();
                // Build tcbuffer instant from lon/lat/radius/ts
                ...
                int r = adwithin_tcbuffer_geo(tcb_inst, target_gs, dist);
                free(tcb_inst);
                free(target_gs);
                return r > 0 ? 1.0 : 0.0;
            } catch (const std::exception&) { return 0.0; }
        },
        lon, lat, radius, ts, wkt, dist);
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return AdwithinTcbufferGeoPhysicalFunction(std::move(arguments.childFunctions[0]),
