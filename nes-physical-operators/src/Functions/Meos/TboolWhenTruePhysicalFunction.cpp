...
#include <Functions/Meos/TboolWhenTruePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTboolWhenTruePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TboolWhenTruePhysicalFunction::TboolWhenTruePhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TboolWhenTruePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                SpanSet* res = (SpanSet*) tbool_when_true(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTboolWhenTruePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TboolWhenTruePhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TboolWhenTruePhysicalFunction(std::move(arg0), std::move(arg1));
}
