...
TfloatToTbigintPhysicalFunction::TfloatToTbigintPhysicalFunction(PhysicalFunction trajFunction)
{
    ...
    auto hexStr = nautilus::invoke(
        +[](const char* aPtr, uint32_t aSize) -> char*
        {
            try
            {
                ...
                Temporal* res = tfloat_to_tbigint(a);
                free(a);
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "TfloatToTbigintPhysicalFunction requires 1 child but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return TfloatToTbigintPhysicalFunction(std::move(arg0));
}
