...
#include <Functions/Meos/ContainsTboxTboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterContainsTboxTboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

ContainsTboxTboxPhysicalFunction::ContainsTboxTboxPhysicalFunction(PhysicalFunction litFunction,
...
VarVal ContainsTboxTboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = contains_tbox_tbox(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterContainsTboxTboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "ContainsTboxTboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return ContainsTboxTboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
