...
#include <Functions/Meos/SubBigintTbigintPhysicalFunction.hpp>
...
SubBigintTbigintPhysicalFunction::SubBigintTbigintPhysicalFunction(PhysicalFunction scalarFunction,
...
VarVal SubBigintTbigintPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](double scalar_d, double value_d, uint64_t ts) -> double {
            try {
                ...
                if (!temp) return 0.0;
                Temporal* res = sub_bigint_tbigint(static_cast<int64_t>(scalar_d), temp);
                free(temp);
                if (!res) return 0.0;
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterSubBigintTbigintPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "SubBigintTbigintPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return SubBigintTbigintPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
