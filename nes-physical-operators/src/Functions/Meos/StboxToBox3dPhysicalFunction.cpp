...
#include <Functions/Meos/StboxToBox3dPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterStboxToBox3dPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

StboxToBox3dPhysicalFunction::StboxToBox3dPhysicalFunction(PhysicalFunction boxFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(boxFunction));
}

VarVal StboxToBox3dPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* boxPtr, uint32_t boxSize) -> char*
        {
            try
            {
                ...
                BOX3D* res = (BOX3D*) stbox_to_box3d(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterStboxToBox3dPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "StboxToBox3dPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return StboxToBox3dPhysicalFunction(std::move(arg0));
}
