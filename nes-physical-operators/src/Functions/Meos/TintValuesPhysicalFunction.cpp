...
#include <Functions/Meos/TintValuesPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTintValuesPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TintValuesPhysicalFunction::TintValuesPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TintValuesPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                int _cnt = 0;
                int32_t* arr = (int32_t*) tint_values(temp, &_cnt);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTintValuesPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TintValuesPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TintValuesPhysicalFunction(std::move(arg0), std::move(arg1));
}
