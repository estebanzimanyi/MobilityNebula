...
#include <Functions/Meos/TnumberTboxesPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTnumberTboxesPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TnumberTboxesPhysicalFunction::TnumberTboxesPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TnumberTboxesPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                int _cnt = 0;
                TBox* arr = (TBox*) tnumber_tboxes(temp, &_cnt);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTnumberTboxesPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TnumberTboxesPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TnumberTboxesPhysicalFunction(std::move(arg0), std::move(arg1));
}
