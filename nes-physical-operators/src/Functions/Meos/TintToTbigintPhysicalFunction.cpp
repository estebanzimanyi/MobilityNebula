...
#include <Functions/Meos/TintToTbigintPhysicalFunction.hpp>
...
TintToTbigintPhysicalFunction::TintToTbigintPhysicalFunction(PhysicalFunction trajFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(trajFunction));
}

VarVal TintToTbigintPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto hexStr = nautilus::invoke(
        +[](const char* aPtr, uint32_t aSize) -> char*
        {
            try
            {
                ...
                Temporal* res = tint_to_tbigint(a);
                free(a);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTintToTbigintPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "TintToTbigintPhysicalFunction requires 1 child but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return TintToTbigintPhysicalFunction(std::move(arg0));
}
