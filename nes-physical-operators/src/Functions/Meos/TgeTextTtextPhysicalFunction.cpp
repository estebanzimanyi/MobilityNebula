...
#include <Functions/Meos/TgeTextTtextPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTgeTextTtextPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TgeTextTtextPhysicalFunction::TgeTextTtextPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TgeTextTtextPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Temporal* res = tge_text_ttext(arg0T, temp);
                free(temp);
                free(arg0T);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTgeTextTtextPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TgeTextTtextPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TgeTextTtextPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
