...
#include <Functions/Meos/TboxExpandTimePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTboxExpandTimePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TboxExpandTimePhysicalFunction::TboxExpandTimePhysicalFunction(PhysicalFunction litFunction,
...
VarVal TboxExpandTimePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                TBox* res = tbox_expand_time(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTboxExpandTimePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TboxExpandTimePhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TboxExpandTimePhysicalFunction(std::move(arg0), std::move(arg1));
}
