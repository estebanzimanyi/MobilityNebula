...
#include <Functions/Meos/StboxToGeoPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterStboxToGeoPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

StboxToGeoPhysicalFunction::StboxToGeoPhysicalFunction(PhysicalFunction boxFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(boxFunction));
}

VarVal StboxToGeoPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* boxPtr, uint32_t boxSize) -> char*
        {
            try
            {
                ...
                GSERIALIZED* res = (GSERIALIZED*) stbox_to_geo(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterStboxToGeoPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "StboxToGeoPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return StboxToGeoPhysicalFunction(std::move(arg0));
}
