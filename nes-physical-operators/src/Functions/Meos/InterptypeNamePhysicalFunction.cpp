...
#include <Functions/Meos/InterptypeNamePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterInterptypeNamePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

InterptypeNamePhysicalFunction::InterptypeNamePhysicalFunction(PhysicalFunction valueFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(valueFunction));
}

VarVal InterptypeNamePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](int32_t value) -> char*
        {
            try
            {
                ...
                interpType temp = (interpType) value;

                const char* res = interptype_name(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterInterptypeNamePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "InterptypeNamePhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return InterptypeNamePhysicalFunction(std::move(arg0));
}
