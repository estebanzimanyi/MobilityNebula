...
#include <Functions/Meos/TfloatValuesPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTfloatValuesPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TfloatValuesPhysicalFunction::TfloatValuesPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TfloatValuesPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                int _cnt = 0;
                double* arr = (double*) tfloat_values(temp, &_cnt);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTfloatValuesPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TfloatValuesPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TfloatValuesPhysicalFunction(std::move(arg0), std::move(arg1));
}
