...
#include <Functions/Meos/GeoPointarrPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterGeoPointarrPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

GeoPointarrPhysicalFunction::GeoPointarrPhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal GeoPointarrPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                int _cnt = 0;
                GSERIALIZED ** arr = (GSERIALIZED **) geo_pointarr(temp, &_cnt);
                if (!arr || _cnt <= 0) return (char*) nullptr;
                std::string _s = "{";
                for (int _i = 0; _i < _cnt; _i++) { if (_i) _s += ", "; char* _e = geo_out(arr[_i]); if (_e) { _s += _e; free(_e); } free(arr[_i]); }
                _s += "}";
                free(arr);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterGeoPointarrPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "GeoPointarrPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return GeoPointarrPhysicalFunction(std::move(arg0));
}
