...
#include <Functions/Meos/DivIntTintPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterDivIntTintPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

DivIntTintPhysicalFunction::DivIntTintPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal DivIntTintPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int32_t arg0) -> char*
        {
            try
            {
                ...
                Temporal* res = div_int_tint(arg0, temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterDivIntTintPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "DivIntTintPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return DivIntTintPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
