...
#include <Functions/Meos/OverleftTextSetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterOverleftTextSetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

OverleftTextSetPhysicalFunction::OverleftTextSetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal OverleftTextSetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = overleft_text_set(arg0T, temp);
                free(temp);
                free(arg0T);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterOverleftTextSetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "OverleftTextSetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return OverleftTextSetPhysicalFunction(std::move(arg0), std::move(arg1));
}
