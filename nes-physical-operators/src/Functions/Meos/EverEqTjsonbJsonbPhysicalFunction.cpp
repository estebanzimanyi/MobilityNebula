...
#include <Functions/Meos/EverEqTjsonbJsonbPhysicalFunction.hpp>
...
EverEqTjsonbJsonbPhysicalFunction::EverEqTjsonbJsonbPhysicalFunction(PhysicalFunction json_str, PhysicalFunction ts, PhysicalFunction target_json)
{
    paramFns.reserve(3);
...
VarVal EverEqTjsonbJsonbPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* json_str, uint32_t json_len, uint64_t ts, const char* target_json, uint32_t target_len) -> double {
            try {
                ...
                bool r = ever_eq_tjsonb_jsonb((Temporal*)inst, target) > 0;
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterEverEqTjsonbJsonbPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "EverEqTjsonbJsonbPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    return EverEqTjsonbJsonbPhysicalFunction(
