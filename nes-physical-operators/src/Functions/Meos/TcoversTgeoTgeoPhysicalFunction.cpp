...
#include <Functions/Meos/TcoversTgeoTgeoPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTcoversTgeoTgeoPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TcoversTgeoTgeoPhysicalFunction::TcoversTgeoTgeoPhysicalFunction(PhysicalFunction trajFunction,
...
VarVal TcoversTgeoTgeoPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto hexStr = nautilus::invoke(
        +[](const char* aPtr, uint32_t aSize, const char* bPtr, uint32_t bSize) -> char*
        {
            try
            {
                ...
                Temporal* res = tcovers_tgeo_tgeo(a, b);
                free(a);
                free(b);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTcoversTgeoTgeoPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TcoversTgeoTgeoPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TcoversTgeoTgeoPhysicalFunction(std::move(arg0), std::move(arg1));
}
