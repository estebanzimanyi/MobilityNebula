...
#include <Functions/Meos/BeforeSpanDatePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterBeforeSpanDatePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

BeforeSpanDatePhysicalFunction::BeforeSpanDatePhysicalFunction(PhysicalFunction litFunction,
...
VarVal BeforeSpanDatePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = before_span_date(temp, arg0V);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterBeforeSpanDatePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "BeforeSpanDatePhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return BeforeSpanDatePhysicalFunction(std::move(arg0), std::move(arg1));
}
