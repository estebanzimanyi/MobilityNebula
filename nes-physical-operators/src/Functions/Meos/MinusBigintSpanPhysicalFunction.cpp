...
#include <Functions/Meos/MinusBigintSpanPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterMinusBigintSpanPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

MinusBigintSpanPhysicalFunction::MinusBigintSpanPhysicalFunction(PhysicalFunction litFunction,
...
VarVal MinusBigintSpanPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int64_t arg0) -> char*
        {
            try
            {
                ...
                SpanSet* res = minus_bigint_span(arg0, temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterMinusBigintSpanPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "MinusBigintSpanPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return MinusBigintSpanPhysicalFunction(std::move(arg0), std::move(arg1));
}
