...
#include <Functions/Meos/UnionSpansetBigintPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterUnionSpansetBigintPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

UnionSpansetBigintPhysicalFunction::UnionSpansetBigintPhysicalFunction(PhysicalFunction litFunction,
...
VarVal UnionSpansetBigintPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int64_t arg0) -> char*
        {
            try
            {
                ...
                SpanSet* res = union_spanset_bigint(temp, arg0);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterUnionSpansetBigintPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "UnionSpansetBigintPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return UnionSpansetBigintPhysicalFunction(std::move(arg0), std::move(arg1));
}
