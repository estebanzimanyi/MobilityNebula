...
#include <Functions/Meos/TintValueTimeSplitPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTintValueTimeSplitPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TintValueTimeSplitPhysicalFunction::TintValueTimeSplitPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TintValueTimeSplitPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto arg2 = parameterValues[4].cast<nautilus::val<int32_t>>();
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg3Ptr, uint32_t arg3Size) -> char*
        {
            try
            {
                ...
                Temporal ** arr = (Temporal **) tint_value_time_split(temp, arg0, arg1B, arg2, arg3V, &_aux, &_aux2, &_cnt);
                free(temp);
                free(arg1B);
                if (!arr || _cnt <= 0) return (char*) nullptr;
                std::string _s = "{";
                for (int _i = 0; _i < _cnt; _i++) { if (_i) _s += ", "; char* _e = tint_out((Temporal *) arr[_i]); if (_e) { _s += _e; free(_e); } free(arr[_i]); }
                _s += "}";
                free(arr);
                free(_aux);
                free(_aux2);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTintValueTimeSplitPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 6,
                 "TintValueTimeSplitPhysicalFunction requires 6 children but got {}",
    ...
    return TintValueTimeSplitPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3), std::move(arg4), std::move(arg5));
}
