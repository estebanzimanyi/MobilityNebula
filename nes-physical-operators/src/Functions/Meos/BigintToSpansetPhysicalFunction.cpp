...
#include <Functions/Meos/BigintToSpansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterBigintToSpansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

BigintToSpansetPhysicalFunction::BigintToSpansetPhysicalFunction(PhysicalFunction valueFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(valueFunction));
}

VarVal BigintToSpansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](int64_t value) -> char*
        {
            try
            {
                ...
                SpanSet* res = bigint_to_spanset(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterBigintToSpansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "BigintToSpansetPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return BigintToSpansetPhysicalFunction(std::move(arg0));
}
