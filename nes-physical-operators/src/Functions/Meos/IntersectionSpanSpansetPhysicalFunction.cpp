...
#include <Functions/Meos/IntersectionSpanSpansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterIntersectionSpanSpansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

IntersectionSpanSpansetPhysicalFunction::IntersectionSpanSpansetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal IntersectionSpanSpansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                SpanSet* res = intersection_span_spanset(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterIntersectionSpanSpansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "IntersectionSpanSpansetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return IntersectionSpanSpansetPhysicalFunction(std::move(arg0), std::move(arg1));
}
