...
#include <Functions/Meos/MinusIntSpansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterMinusIntSpansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

MinusIntSpansetPhysicalFunction::MinusIntSpansetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal MinusIntSpansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int32_t arg0) -> char*
        {
            try
            {
                ...
                SpanSet* res = minus_int_spanset(arg0, temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterMinusIntSpansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "MinusIntSpansetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return MinusIntSpansetPhysicalFunction(std::move(arg0), std::move(arg1));
}
