...
#include <Functions/Meos/EverNeGeoTrgeometryPhysicalFunction.hpp>
...
EverNeGeoTrgeometryPhysicalFunction::EverNeGeoTrgeometryPhysicalFunction(PhysicalFunction tgt_wkt, PhysicalFunction ref_wkt, PhysicalFunction x, PhysicalFunction y, PhysicalFunction theta, PhysicalFunction ts)
{
    paramFns.reserve(6);
    ...
    const auto result = nautilus::invoke(
        +[](const char* tgt_wkt, uint32_t tgt_len, const char* ref_wkt, uint32_t ref_len, double x, double y, double theta, uint64_t ts) -> double {
            try {
                ...
                int r = ever_ne_geo_trgeometry(gs_tgt, (Temporal*)inst);
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 6,
                 "EverNeGeoTrgeometryPhysicalFunction requires 6 children but got {}",
                 arguments.childFunctions.size());
    return EverNeGeoTrgeometryPhysicalFunction(
