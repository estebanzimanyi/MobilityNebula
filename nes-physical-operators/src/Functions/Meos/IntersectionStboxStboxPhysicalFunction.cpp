...
#include <Functions/Meos/IntersectionStboxStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterIntersectionStboxStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

IntersectionStboxStboxPhysicalFunction::IntersectionStboxStboxPhysicalFunction(PhysicalFunction boxFunction,
...
VarVal IntersectionStboxStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                STBox* res = intersection_stbox_stbox(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterIntersectionStboxStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "IntersectionStboxStboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return IntersectionStboxStboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
