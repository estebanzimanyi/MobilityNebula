...
#include <Functions/Meos/TnumberValuespansPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTnumberValuespansPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TnumberValuespansPhysicalFunction::TnumberValuespansPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TnumberValuespansPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                SpanSet* res = tnumber_valuespans(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTnumberValuespansPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TnumberValuespansPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TnumberValuespansPhysicalFunction(std::move(arg0), std::move(arg1));
}
