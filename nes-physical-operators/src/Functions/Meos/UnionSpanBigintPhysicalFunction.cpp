...
#include <Functions/Meos/UnionSpanBigintPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterUnionSpanBigintPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

UnionSpanBigintPhysicalFunction::UnionSpanBigintPhysicalFunction(PhysicalFunction litFunction,
...
VarVal UnionSpanBigintPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int64_t arg0) -> char*
        {
            try
            {
                ...
                SpanSet* res = union_span_bigint(temp, arg0);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterUnionSpanBigintPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "UnionSpanBigintPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return UnionSpanBigintPhysicalFunction(std::move(arg0), std::move(arg1));
}
