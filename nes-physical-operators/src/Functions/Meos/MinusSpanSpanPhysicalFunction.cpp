...
#include <Functions/Meos/MinusSpanSpanPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterMinusSpanSpanPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

MinusSpanSpanPhysicalFunction::MinusSpanSpanPhysicalFunction(PhysicalFunction litFunction,
...
VarVal MinusSpanSpanPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                SpanSet* res = minus_span_span(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterMinusSpanSpanPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "MinusSpanSpanPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return MinusSpanSpanPhysicalFunction(std::move(arg0), std::move(arg1));
}
