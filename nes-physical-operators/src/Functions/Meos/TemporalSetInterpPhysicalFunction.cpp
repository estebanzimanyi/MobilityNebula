...
#include <Functions/Meos/TemporalSetInterpPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTemporalSetInterpPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TemporalSetInterpPhysicalFunction::TemporalSetInterpPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TemporalSetInterpPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                Temporal* res = (Temporal*) temporal_set_interp(temp, (interpType) 2);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTemporalSetInterpPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TemporalSetInterpPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TemporalSetInterpPhysicalFunction(std::move(arg0), std::move(arg1));
}
