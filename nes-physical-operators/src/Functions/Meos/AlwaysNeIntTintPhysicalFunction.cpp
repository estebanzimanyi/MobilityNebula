...
#include <Functions/Meos/AlwaysNeIntTintPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterAlwaysNeIntTintPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

AlwaysNeIntTintPhysicalFunction::AlwaysNeIntTintPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal AlwaysNeIntTintPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](int valueValue,
            ...
            try
            {
                ...
                if (!temp) return 0;
                int r = always_ne_int_tint(scalarValue, temp);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterAlwaysNeIntTintPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "AlwaysNeIntTintPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return AlwaysNeIntTintPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
