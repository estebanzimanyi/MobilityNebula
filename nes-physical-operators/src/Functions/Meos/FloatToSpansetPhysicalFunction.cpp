...
#include <Functions/Meos/FloatToSpansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterFloatToSpansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

FloatToSpansetPhysicalFunction::FloatToSpansetPhysicalFunction(PhysicalFunction valueFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(valueFunction));
}

VarVal FloatToSpansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](double value) -> char*
        {
            try
            {
                ...
                SpanSet* res = float_to_spanset(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterFloatToSpansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "FloatToSpansetPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return FloatToSpansetPhysicalFunction(std::move(arg0));
}
