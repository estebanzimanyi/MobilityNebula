...
Th3indexAreNeighborCellsPhysicalFunction::Th3indexAreNeighborCellsPhysicalFunction(PhysicalFunction cell1, PhysicalFunction ts1,
    ...
    const auto result = nautilus::invoke(
        +[](uint64_t cell1, uint64_t ts1, uint64_t cell2, uint64_t ts2) -> double {
            try {
                ...
                Temporal* res = th3index_are_neighbor_cells(inst1, inst2);
                free(inst1);
                free(inst2);
                if (!res) return 0.0;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return Th3indexAreNeighborCellsPhysicalFunction(std::move(arguments.childFunctions[0]),
