...
#include <Functions/Meos/GboxToStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterGboxToStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

GboxToStboxPhysicalFunction::GboxToStboxPhysicalFunction(PhysicalFunction boxFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(boxFunction));
}

VarVal GboxToStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* boxPtr, uint32_t boxSize) -> char*
        {
            try
            {
                ...
                GBOX* temp = gbox_in(tempS.c_str());
                if (!temp) return (char*) nullptr;

                STBox* res = (STBox*) gbox_to_stbox(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterGboxToStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "GboxToStboxPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return GboxToStboxPhysicalFunction(std::move(arg0));
}
