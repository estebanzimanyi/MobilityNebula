...
#include <Functions/Meos/EdwithinTrgeometryGeoPhysicalFunction.hpp>
...
EdwithinTrgeometryGeoPhysicalFunction::EdwithinTrgeometryGeoPhysicalFunction(PhysicalFunction ref_wkt, PhysicalFunction x1, PhysicalFunction y1, PhysicalFunction theta1, PhysicalFunction ts1, PhysicalFunction tgt_wkt, PhysicalFunction dist) {
    paramFns.reserve(7);
...
VarVal EdwithinTrgeometryGeoPhysicalFunction::execute(const Record& record, ArenaRef& arena) const {
    ...
    auto dist = paramFns[6].execute(record, arena).cast<nautilus::val<double>>();
    const auto result = nautilus::invoke(
        +[](const char* ref_wkt, uint32_t ref_wktsz, double x1, double y1, double theta1, uint64_t ts1, const char* tgt_wkt, uint32_t tgt_wktsz, double dist) -> double {
            try {
                ...
                int r = edwithin_trgeometry_geo((Temporal*)inst1, gs_tgt, dist);
                free(inst1); free(gs_tgt);
                return r > 0 ? 1.0 : 0.0;
            } catch (const std::exception&) { return 0.0; }
        },
        ref_wkt.getContent(), ref_wkt.getContentSize(), x1, y1, theta1, ts1, tgt_wkt.getContent(), tgt_wkt.getContentSize(), dist);
    return VarVal(result);
}

PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterEdwithinTrgeometryGeoPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size()==7,
                 "EdwithinTrgeometryGeoPhysicalFunction requires 7 children but got {}",
                 arguments.childFunctions.size());
    return EdwithinTrgeometryGeoPhysicalFunction(
