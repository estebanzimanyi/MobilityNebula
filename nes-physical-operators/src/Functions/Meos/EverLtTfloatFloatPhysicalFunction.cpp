...
#include <Functions/Meos/EverLtTfloatFloatPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterEverLtTfloatFloatPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

EverLtTfloatFloatPhysicalFunction::EverLtTfloatFloatPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal EverLtTfloatFloatPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](double valueValue,
            ...
            try
            {
                ...
                if (!temp) return 0;
                int r = ever_lt_tfloat_float(temp, scalarValue);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterEverLtTfloatFloatPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "EverLtTfloatFloatPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return EverLtTfloatFloatPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
