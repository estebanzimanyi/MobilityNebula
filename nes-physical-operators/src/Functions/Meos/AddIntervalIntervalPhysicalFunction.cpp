...
#include <Functions/Meos/AddIntervalIntervalPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterAddIntervalIntervalPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

AddIntervalIntervalPhysicalFunction::AddIntervalIntervalPhysicalFunction(PhysicalFunction litFunction,
...
VarVal AddIntervalIntervalPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Interval* temp = interval_in(tempS.c_str(), -1);
                ...
                Interval* res = add_interval_interval(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterAddIntervalIntervalPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "AddIntervalIntervalPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return AddIntervalIntervalPhysicalFunction(std::move(arg0), std::move(arg1));
}
