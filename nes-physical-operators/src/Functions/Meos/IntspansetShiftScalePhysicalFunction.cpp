...
#include <Functions/Meos/IntspansetShiftScalePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterIntspansetShiftScalePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

IntspansetShiftScalePhysicalFunction::IntspansetShiftScalePhysicalFunction(PhysicalFunction litFunction,
...
VarVal IntspansetShiftScalePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            int arg1) -> char*
        {
            try
            {
                ...
                SpanSet* res = intspanset_shift_scale(temp, arg0, arg1, true, true);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterIntspansetShiftScalePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "IntspansetShiftScalePhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return IntspansetShiftScalePhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
