...
#include <Functions/Meos/AfterSetDatePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterAfterSetDatePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

AfterSetDatePhysicalFunction::AfterSetDatePhysicalFunction(PhysicalFunction litFunction,
...
VarVal AfterSetDatePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = after_set_date(temp, arg0V);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterAfterSetDatePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "AfterSetDatePhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return AfterSetDatePhysicalFunction(std::move(arg0), std::move(arg1));
}
