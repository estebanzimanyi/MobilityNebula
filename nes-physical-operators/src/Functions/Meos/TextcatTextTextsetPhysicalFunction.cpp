...
#include <Functions/Meos/TextcatTextTextsetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTextcatTextTextsetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TextcatTextTextsetPhysicalFunction::TextcatTextTextsetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal TextcatTextTextsetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Set* arg0B = textset_in(arg0S.c_str());
                if (!arg0B) { free(temp); return (char*) nullptr; }

                Set* res = (Set*) textcat_text_textset(temp, arg0B);
                free(temp);
                free(arg0B);
                if (!res) return (char*) nullptr;
                char* outStr = textset_out(res);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTextcatTextTextsetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TextcatTextTextsetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TextcatTextTextsetPhysicalFunction(std::move(arg0), std::move(arg1));
}
