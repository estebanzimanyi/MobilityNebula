...
#include <Functions/Meos/TimestamptzToSetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTimestamptzToSetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TimestamptzToSetPhysicalFunction::TimestamptzToSetPhysicalFunction(PhysicalFunction valueFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(valueFunction));
}

VarVal TimestamptzToSetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](int64_t value) -> char*
        {
            try
            {
                ...
                Set* res = timestamptz_to_set(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTimestamptzToSetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "TimestamptzToSetPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return TimestamptzToSetPhysicalFunction(std::move(arg0));
}
