...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterGeoTransformPipelinePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

GeoTransformPipelinePhysicalFunction::GeoTransformPipelinePhysicalFunction(PhysicalFunction litFunction)
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                GSERIALIZED* res = (GSERIALIZED*) geo_transform_pipeline(temp, (char*)"+proj=pipeline +step +proj=unitconvert +xy_in=deg +xy_out=rad +step +proj=webmerc +ellps=WGS84", 3857, true);
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "GeoTransformPipelinePhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return GeoTransformPipelinePhysicalFunction(std::move(arg0));
}
