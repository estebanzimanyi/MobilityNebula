...
#include <Functions/Meos/DistanceDatesetDatesetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterDistanceDatesetDatesetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

DistanceDatesetDatesetPhysicalFunction::DistanceDatesetDatesetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal DistanceDatesetDatesetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                Set* arg0B = dateset_in(arg0S.c_str());
                if (!arg0B) { free(temp); return 0; }

                int r = distance_dateset_dateset(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterDistanceDatesetDatesetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "DistanceDatesetDatesetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return DistanceDatesetDatesetPhysicalFunction(std::move(arg0), std::move(arg1));
}
