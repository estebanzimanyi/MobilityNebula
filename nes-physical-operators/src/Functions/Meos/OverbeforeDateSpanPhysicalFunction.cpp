...
#include <Functions/Meos/OverbeforeDateSpanPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterOverbeforeDateSpanPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

OverbeforeDateSpanPhysicalFunction::OverbeforeDateSpanPhysicalFunction(PhysicalFunction litFunction,
...
VarVal OverbeforeDateSpanPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                DateADT arg0V = date_in(arg0S.c_str());

                int r = overbefore_date_span(arg0V, temp);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterOverbeforeDateSpanPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "OverbeforeDateSpanPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return OverbeforeDateSpanPhysicalFunction(std::move(arg0), std::move(arg1));
}
