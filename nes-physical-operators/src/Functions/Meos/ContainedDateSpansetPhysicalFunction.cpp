...
#include <Functions/Meos/ContainedDateSpansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterContainedDateSpansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

ContainedDateSpansetPhysicalFunction::ContainedDateSpansetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal ContainedDateSpansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = contained_date_spanset(arg0V, temp);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterContainedDateSpansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "ContainedDateSpansetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return ContainedDateSpansetPhysicalFunction(std::move(arg0), std::move(arg1));
}
