...
#include <Functions/Meos/LeftStboxStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterLeftStboxStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

LeftStboxStboxPhysicalFunction::LeftStboxStboxPhysicalFunction(PhysicalFunction boxFunction,
...
VarVal LeftStboxStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* boxPtr, uint32_t boxSize,
            ...
            try
            {
                ...
                bool r = left_stbox_stbox(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterLeftStboxStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "LeftStboxStboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return LeftStboxStboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
