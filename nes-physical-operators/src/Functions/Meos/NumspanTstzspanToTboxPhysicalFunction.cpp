...
#include <Functions/Meos/NumspanTstzspanToTboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterNumspanTstzspanToTboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

NumspanTstzspanToTboxPhysicalFunction::NumspanTstzspanToTboxPhysicalFunction(PhysicalFunction litFunction,
...
VarVal NumspanTstzspanToTboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                TBox* res = numspan_tstzspan_to_tbox(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterNumspanTstzspanToTboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "NumspanTstzspanToTboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return NumspanTstzspanToTboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
