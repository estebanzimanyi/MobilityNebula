...
#include <Functions/Meos/IntersectionSpansetSpansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterIntersectionSpansetSpansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

IntersectionSpansetSpansetPhysicalFunction::IntersectionSpansetSpansetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal IntersectionSpansetSpansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                SpanSet* arg0B = intspanset_in(arg0S.c_str());
                if (!arg0B) { free(temp); return (char*) nullptr; }

                SpanSet* res = intersection_spanset_spanset(temp, arg0B);
                free(temp);
                free(arg0B);
                if (!res) return (char*) nullptr;
                char* outStr = intspanset_out(res);
                free(res);
                return outStr;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterIntersectionSpansetSpansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "IntersectionSpansetSpansetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return IntersectionSpansetSpansetPhysicalFunction(std::move(arg0), std::move(arg1));
}
