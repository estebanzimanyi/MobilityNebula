...
#include <Functions/Meos/TcontainsTgeoTgeoPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTcontainsTgeoTgeoPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TcontainsTgeoTgeoPhysicalFunction::TcontainsTgeoTgeoPhysicalFunction(PhysicalFunction trajFunction,
...
VarVal TcontainsTgeoTgeoPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto hexStr = nautilus::invoke(
        +[](const char* aPtr, uint32_t aSize, const char* bPtr, uint32_t bSize) -> char*
        {
            try
            {
                ...
                Temporal* res = tcontains_tgeo_tgeo(a, b);
                free(a);
                free(b);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTcontainsTgeoTgeoPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TcontainsTgeoTgeoPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TcontainsTgeoTgeoPhysicalFunction(std::move(arg0), std::move(arg1));
}
