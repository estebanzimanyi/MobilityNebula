...
#include <Functions/Meos/TemparrRoundPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTemparrRoundPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TemparrRoundPhysicalFunction::TemparrRoundPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TemparrRoundPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                Temporal* _ina[1] = { (Temporal*) temp };
                Temporal** _out = (Temporal**) temparr_round((Temporal**) _ina, 1, 6);
                free(temp);
                if (!_out || !_out[0]) return (char*) nullptr;
                char* outStr = tfloat_out(_out[0], 6);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTemparrRoundPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TemparrRoundPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TemparrRoundPhysicalFunction(std::move(arg0), std::move(arg1));
}
