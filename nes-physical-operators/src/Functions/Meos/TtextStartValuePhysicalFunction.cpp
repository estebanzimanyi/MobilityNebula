...
#include <Functions/Meos/TtextStartValuePhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTtextStartValuePhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TtextStartValuePhysicalFunction::TtextStartValuePhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TtextStartValuePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            uint64_t ts) -> char*
        {
            try
            {
                ...
                text* res = ttext_start_value(temp);
                free(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTtextStartValuePhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "TtextStartValuePhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return TtextStartValuePhysicalFunction(std::move(arg0), std::move(arg1));
}
