...
#include <Functions/Meos/TnumberMinusTboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTnumberMinusTboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TnumberMinusTboxPhysicalFunction::TnumberMinusTboxPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal TnumberMinusTboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                Temporal* res = tnumber_minus_tbox(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTnumberMinusTboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "TnumberMinusTboxPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return TnumberMinusTboxPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
