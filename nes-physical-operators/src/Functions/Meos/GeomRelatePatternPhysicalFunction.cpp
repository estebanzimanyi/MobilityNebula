...
GeomRelatePatternPhysicalFunction::GeomRelatePatternPhysicalFunction(PhysicalFunction wkt1, PhysicalFunction wkt2, PhysicalFunction pattern)
{
    paramFns.reserve(3);
    paramFns.push_back(std::move(wkt1));
    paramFns.push_back(std::move(wkt2));
    paramFns.push_back(std::move(pattern));
}
...
VarVal GeomRelatePatternPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto pattern = paramFns[2].execute(record, arena).cast<VariableSizedData>();
    const auto result = nautilus::invoke(
        +[](const char* w1, uint32_t w1sz, const char* w2, uint32_t w2sz, const char* p, uint32_t psz) -> double {
            try {
                MEOS::Meos::ensureMeosInitialized();
                std::string s1(w1, w1sz), s2(w2, w2sz), sp(p, psz);
                GSERIALIZED* gs1 = geom_in(s1.c_str(), -1);
                if (!gs1) return 0.0;
                GSERIALIZED* gs2 = geom_in(s2.c_str(), -1);
                if (!gs2) { free(gs1); return 0.0; }
                bool r = geom_relate_pattern(gs1, gs2, const_cast<char*>(sp.c_str()));
                free(gs1); free(gs2);
                return r ? 1.0 : 0.0;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return GeomRelatePatternPhysicalFunction(
