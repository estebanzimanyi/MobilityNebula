...
#include <Functions/Meos/IntTstzspanToTboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterIntTstzspanToTboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

IntTstzspanToTboxPhysicalFunction::IntTstzspanToTboxPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal IntTstzspanToTboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                TBox* res = int_tstzspan_to_tbox(temp, arg0B);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterIntTstzspanToTboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "IntTstzspanToTboxPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return IntTstzspanToTboxPhysicalFunction(std::move(arg0), std::move(arg1));
}
