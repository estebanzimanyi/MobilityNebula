...
#include <Functions/Meos/GeomTouchesPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterGeomTouchesPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

GeomTouchesPhysicalFunction::GeomTouchesPhysicalFunction(PhysicalFunction litFunction,
...
VarVal GeomTouchesPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = geom_touches(temp, arg0G.getGeometry());
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterGeomTouchesPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "GeomTouchesPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return GeomTouchesPhysicalFunction(std::move(arg0), std::move(arg1));
}
