...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterGeoRoundPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

GeoRoundPhysicalFunction::GeoRoundPhysicalFunction(PhysicalFunction litFunction)
{
    ...
    auto outStr = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize) -> char*
        {
            try
            {
                ...
                GSERIALIZED* res = geo_round(temp, 2);
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "GeoRoundPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return GeoRoundPhysicalFunction(std::move(arg0));
}
