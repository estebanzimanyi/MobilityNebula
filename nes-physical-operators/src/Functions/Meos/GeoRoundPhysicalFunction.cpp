...
GeoRoundPhysicalFunction::GeoRoundPhysicalFunction(PhysicalFunction wktFunction,
    ...
    const auto actualLen = nautilus::invoke(
        +[](const char* wktPtr, uint32_t wktSize,
            ...
            try
            {
                ...
                GSERIALIZED* gres = geo_round(temp, (int)dec);
                free(temp);
                if (!gres) return 0u;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    ...
    return GeoRoundPhysicalFunction(std::move(arg0), std::move(arg1));
}
