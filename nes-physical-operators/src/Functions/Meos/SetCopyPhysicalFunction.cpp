...
#include <Functions/Meos/SetCopyPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterSetCopyPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

SetCopyPhysicalFunction::SetCopyPhysicalFunction(PhysicalFunction litFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(litFunction));
}

VarVal SetCopyPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterSetCopyPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "SetCopyPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return SetCopyPhysicalFunction(std::move(arg0));
}
