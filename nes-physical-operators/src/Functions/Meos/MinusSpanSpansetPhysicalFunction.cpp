...
#include <Functions/Meos/MinusSpanSpansetPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterMinusSpanSpansetPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

MinusSpanSpansetPhysicalFunction::MinusSpanSpansetPhysicalFunction(PhysicalFunction litFunction,
...
VarVal MinusSpanSpansetPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
            ...
            const char* arg0Ptr, uint32_t arg0Size) -> char*
        {
            try
            {
                ...
                SpanSet* res = minus_span_spanset(temp, arg0B);
                free(temp);
                free(arg0B);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterMinusSpanSpansetPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "MinusSpanSpansetPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return MinusSpanSpansetPhysicalFunction(std::move(arg0), std::move(arg1));
}
