...
#include <Functions/Meos/EverLeFloatTfloatPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterEverLeFloatTfloatPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

EverLeFloatTfloatPhysicalFunction::EverLeFloatTfloatPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal EverLeFloatTfloatPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](double valueValue,
            ...
            try
            {
                ...
                if (!temp) return 0;
                int r = ever_le_float_tfloat(scalarValue, temp);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterEverLeFloatTfloatPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "EverLeFloatTfloatPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return EverLeFloatTfloatPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
