...
#include <Functions/Meos/TimestamptzToTboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterTimestamptzToTboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

TimestamptzToTboxPhysicalFunction::TimestamptzToTboxPhysicalFunction(PhysicalFunction valueFunction)
{
    parameterFunctions.reserve(1);
    parameterFunctions.push_back(std::move(valueFunction));
}

VarVal TimestamptzToTboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    auto outStr = nautilus::invoke(
        +[](int64_t value) -> char*
        {
            try
            {
                ...
                TBox* res = timestamptz_to_tbox(temp);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterTimestamptzToTboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 1,
                 "TimestamptzToTboxPhysicalFunction requires 1 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    return TimestamptzToTboxPhysicalFunction(std::move(arg0));
}
