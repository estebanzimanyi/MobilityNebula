...
EverNeTnpointNpointPhysicalFunction::EverNeTnpointNpointPhysicalFunction(PhysicalFunction rid1, PhysicalFunction pos1, PhysicalFunction ts, PhysicalFunction rid2, PhysicalFunction pos2)
{
    paramFns.reserve(5);
    ...
    const auto result = nautilus::invoke(
        +[](uint64_t rid1, double pos1, uint64_t ts, uint64_t rid2, double pos2) -> double {
            try {
                ...
                int r = ever_ne_tnpoint_npoint(inst, np2);
                free(inst); free(np2);
                return r > 0 ? 1.0 : 0.0;
    ...
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 5,
                 "EverNeTnpointNpointPhysicalFunction requires 5 children but got {}",
                 arguments.childFunctions.size());
    return EverNeTnpointNpointPhysicalFunction(
