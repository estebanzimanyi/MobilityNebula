...
#include <Functions/Meos/AlwaysEqTjsonbTjsonbPhysicalFunction.hpp>
...
AlwaysEqTjsonbTjsonbPhysicalFunction::AlwaysEqTjsonbTjsonbPhysicalFunction(PhysicalFunction json1, PhysicalFunction ts1, PhysicalFunction json2, PhysicalFunction ts2)
{
    paramFns.reserve(4);
...
VarVal AlwaysEqTjsonbTjsonbPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* json1, uint32_t json1_len, uint64_t ts1, const char* json2, uint32_t json2_len, uint64_t ts2) -> double {
            try {
                ...
                int r = always_eq_tjsonb_tjsonb((Temporal*)inst1, (Temporal*)inst2);
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterAlwaysEqTjsonbTjsonbPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 4,
                 "AlwaysEqTjsonbTjsonbPhysicalFunction requires 4 children but got {}",
                 arguments.childFunctions.size());
    return AlwaysEqTjsonbTjsonbPhysicalFunction(
