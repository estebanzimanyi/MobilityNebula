...
#include <Functions/Meos/OverrightTspatialStboxPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterOverrightTspatialStboxPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

OverrightTspatialStboxPhysicalFunction::OverrightTspatialStboxPhysicalFunction(PhysicalFunction lonFunction,
...
VarVal OverrightTspatialStboxPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](double lon,
            ...
            try
            {
                ...
                bool r = overright_tspatial_stbox(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterOverrightTspatialStboxPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 4,
                 "OverrightTspatialStboxPhysicalFunction requires 4 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    auto arg3 = std::move(arguments.childFunctions[3]);
    return OverrightTspatialStboxPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2), std::move(arg3));
}
