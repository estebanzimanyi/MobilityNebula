...
#include <Functions/Meos/ContainsSpanSpanPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterContainsSpanSpanPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

ContainsSpanSpanPhysicalFunction::ContainsSpanSpanPhysicalFunction(PhysicalFunction litFunction,
...
VarVal ContainsSpanSpanPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](const char* litPtr, uint32_t litSize,
            ...
            try
            {
                ...
                int r = contains_span_span(temp, arg0B);
                free(temp);
                free(arg0B);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterContainsSpanSpanPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 2,
                 "ContainsSpanSpanPhysicalFunction requires 2 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    return ContainsSpanSpanPhysicalFunction(std::move(arg0), std::move(arg1));
}
