...
#include <Functions/Meos/EverGeIntTintPhysicalFunction.hpp>
...
namespace NES::PhysicalFunctionGeneratedRegistrar { PhysicalFunctionRegistryReturnType RegisterEverGeIntTintPhysicalFunction(PhysicalFunctionRegistryArguments); }

namespace NES {

EverGeIntTintPhysicalFunction::EverGeIntTintPhysicalFunction(PhysicalFunction valueFunction,
...
VarVal EverGeIntTintPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto result = nautilus::invoke(
        +[](int valueValue,
            ...
            try
            {
                ...
                if (!temp) return 0;
                int r = ever_ge_int_tint(scalarValue, temp);
                free(temp);
                return r;
            }
...
PhysicalFunctionRegistryReturnType PhysicalFunctionGeneratedRegistrar::RegisterEverGeIntTintPhysicalFunction(
    PhysicalFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.childFunctions.size() == 3,
                 "EverGeIntTintPhysicalFunction requires 3 children but got {}",
                 arguments.childFunctions.size());
    auto arg0 = std::move(arguments.childFunctions[0]);
    auto arg1 = std::move(arguments.childFunctions[1]);
    auto arg2 = std::move(arguments.childFunctions[2]);
    return EverGeIntTintPhysicalFunction(std::move(arg0), std::move(arg1), std::move(arg2));
}
