...
static std::mutex meos_temporaltfloatendvalue_mutex;


TfloatEndValueAggregationPhysicalFunction::TfloatEndValueAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TfloatEndValueAggregationPhysicalFunction::lift(
...
void TfloatEndValueAggregationPhysicalFunction::combine(
...
Nautilus::Record TfloatEndValueAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto resultValue = nautilus::invoke(
        +[](const char* seqStr) -> double
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_temporaltfloatendvalue_mutex);
            ...
            double value = tfloat_end_value(temp);
...
void TfloatEndValueAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TfloatEndValueAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TfloatEndValueAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTfloatEndValueAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TfloatEndValue aggregation cannot be created through the registry. "
