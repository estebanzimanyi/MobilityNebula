...
static std::mutex meos_tnumbertavgtransfn_mutex;


TnumberTavgTransfnAggregationPhysicalFunction::TnumberTavgTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TnumberTavgTransfnAggregationPhysicalFunction::lift(
...
void TnumberTavgTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TnumberTavgTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        skipState = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_tnumbertavgtransfn_mutex);
                ...
                SkipList* ns = tnumber_tavg_transfn(static_cast<SkipList*>(state), inst);
                free(inst);
...
void TnumberTavgTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TnumberTavgTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TnumberTavgTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTnumberTavgTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TnumberTavgTransfn aggregation cannot be created through the registry. "
