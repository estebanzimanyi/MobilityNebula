...
static std::mutex meos_temporalendsequence_mutex;


TemporalEndSequenceAggregationPhysicalFunction::TemporalEndSequenceAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TemporalEndSequenceAggregationPhysicalFunction::lift(
...
void TemporalEndSequenceAggregationPhysicalFunction::combine(
...
Nautilus::Record TemporalEndSequenceAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto boxStr = nautilus::invoke(
        +[](const char* trajStr) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_temporalendsequence_mutex);
            ...
            Temporal* seqRes = (Temporal*) temporal_end_sequence(static_cast<Temporal*>(temp));
...
void TemporalEndSequenceAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TemporalEndSequenceAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TemporalEndSequenceAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTemporalEndSequenceAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TemporalEndSequence aggregation cannot be created through the registry. "
