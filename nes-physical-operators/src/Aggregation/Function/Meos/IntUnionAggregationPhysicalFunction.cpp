...
IntUnionAggregationPhysicalFunction::IntUnionAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void IntUnionAggregationPhysicalFunction::lift(
...
void IntUnionAggregationPhysicalFunction::combine(
...
Nautilus::Record IntUnionAggregationPhysicalFunction::lower(
...
void IntUnionAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Set** slot = reinterpret_cast<Set**>(st); *slot = nullptr; }, aggregationState);
}

size_t IntUnionAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Set*);
}

void IntUnionAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Set** slot = reinterpret_cast<Set**>(st); if (*slot) { free(*slot); *slot = nullptr; } }, aggregationState);
}

AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterIntUnionAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("INT_UNION aggregation cannot be created through the registry. "
