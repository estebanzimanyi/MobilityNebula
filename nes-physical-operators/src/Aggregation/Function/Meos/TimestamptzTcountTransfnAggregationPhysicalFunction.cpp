...
static std::mutex meos_timestamptztcounttransfn_mutex;


TimestamptzTcountTransfnAggregationPhysicalFunction::TimestamptzTcountTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TimestamptzTcountTransfnAggregationPhysicalFunction::lift(
...
void TimestamptzTcountTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TimestamptzTcountTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        cstate = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_timestamptztcounttransfn_mutex);
                std::string s(valPtr, valSize);
                while (!s.empty() && (s.front()=='\'' || s.front()=='"')) s.erase(s.begin());
                while (!s.empty() && (s.back()=='\'' || s.back()=='"')) s.pop_back();
                TimestampTz c = timestamptz_in(s.c_str(), -1); SkipList* ns = timestamptz_tcount_transfn(static_cast<SkipList*>(state), c); return reinterpret_cast<void*>(ns);
            },
            cstate,
            valVar.getContent(),
...
void TimestamptzTcountTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TimestamptzTcountTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TimestamptzTcountTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTimestamptzTcountTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TimestamptzTcountTransfn aggregation cannot be created through the registry. "
