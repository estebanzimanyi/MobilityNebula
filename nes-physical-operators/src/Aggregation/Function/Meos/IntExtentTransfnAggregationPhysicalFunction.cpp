...
static std::mutex meos_intextent_mutex;


IntExtentTransfnAggregationPhysicalFunction::IntExtentTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void IntExtentTransfnAggregationPhysicalFunction::lift(
    const nautilus::val<AggregationState*>& aggregationState, PipelineMemoryProvider& pipelineMemoryProvider, const Nautilus::Record& record)
{
    ...
    nautilus::invoke(
        +[](AggregationState* st, int32_t val) -> void
        {
            ...
            *slot = int_extent_transfn(*slot, val);
        },
        aggregationState, value);
}

void IntExtentTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record IntExtentTransfnAggregationPhysicalFunction::lower(
...
void IntExtentTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Span** slot = reinterpret_cast<Span**>(st); *slot = nullptr; }, aggregationState);
}

size_t IntExtentTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Span*);
}

void IntExtentTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Span** slot = reinterpret_cast<Span**>(st); if (*slot) { free(*slot); *slot = nullptr; } }, aggregationState);
}

AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterIntExtentTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("INT_EXTENT_TRANSFN aggregation cannot be created through the registry. "
