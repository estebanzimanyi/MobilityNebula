...
TimestamptzExtentTransfnAggregationPhysicalFunction::TimestamptzExtentTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TimestamptzExtentTransfnAggregationPhysicalFunction::lift(
    const nautilus::val<AggregationState*>& aggregationState, PipelineMemoryProvider& pipelineMemoryProvider, const Nautilus::Record& record)
{
    ...
    nautilus::invoke(
        +[](AggregationState* st, int64_t val) -> void
        {
            ...
            *slot = timestamptz_extent_transfn(*slot, tstz);
        },
        aggregationState, value);
}

void TimestamptzExtentTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TimestamptzExtentTransfnAggregationPhysicalFunction::lower(
...
void TimestamptzExtentTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Span** slot = reinterpret_cast<Span**>(st); *slot = nullptr; }, aggregationState);
}

size_t TimestamptzExtentTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Span*);
}

void TimestamptzExtentTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Span** slot = reinterpret_cast<Span**>(st); if (*slot) { free(*slot); *slot = nullptr; } }, aggregationState);
}

AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTimestamptzExtentTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TIMESTAMPTZ_EXTENT_TRANSFN aggregation cannot be created through the registry. "
