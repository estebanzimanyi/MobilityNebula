...
static std::mutex meos_npointsetmake_mutex;


NpointsetMakeAggregationPhysicalFunction::NpointsetMakeAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void NpointsetMakeAggregationPhysicalFunction::lift(
...
void NpointsetMakeAggregationPhysicalFunction::combine(
...
Nautilus::Record NpointsetMakeAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    // via npoint_in into a heap pointer, store it in a Npoint*[] array, then
    // build the result set via npointset_make and free each element + the array.
    auto arrState = nautilus::invoke(
        +[](size_t n) -> void* { return malloc(sizeof(Npoint*) * (n > 0 ? n : 1)); },
        numberOfEntries);
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        arrIdx = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_npointsetmake_mutex);
                std::string s(vp, vs);
                while (!s.empty() && (s.front()=='\'' || s.front()=='"')) s.erase(s.begin());
                while (!s.empty() && (s.back()=='\'' || s.back()=='"')) s.pop_back();
                Npoint* e = npoint_in(s.c_str());
                if (e) { ((Npoint**) a)[i] = e; return i + 1; }
                return i;
            },
            arrState,
            arrIdx,
    ...
    auto boxStr = nautilus::invoke(
        +[](void* a, int64_t n) -> char*
        {
            ...
            Set* s = npointset_make((Npoint **) a, (int) n);
            for (int64_t i = 0; i < n; i++) { free(((Npoint**) a)[i]); }
            free(a);
            if (!s) { return (char*)nullptr; }
            char* out = npointset_out(s, 6);
            free(s);
            return out;
        },
        arrState,
...
void NpointsetMakeAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t NpointsetMakeAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void NpointsetMakeAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterNpointsetMakeAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("NpointsetMake aggregation cannot be created through the registry. "
