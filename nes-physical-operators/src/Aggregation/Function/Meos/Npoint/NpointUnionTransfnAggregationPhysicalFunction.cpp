...
static std::mutex meos_npointuniontransfn_mutex;


NpointUnionTransfnAggregationPhysicalFunction::NpointUnionTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void NpointUnionTransfnAggregationPhysicalFunction::lift(
...
void NpointUnionTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record NpointUnionTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        cstate = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_npointuniontransfn_mutex);
                std::string s(valPtr, valSize);
                while (!s.empty() && (s.front()=='\'' || s.front()=='"')) s.erase(s.begin());
                while (!s.empty() && (s.back()=='\'' || s.back()=='"')) s.pop_back();
                Npoint* c = npoint_in(s.c_str()); if (!c) return state; Set* ns = npoint_union_transfn(static_cast<Set*>(state), c); free(c); return reinterpret_cast<void*>(ns);
            },
            cstate,
            valVar.getContent(),
    ...
    auto boxStr = nautilus::invoke(
        +[](void* state) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_npointuniontransfn_mutex);
            Set* fin = set_union_finalfn(static_cast<Set*>(state)); if (!fin) return (char*)nullptr; char* out = npointset_out(fin, 15); free(fin); return out;
        },
        cstate);
...
void NpointUnionTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t NpointUnionTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void NpointUnionTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterNpointUnionTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("NpointUnionTransfn aggregation cannot be created through the registry. "
