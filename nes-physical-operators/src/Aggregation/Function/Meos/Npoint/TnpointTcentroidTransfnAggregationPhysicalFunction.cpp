...
static std::mutex meos_tnpointtcentroidtransfn_mutex;


TnpointTcentroidTransfnAggregationPhysicalFunction::TnpointTcentroidTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TnpointTcentroidTransfnAggregationPhysicalFunction::lift(
...
void TnpointTcentroidTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TnpointTcentroidTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        cstate = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_tnpointtcentroidtransfn_mutex);
                std::string s(valPtr, valSize);
                while (!s.empty() && (s.front()=='\'' || s.front()=='"')) s.erase(s.begin());
                while (!s.empty() && (s.back()=='\'' || s.back()=='"')) s.pop_back();
                Temporal* c = tnpoint_in(s.c_str()); if (!c) return state; SkipList* ns = tnpoint_tcentroid_transfn(static_cast<SkipList*>(state), c); free(c); return reinterpret_cast<void*>(ns);
            },
            cstate,
            valVar.getContent(),
    ...
    auto boxStr = nautilus::invoke(
        +[](void* state) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_tnpointtcentroidtransfn_mutex);
            Temporal* res = tpoint_tcentroid_finalfn(static_cast<SkipList*>(state)); if (!res) return (char*)nullptr; size_t sz = 0; char* h = temporal_as_hexwkb(res, 0x04 /* WKB_EXTENDED */, &sz); free(res); return h;
        },
        cstate);
...
void TnpointTcentroidTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TnpointTcentroidTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TnpointTcentroidTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTnpointTcentroidTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TnpointTcentroidTransfn aggregation cannot be created through the registry. "
