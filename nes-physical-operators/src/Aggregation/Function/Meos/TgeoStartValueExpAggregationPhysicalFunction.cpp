...
static std::mutex meos_tgeostartvalueexp_mutex;


TgeoStartValueExpAggregationPhysicalFunction::TgeoStartValueExpAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TgeoStartValueExpAggregationPhysicalFunction::lift(
    const nautilus::val<AggregationState*>& aggregationState, PipelineMemoryProvider& pipelineMemoryProvider, const Nautilus::Record& record)
{
    ...
    nautilus::invoke(
        +[](AggregationState* st, double lonVal, double latVal, int64_t tsVal) -> void
        {
            ...
            std::lock_guard<std::mutex> lock(meos_tgeostartvalueexp_mutex);
...
void TgeoStartValueExpAggregationPhysicalFunction::combine(
...
Nautilus::Record TgeoStartValueExpAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto hexStr = nautilus::invoke(
        +[](AggregationState* st) -> char*
        {
            ...
            GSERIALIZED* res = tgeo_start_value(*slot);
            if (!res) {
                return (char*)nullptr;
            }
...
void TgeoStartValueExpAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TgeoStartValueExpAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Temporal*);
}

void TgeoStartValueExpAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* st) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTgeoStartValueExpAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TgeoStartValueExp aggregation cannot be created through the registry. "
