...
TimestamptzUnionAggregationPhysicalFunction::TimestamptzUnionAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TimestamptzUnionAggregationPhysicalFunction::lift(
...
void TimestamptzUnionAggregationPhysicalFunction::combine(
...
Nautilus::Record TimestamptzUnionAggregationPhysicalFunction::lower(
...
void TimestamptzUnionAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Set** slot = reinterpret_cast<Set**>(st); *slot = nullptr; }, aggregationState);
}

size_t TimestamptzUnionAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Set*);
}

void TimestamptzUnionAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Set** slot = reinterpret_cast<Set**>(st); if (*slot) { free(*slot); *slot = nullptr; } }, aggregationState);
}

AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTimestamptzUnionAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TIMESTAMPTZ_UNION aggregation cannot be created through the registry. "
