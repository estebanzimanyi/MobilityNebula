...
static std::mutex meos_tnumberangulardifferenceexp_mutex;


TnumberAngularDifferenceExpAggregationPhysicalFunction::TnumberAngularDifferenceExpAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TnumberAngularDifferenceExpAggregationPhysicalFunction::lift(
    const nautilus::val<AggregationState*>& aggregationState, PipelineMemoryProvider& pipelineMemoryProvider, const Nautilus::Record& record)
{
    ...
    nautilus::invoke(
        +[](AggregationState* st, double valueVal, int64_t tsVal) -> void
        {
            ...
            std::lock_guard<std::mutex> lock(meos_tnumberangulardifferenceexp_mutex);
...
void TnumberAngularDifferenceExpAggregationPhysicalFunction::combine(
...
Nautilus::Record TnumberAngularDifferenceExpAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto hexStr = nautilus::invoke(
        +[](AggregationState* st) -> char*
        {
            ...
            Temporal* res = tnumber_angular_difference(*slot);
            if (!res) {
                return (char*)nullptr;
            }
            size_t hexSize = 0;
...
void TnumberAngularDifferenceExpAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TnumberAngularDifferenceExpAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Temporal*);
}

void TnumberAngularDifferenceExpAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* st) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTnumberAngularDifferenceExpAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TNUMBER_ANGULAR_DIFFERENCE_EXP aggregation cannot be created through the registry. "
