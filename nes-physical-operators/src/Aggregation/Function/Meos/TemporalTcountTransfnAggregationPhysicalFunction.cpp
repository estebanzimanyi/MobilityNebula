...
static std::mutex meos_temporaltcounttransfn_mutex;


TemporalTcountTransfnAggregationPhysicalFunction::TemporalTcountTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TemporalTcountTransfnAggregationPhysicalFunction::lift(
...
void TemporalTcountTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TemporalTcountTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        skipState = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_temporaltcounttransfn_mutex);
                ...
                SkipList* ns = temporal_tcount_transfn(static_cast<SkipList*>(state), inst);
                free(inst);
...
void TemporalTcountTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TemporalTcountTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TemporalTcountTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTemporalTcountTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TemporalTcountTransfn aggregation cannot be created through the registry. "
