...
static std::mutex meos_temporaltsample_mutex;


TemporalTsampleAggregationPhysicalFunction::TemporalTsampleAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TemporalTsampleAggregationPhysicalFunction::lift(
...
void TemporalTsampleAggregationPhysicalFunction::combine(
...
Nautilus::Record TemporalTsampleAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto boxStr = nautilus::invoke(
        +[](const char* trajStr, const char* constArg0, const char* constArg1, const char* constArg2) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_temporaltsample_mutex);
            ...
            TimestampTz c1 = timestamptz_in(constArg1, -1);
            interpType c2 = (interpType) atoi(constArg2);

            Temporal* res = (Temporal*) temporal_tsample(static_cast<Temporal*>(temp), c0, c1, c2);
...
void TemporalTsampleAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TemporalTsampleAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TemporalTsampleAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTemporalTsampleAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TemporalTsample aggregation cannot be created through the registry. "
