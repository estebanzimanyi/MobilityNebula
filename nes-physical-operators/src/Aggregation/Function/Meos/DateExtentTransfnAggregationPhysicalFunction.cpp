...
static std::mutex meos_dateextenttransfn_mutex;


DateExtentTransfnAggregationPhysicalFunction::DateExtentTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void DateExtentTransfnAggregationPhysicalFunction::lift(
...
void DateExtentTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record DateExtentTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        spanState = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_dateextenttransfn_mutex);
                Span* ns = date_extent_transfn(static_cast<Span*>(state), val); return reinterpret_cast<void*>(ns);
            },
            spanState,
            value);
    }
...
void DateExtentTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t DateExtentTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void DateExtentTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterDateExtentTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("DateExtentTransfn aggregation cannot be created through the registry. "
