...
static std::mutex meos_temporalextenttransfn_mutex;


TemporalExtentTransfnAggregationPhysicalFunction::TemporalExtentTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TemporalExtentTransfnAggregationPhysicalFunction::lift(
...
void TemporalExtentTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TemporalExtentTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        cstate = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_temporalextenttransfn_mutex);
                std::string s(valPtr, valSize);
                while (!s.empty() && (s.front()=='\'' || s.front()=='"')) s.erase(s.begin());
                while (!s.empty() && (s.back()=='\'' || s.back()=='"')) s.pop_back();
                Temporal* c = tfloat_in(s.c_str()); if (!c) return state; Span* ns = temporal_extent_transfn(static_cast<Span*>(state), c);  return reinterpret_cast<void*>(ns);
            },
            cstate,
            valVar.getContent(),
    ...
    auto boxStr = nautilus::invoke(
        +[](void* state) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_temporalextenttransfn_mutex);
            Span* fin = static_cast<Span*>(state); char* out = tstzspan_out(fin); free(fin); return out;
        },
        cstate);
...
void TemporalExtentTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TemporalExtentTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TemporalExtentTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTemporalExtentTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TemporalExtentTransfn aggregation cannot be created through the registry. "
