...
static std::mutex meos_tstzsettcounttransfn_mutex;


TstzsetTcountTransfnAggregationPhysicalFunction::TstzsetTcountTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TstzsetTcountTransfnAggregationPhysicalFunction::lift(
...
void TstzsetTcountTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TstzsetTcountTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        cstate = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_tstzsettcounttransfn_mutex);
                std::string s(valPtr, valSize);
                while (!s.empty() && (s.front()=='\'' || s.front()=='"')) s.erase(s.begin());
                while (!s.empty() && (s.back()=='\'' || s.back()=='"')) s.pop_back();
                Set* c = tstzset_in(s.c_str()); if (!c) return state; SkipList* ns = tstzset_tcount_transfn(static_cast<SkipList*>(state), c); free(c); return reinterpret_cast<void*>(ns);
            },
            cstate,
            valVar.getContent(),
...
void TstzsetTcountTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TstzsetTcountTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TstzsetTcountTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTstzsetTcountTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TstzsetTcountTransfn aggregation cannot be created through the registry. "
