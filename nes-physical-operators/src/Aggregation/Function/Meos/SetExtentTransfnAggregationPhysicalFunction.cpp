...
static std::mutex meos_setextenttransfn_mutex;


SetExtentTransfnAggregationPhysicalFunction::SetExtentTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void SetExtentTransfnAggregationPhysicalFunction::lift(
...
void SetExtentTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record SetExtentTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        cstate = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_setextenttransfn_mutex);
                std::string s(valPtr, valSize);
                while (!s.empty() && (s.front()=='\'' || s.front()=='"')) s.erase(s.begin());
                while (!s.empty() && (s.back()=='\'' || s.back()=='"')) s.pop_back();
                Set* c = floatset_in(s.c_str()); if (!c) return state; Span* ns = set_extent_transfn(static_cast<Span*>(state), c); free(c); return reinterpret_cast<void*>(ns);
            },
            cstate,
            valVar.getContent(),
...
void SetExtentTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t SetExtentTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void SetExtentTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterSetExtentTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("SetExtentTransfn aggregation cannot be created through the registry. "
