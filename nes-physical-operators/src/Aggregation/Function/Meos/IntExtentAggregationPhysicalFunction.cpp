...
IntExtentAggregationPhysicalFunction::IntExtentAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void IntExtentAggregationPhysicalFunction::lift(
...
void IntExtentAggregationPhysicalFunction::combine(
...
Nautilus::Record IntExtentAggregationPhysicalFunction::lower(
...
void IntExtentAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Span** slot = reinterpret_cast<Span**>(st); *slot = nullptr; }, aggregationState);
}

size_t IntExtentAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Span*);
}

void IntExtentAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Span** slot = reinterpret_cast<Span**>(st); if (*slot) { free(*slot); *slot = nullptr; } }, aggregationState);
}

AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterIntExtentAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("INT_EXTENT aggregation cannot be created through the registry. "
