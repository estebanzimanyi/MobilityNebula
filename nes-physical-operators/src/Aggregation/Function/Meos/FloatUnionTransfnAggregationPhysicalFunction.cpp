...
static std::mutex meos_floatunion_mutex;


FloatUnionTransfnAggregationPhysicalFunction::FloatUnionTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void FloatUnionTransfnAggregationPhysicalFunction::lift(
...
void FloatUnionTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record FloatUnionTransfnAggregationPhysicalFunction::lower(
...
void FloatUnionTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Set** slot = reinterpret_cast<Set**>(st); *slot = nullptr; }, aggregationState);
}

size_t FloatUnionTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Set*);
}

void FloatUnionTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Set** slot = reinterpret_cast<Set**>(st); if (*slot) { free(*slot); *slot = nullptr; } }, aggregationState);
}

AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterFloatUnionTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("FLOAT_UNION_TRANSFN aggregation cannot be created through the registry. "
