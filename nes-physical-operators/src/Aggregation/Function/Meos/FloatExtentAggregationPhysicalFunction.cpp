...
FloatExtentAggregationPhysicalFunction::FloatExtentAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void FloatExtentAggregationPhysicalFunction::lift(
...
void FloatExtentAggregationPhysicalFunction::combine(
...
Nautilus::Record FloatExtentAggregationPhysicalFunction::lower(
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        spanState = nautilus::invoke(
            +[](void* state, double val) -> void*
            {
                ...
                return (void*) float_extent_transfn(static_cast<Span*>(state), val);
            },
            spanState,
            value);
    }
...
void FloatExtentAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t FloatExtentAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void FloatExtentAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterFloatExtentAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("FLOAT_EXTENT aggregation cannot be created through the registry. "
