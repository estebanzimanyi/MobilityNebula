...
static std::mutex meos_ttexttmintransfn_mutex;


TtextTminTransfnAggregationPhysicalFunction::TtextTminTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TtextTminTransfnAggregationPhysicalFunction::lift(
...
void TtextTminTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TtextTminTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        skipState = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_ttexttmintransfn_mutex);
                ...
                SkipList* ns = ttext_tmin_transfn(static_cast<SkipList*>(state), inst);
                free(inst);
...
void TtextTminTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TtextTminTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TtextTminTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTtextTminTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TtextTminTransfn aggregation cannot be created through the registry. "
