...
static std::mutex meos_intsetmake_mutex;


IntsetMakeAggregationPhysicalFunction::IntsetMakeAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void IntsetMakeAggregationPhysicalFunction::lift(
...
void IntsetMakeAggregationPhysicalFunction::combine(
...
Nautilus::Record IntsetMakeAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    // via the MEOS array constructor intset_make. The array pointer threads across
    // events as an opaque void* (Nautilus invoke ABI), sized to numberOfEntries.
    auto arrState = nautilus::invoke(
        +[](size_t n) -> void* { return malloc(sizeof(int) * (n > 0 ? n : 1)); },
        numberOfEntries);
    ...
    auto boxStr = nautilus::invoke(
        ...
        {
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_intsetmake_mutex);
            Set* s = intset_make((int*) a, (int) n);
            free(a);
            if (!s) {
                return (char*)nullptr;
            }
            char* out = intset_out(s);
            free(s);
            return out;
        },
        arrState,
...
void IntsetMakeAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t IntsetMakeAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void IntsetMakeAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterIntsetMakeAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("IntsetMake aggregation cannot be created through the registry. "
