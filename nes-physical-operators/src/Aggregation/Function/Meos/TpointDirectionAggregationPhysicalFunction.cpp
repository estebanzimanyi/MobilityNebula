...
static std::mutex meos_tpointdirection_mutex;


TpointDirectionAggregationPhysicalFunction::TpointDirectionAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TpointDirectionAggregationPhysicalFunction::lift(
...
void TpointDirectionAggregationPhysicalFunction::combine(
...
Nautilus::Record TpointDirectionAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto resultValue = nautilus::invoke(
        +[](const char* trajStr) -> double
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_tpointdirection_mutex);
            ...
            tpoint_direction(static_cast<Temporal*>(temp), &value);
...
void TpointDirectionAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TpointDirectionAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TpointDirectionAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTpointDirectionAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TpointDirection aggregation cannot be created through the registry. "
