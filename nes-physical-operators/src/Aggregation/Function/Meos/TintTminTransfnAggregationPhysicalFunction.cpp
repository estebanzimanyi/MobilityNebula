...
static std::mutex meos_tinttmintransfn_mutex;


TintTminTransfnAggregationPhysicalFunction::TintTminTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TintTminTransfnAggregationPhysicalFunction::lift(
...
void TintTminTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TintTminTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        skipState = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_tinttmintransfn_mutex);
                ...
                SkipList* ns = tint_tmin_transfn(static_cast<SkipList*>(state), inst);
                free(inst);
...
void TintTminTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TintTminTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TintTminTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTintTminTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TintTminTransfn aggregation cannot be created through the registry. "
