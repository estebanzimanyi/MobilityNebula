...
static std::mutex meos_tintwmaxtransfn_mutex;


TintWmaxTransfnAggregationPhysicalFunction::TintWmaxTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TintWmaxTransfnAggregationPhysicalFunction::lift(
...
void TintWmaxTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TintWmaxTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        skipState = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_tintwmaxtransfn_mutex);
                ...
                Interval* iv = interval_in("1 hour", -1); if (!iv) { free(inst); return state; } SkipList* ns = tint_wmax_transfn(static_cast<SkipList*>(state), inst, iv);
                free(iv); free(inst);
...
void TintWmaxTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TintWmaxTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TintWmaxTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTintWmaxTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TintWmaxTransfn aggregation cannot be created through the registry. "
