...
static std::mutex meos_tbooltortransfn_mutex;


TboolTorTransfnAggregationPhysicalFunction::TboolTorTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TboolTorTransfnAggregationPhysicalFunction::lift(
...
void TboolTorTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TboolTorTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        skipState = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_tbooltortransfn_mutex);
                ...
                SkipList* ns = tbool_tor_transfn(static_cast<SkipList*>(state), inst);
                free(inst);
...
void TboolTorTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TboolTorTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TboolTorTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTboolTorTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TboolTorTransfn aggregation cannot be created through the registry. "
