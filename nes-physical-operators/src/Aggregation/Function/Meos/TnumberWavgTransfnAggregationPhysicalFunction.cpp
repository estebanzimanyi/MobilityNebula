...
static std::mutex meos_tnumberwavgtransfn_mutex;


TnumberWavgTransfnAggregationPhysicalFunction::TnumberWavgTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TnumberWavgTransfnAggregationPhysicalFunction::lift(
...
void TnumberWavgTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TnumberWavgTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        skipState = nautilus::invoke(
            +[](void* state, double valueVal, int64_t tsVal) -> void*
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_tnumberwavgtransfn_mutex);
                ...
                char itemStr[80];
                sprintf(itemStr, "%.6f@%s", valueVal, tsS.c_str());
                Temporal* inst = tfloat_in(itemStr);
                if (!inst) { return state; }
                Interval* iv = interval_in("1 hour", -1); if (!iv) { free(inst); return state; } SkipList* ns = tnumber_wavg_transfn(static_cast<SkipList*>(state), inst, iv);
                free(iv); free(inst);
    ...
    auto boxStr = nautilus::invoke(
        +[](void* state) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_tnumberwavgtransfn_mutex);
            Temporal* res = tnumber_tavg_finalfn(static_cast<SkipList*>(state));
            if (!res) {
                return (char*)nullptr;
            }
            size_t hexSize = 0;
...
void TnumberWavgTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TnumberWavgTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TnumberWavgTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTnumberWavgTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TnumberWavgTransfn aggregation cannot be created through the registry. "
