...
static std::mutex meos_temporalminusminexp_mutex;


TemporalMinusMinAggregationPhysicalFunction::TemporalMinusMinAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TemporalMinusMinAggregationPhysicalFunction::lift(
    const nautilus::val<AggregationState*>& aggregationState, PipelineMemoryProvider& pipelineMemoryProvider, const Nautilus::Record& record)
{
    ...
    nautilus::invoke(
        +[](AggregationState* st, double valueVal, int64_t tsVal) -> void
        {
            ...
            std::lock_guard<std::mutex> lock(meos_temporalminusminexp_mutex);
...
void TemporalMinusMinAggregationPhysicalFunction::combine(
...
Nautilus::Record TemporalMinusMinAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto hexStr = nautilus::invoke(
        +[](AggregationState* st) -> char*
        {
            ...
            Temporal* res = temporal_minus_min(*slot);
            if (!res) {
                return (char*)nullptr;
            }
            size_t hexSize = 0;
...
void TemporalMinusMinAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TemporalMinusMinAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Temporal*);
}

void TemporalMinusMinAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* st) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTemporalMinusMinAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TEMPORAL_MINUS_MIN aggregation cannot be created through the registry. "
