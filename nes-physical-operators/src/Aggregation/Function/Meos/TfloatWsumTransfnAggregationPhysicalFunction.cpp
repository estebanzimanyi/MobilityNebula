...
static std::mutex meos_tfloatwsumtransfn_mutex;


TfloatWsumTransfnAggregationPhysicalFunction::TfloatWsumTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TfloatWsumTransfnAggregationPhysicalFunction::lift(
...
void TfloatWsumTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TfloatWsumTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        skipState = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_tfloatwsumtransfn_mutex);
                ...
                Interval* iv = interval_in("1 hour", -1); if (!iv) { free(inst); return state; } SkipList* ns = tfloat_wsum_transfn(static_cast<SkipList*>(state), inst, iv);
                free(iv); free(inst);
...
void TfloatWsumTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TfloatWsumTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TfloatWsumTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTfloatWsumTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TfloatWsumTransfn aggregation cannot be created through the registry. "
