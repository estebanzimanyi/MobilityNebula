...
static std::mutex meos_dateuniontransfn_mutex;


DateUnionTransfnAggregationPhysicalFunction::DateUnionTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void DateUnionTransfnAggregationPhysicalFunction::lift(
...
void DateUnionTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record DateUnionTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        spanState = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_dateuniontransfn_mutex);
                Set* ns = date_union_transfn(static_cast<Set*>(state), val); return reinterpret_cast<void*>(ns);
            },
            spanState,
            value);
    }
...
void DateUnionTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t DateUnionTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void DateUnionTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterDateUnionTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("DateUnionTransfn aggregation cannot be created through the registry. "
