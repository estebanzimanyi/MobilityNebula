...
static std::mutex meos_tfloatwmaxtransfn_mutex;


TfloatWmaxTransfnAggregationPhysicalFunction::TfloatWmaxTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TfloatWmaxTransfnAggregationPhysicalFunction::lift(
...
void TfloatWmaxTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TfloatWmaxTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        skipState = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_tfloatwmaxtransfn_mutex);
                ...
                Interval* iv = interval_in("1 hour", -1); if (!iv) { free(inst); return state; } SkipList* ns = tfloat_wmax_transfn(static_cast<SkipList*>(state), inst, iv);
                free(iv); free(inst);
...
void TfloatWmaxTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TfloatWmaxTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TfloatWmaxTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTfloatWmaxTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TfloatWmaxTransfn aggregation cannot be created through the registry. "
