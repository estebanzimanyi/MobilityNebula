...
static std::mutex meos_spansetmake_mutex;


SpansetMakeAggregationPhysicalFunction::SpansetMakeAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void SpansetMakeAggregationPhysicalFunction::lift(
...
void SpansetMakeAggregationPhysicalFunction::combine(
...
Nautilus::Record SpansetMakeAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    // Collect the windowed span literals into a CONTIGUOUS Span value array:
    // parse each via intspan_in into a heap Span*, memcpy it into arr[i], and
    // free the parsed heap span. At finalize build the result via spanset_make over
    // the value array, serialize via intspanset_out(s), then free the array + result.
    auto arrState = nautilus::invoke(
        +[](size_t n) -> void* { return malloc(sizeof(Span) * (n > 0 ? n : 1)); },
        numberOfEntries);
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        arrIdx = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_spansetmake_mutex);
                std::string s(vp, vs);
                while (!s.empty() && (s.front()=='\'' || s.front()=='"')) s.erase(s.begin());
                while (!s.empty() && (s.back()=='\'' || s.back()=='"')) s.pop_back();
                Span* e = intspan_in(s.c_str());
                if (e) { ((Span*) a)[i] = *e; free(e); return i + 1; }
                return i;
            },
            arrState,
            arrIdx,
    ...
    auto boxStr = nautilus::invoke(
        +[](void* a, int64_t n) -> char*
        {
            ...
            SpanSet* s = spanset_make((Span*) a, (int) n);
            free(a);
            if (!s) { return (char*)nullptr; }
            char* out = intspanset_out(s);
            free(s);
            return out;
        },
        arrState,
...
void SpansetMakeAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t SpansetMakeAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void SpansetMakeAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterSpansetMakeAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("SpansetMake aggregation cannot be created through the registry. "
