...
static std::mutex meos_tintwmintransfn_mutex;


TintWminTransfnAggregationPhysicalFunction::TintWminTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TintWminTransfnAggregationPhysicalFunction::lift(
...
void TintWminTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TintWminTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        skipState = nautilus::invoke(
            +[](void* state, int32_t valueVal, int64_t tsVal) -> void*
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_tintwmintransfn_mutex);
                ...
                char itemStr[80];
                sprintf(itemStr, "%d@%s", valueVal, tsS.c_str());
                Temporal* inst = tint_in(itemStr);
                if (!inst) { return state; }
                Interval* iv = interval_in("1 hour", -1); if (!iv) { free(inst); return state; } SkipList* ns = tint_wmin_transfn(static_cast<SkipList*>(state), inst, iv);
                free(iv); free(inst);
...
void TintWminTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TintWminTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TintWminTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTintWminTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TintWminTransfn aggregation cannot be created through the registry. "
