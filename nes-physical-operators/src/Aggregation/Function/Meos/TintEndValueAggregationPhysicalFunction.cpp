...
static std::mutex meos_temporaltintendvalue_mutex;


TintEndValueAggregationPhysicalFunction::TintEndValueAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TintEndValueAggregationPhysicalFunction::lift(
...
void TintEndValueAggregationPhysicalFunction::combine(
...
Nautilus::Record TintEndValueAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto resultValue = nautilus::invoke(
        +[](const char* seqStr) -> int
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_temporaltintendvalue_mutex);
...
void TintEndValueAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TintEndValueAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TintEndValueAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTintEndValueAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TintEndValue aggregation cannot be created through the registry. "
