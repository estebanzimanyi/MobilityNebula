...
static std::mutex meos_tgeompointtotgeometryexp_mutex;


TgeompointToTgeometryExpAggregationPhysicalFunction::TgeompointToTgeometryExpAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TgeompointToTgeometryExpAggregationPhysicalFunction::lift(
    const nautilus::val<AggregationState*>& aggregationState, PipelineMemoryProvider& pipelineMemoryProvider, const Nautilus::Record& record)
{
    ...
    nautilus::invoke(
        +[](AggregationState* st, double lonVal, double latVal, int64_t tsVal) -> void
        {
            ...
            std::lock_guard<std::mutex> lock(meos_tgeompointtotgeometryexp_mutex);
...
void TgeompointToTgeometryExpAggregationPhysicalFunction::combine(
...
Nautilus::Record TgeompointToTgeometryExpAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto hexStr = nautilus::invoke(
        +[](AggregationState* st) -> char*
        {
            ...
            Temporal* res = tgeompoint_to_tgeometry(*slot);
            if (!res) {
                return (char*)nullptr;
            }
            size_t hexSize = 0;
...
void TgeompointToTgeometryExpAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TgeompointToTgeometryExpAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Temporal*);
}

void TgeompointToTgeometryExpAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* st) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTgeompointToTgeometryExpAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TGEOMPOINT_TO_TGEOMETRY_EXP aggregation cannot be created through the registry. "
