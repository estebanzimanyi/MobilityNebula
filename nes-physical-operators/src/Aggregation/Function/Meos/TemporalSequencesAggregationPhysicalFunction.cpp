...
static std::mutex meos_temporalsequences_mutex;


TemporalSequencesAggregationPhysicalFunction::TemporalSequencesAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TemporalSequencesAggregationPhysicalFunction::lift(
...
void TemporalSequencesAggregationPhysicalFunction::combine(
...
Nautilus::Record TemporalSequencesAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto boxStr = nautilus::invoke(
        +[](const char* trajStr) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_temporalsequences_mutex);
            ...
            int _cnt = 0;
            void** arr = (void**) temporal_sequences(static_cast<Temporal*>(temp), &_cnt);
...
void TemporalSequencesAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TemporalSequencesAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TemporalSequencesAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTemporalSequencesAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TemporalSequences aggregation cannot be created through the registry. "
