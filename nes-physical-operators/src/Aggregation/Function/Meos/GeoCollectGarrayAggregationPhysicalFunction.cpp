...
static std::mutex meos_geocollectgarray_mutex;


GeoCollectGarrayAggregationPhysicalFunction::GeoCollectGarrayAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void GeoCollectGarrayAggregationPhysicalFunction::lift(
...
void GeoCollectGarrayAggregationPhysicalFunction::combine(
...
Nautilus::Record GeoCollectGarrayAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    // build the result set via geo_collect_garray and free each element + the array.
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        arrIdx = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_geocollectgarray_mutex);
                std::string s(vp, vs);
    ...
    auto boxStr = nautilus::invoke(
        +[](void* a, int64_t n) -> char*
        {
            ...
            GSERIALIZED* s = geo_collect_garray((GSERIALIZED **) a, (int) n);
...
void GeoCollectGarrayAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t GeoCollectGarrayAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void GeoCollectGarrayAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterGeoCollectGarrayAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("GeoCollectGarray aggregation cannot be created through the registry. "
