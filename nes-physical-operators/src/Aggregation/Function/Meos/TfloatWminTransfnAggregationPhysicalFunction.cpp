...
static std::mutex meos_tfloatwmintransfn_mutex;


TfloatWminTransfnAggregationPhysicalFunction::TfloatWminTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TfloatWminTransfnAggregationPhysicalFunction::lift(
...
void TfloatWminTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TfloatWminTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        skipState = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_tfloatwmintransfn_mutex);
                ...
                Interval* iv = interval_in("1 hour", -1); if (!iv) { free(inst); return state; } SkipList* ns = tfloat_wmin_transfn(static_cast<SkipList*>(state), inst, iv);
                free(iv); free(inst);
...
void TfloatWminTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TfloatWminTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TfloatWminTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTfloatWminTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TfloatWminTransfn aggregation cannot be created through the registry. "
