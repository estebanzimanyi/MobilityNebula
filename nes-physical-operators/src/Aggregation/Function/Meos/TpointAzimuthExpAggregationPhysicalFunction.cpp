...
static std::mutex meos_tpointazimuthexp_mutex;


TpointAzimuthExpAggregationPhysicalFunction::TpointAzimuthExpAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TpointAzimuthExpAggregationPhysicalFunction::lift(
...
void TpointAzimuthExpAggregationPhysicalFunction::combine(
...
Nautilus::Record TpointAzimuthExpAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto hexStr = nautilus::invoke(
        +[](AggregationState* st) -> char*
        {
            ...
            Temporal* res = tpoint_azimuth(*slot);
            if (!res) {
                return (char*)nullptr;
            }
            size_t hexSize = 0;
...
void TpointAzimuthExpAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TpointAzimuthExpAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Temporal*);
}

void TpointAzimuthExpAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* st) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTpointAzimuthExpAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TPOINT_AZIMUTH_EXP aggregation cannot be created through the registry. "
