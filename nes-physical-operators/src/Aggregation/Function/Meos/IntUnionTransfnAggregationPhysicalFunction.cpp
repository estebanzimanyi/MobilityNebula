...
static std::mutex meos_intunion_mutex;


IntUnionTransfnAggregationPhysicalFunction::IntUnionTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void IntUnionTransfnAggregationPhysicalFunction::lift(
    const nautilus::val<AggregationState*>& aggregationState, PipelineMemoryProvider& pipelineMemoryProvider, const Nautilus::Record& record)
{
    ...
    auto value = valueValue.cast<nautilus::val<int32_t>>();

    nautilus::invoke(
        +[](AggregationState* st, int32_t val) -> void
        {
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_intunion_mutex);
            Set** slot = reinterpret_cast<Set**>(st);
            *slot = int_union_transfn(*slot, val);
        },
        aggregationState, value);
}

void IntUnionTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record IntUnionTransfnAggregationPhysicalFunction::lower(
...
void IntUnionTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Set** slot = reinterpret_cast<Set**>(st); *slot = nullptr; }, aggregationState);
}

size_t IntUnionTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Set*);
}

void IntUnionTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Set** slot = reinterpret_cast<Set**>(st); if (*slot) { free(*slot); *slot = nullptr; } }, aggregationState);
}

AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterIntUnionTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("INT_UNION_TRANSFN aggregation cannot be created through the registry. "
