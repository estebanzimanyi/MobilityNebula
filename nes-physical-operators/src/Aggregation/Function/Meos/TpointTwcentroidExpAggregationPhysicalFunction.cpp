...
static std::mutex meos_tpointtwcentroidexp_mutex;


TpointTwcentroidExpAggregationPhysicalFunction::TpointTwcentroidExpAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TpointTwcentroidExpAggregationPhysicalFunction::lift(
    const nautilus::val<AggregationState*>& aggregationState, PipelineMemoryProvider& pipelineMemoryProvider, const Nautilus::Record& record)
{
    ...
    nautilus::invoke(
        +[](AggregationState* st, double lonVal, double latVal, int64_t tsVal) -> void
        {
            ...
            std::lock_guard<std::mutex> lock(meos_tpointtwcentroidexp_mutex);
...
void TpointTwcentroidExpAggregationPhysicalFunction::combine(
...
Nautilus::Record TpointTwcentroidExpAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto hexStr = nautilus::invoke(
        +[](AggregationState* st) -> char*
        {
            ...
            GSERIALIZED* res = tpoint_twcentroid(*slot);
            if (!res) {
                return (char*)nullptr;
            }
...
void TpointTwcentroidExpAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TpointTwcentroidExpAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Temporal*);
}

void TpointTwcentroidExpAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* st) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTpointTwcentroidExpAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TpointTwcentroidExp aggregation cannot be created through the registry. "
