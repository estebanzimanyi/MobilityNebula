...
static std::mutex meos_geosetmake_mutex;


GeosetMakeAggregationPhysicalFunction::GeosetMakeAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void GeosetMakeAggregationPhysicalFunction::lift(
...
void GeosetMakeAggregationPhysicalFunction::combine(
...
Nautilus::Record GeosetMakeAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    // build the result set via geoset_make and free each element + the array.
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        arrIdx = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_geosetmake_mutex);
                std::string s(vp, vs);
    ...
    auto boxStr = nautilus::invoke(
        +[](void* a, int64_t n) -> char*
        {
            ...
            Set* s = geoset_make((GSERIALIZED **) a, (int) n);
            for (int64_t i = 0; i < n; i++) { free(((GSERIALIZED**) a)[i]); }
            free(a);
            if (!s) { return (char*)nullptr; }
            char* out = spatialset_as_text(s, 6);
            free(s);
            return out;
        },
        arrState,
...
void GeosetMakeAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t GeosetMakeAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void GeosetMakeAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterGeosetMakeAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("GeosetMake aggregation cannot be created through the registry. "
