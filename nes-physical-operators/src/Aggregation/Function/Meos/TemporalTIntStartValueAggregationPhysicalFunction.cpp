...
static std::mutex meos_temporaltintstartvalue_mutex;


TemporalTIntStartValueAggregationPhysicalFunction::TemporalTIntStartValueAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TemporalTIntStartValueAggregationPhysicalFunction::lift(
...
void TemporalTIntStartValueAggregationPhysicalFunction::combine(
...
Nautilus::Record TemporalTIntStartValueAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        auto value = valueRaw.cast<nautilus::val<int32_t>>();
        auto timestamp = timestampRaw.cast<nautilus::val<int64_t>>();

        sequenceStr = nautilus::invoke(
            +[](char* buffer, int32_t valueVal, int64_t tsVal, int64_t counter) -> char*
            {
                if (counter > 0) {
                    strcat(buffer, ", ");
                }
                ...
                char itemStr[80];
                sprintf(itemStr, "%d@%s", valueVal, timestampStr);
    ...
    auto resultValue = nautilus::invoke(
        +[](const char* seqStr) -> int
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_temporaltintstartvalue_mutex);
            ...
            int value = tint_start_value(temp);
...
void TemporalTIntStartValueAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TemporalTIntStartValueAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TemporalTIntStartValueAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTemporalTIntStartValueAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TemporalTIntStartValue aggregation cannot be created through the registry. "
