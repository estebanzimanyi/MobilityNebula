...
static std::mutex meos_temporalendtimestamp_mutex;


TemporalEndTimestamptzAggregationPhysicalFunction::TemporalEndTimestamptzAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TemporalEndTimestamptzAggregationPhysicalFunction::lift(
...
void TemporalEndTimestamptzAggregationPhysicalFunction::combine(
...
Nautilus::Record TemporalEndTimestamptzAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    if (numberOfEntries == nautilus::val<size_t>(0)) {
        ...
        resultRecord.write(resultFieldIdentifier, nautilus::val<int64_t>(0));
        return resultRecord;
    }
    ...
    auto resultValue = nautilus::invoke(
        +[](const char* trajStr) -> int64_t
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_temporalendtimestamp_mutex);
            ...
            int64_t value = temporal_end_timestamptz(static_cast<Temporal*>(temp));
...
void TemporalEndTimestamptzAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TemporalEndTimestamptzAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TemporalEndTimestamptzAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTemporalEndTimestamptzAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TemporalEndTimestamptz aggregation cannot be created through the registry. "
