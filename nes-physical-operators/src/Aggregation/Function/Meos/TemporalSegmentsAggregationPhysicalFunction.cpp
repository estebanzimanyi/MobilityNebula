...
static std::mutex meos_temporalsegments_mutex;


TemporalSegmentsAggregationPhysicalFunction::TemporalSegmentsAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TemporalSegmentsAggregationPhysicalFunction::lift(
...
void TemporalSegmentsAggregationPhysicalFunction::combine(
...
Nautilus::Record TemporalSegmentsAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto boxStr = nautilus::invoke(
        +[](const char* trajStr) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_temporalsegments_mutex);
            ...
            int _cnt = 0;
            void** arr = (void**) temporal_segments(static_cast<Temporal*>(temp), &_cnt);
...
void TemporalSegmentsAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TemporalSegmentsAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TemporalSegmentsAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTemporalSegmentsAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TemporalSegments aggregation cannot be created through the registry. "
