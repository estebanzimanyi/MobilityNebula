...
static std::mutex meos_tfloattsumtransfn_mutex;


TfloatTsumTransfnAggregationPhysicalFunction::TfloatTsumTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TfloatTsumTransfnAggregationPhysicalFunction::lift(
...
void TfloatTsumTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TfloatTsumTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        skipState = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_tfloattsumtransfn_mutex);
                ...
                SkipList* ns = tfloat_tsum_transfn(static_cast<SkipList*>(state), inst);
                free(inst);
...
void TfloatTsumTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TfloatTsumTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TfloatTsumTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTfloatTsumTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TfloatTsumTransfn aggregation cannot be created through the registry. "
