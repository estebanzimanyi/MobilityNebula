...
static std::mutex meos_posesetmake_mutex;


PosesetMakeAggregationPhysicalFunction::PosesetMakeAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void PosesetMakeAggregationPhysicalFunction::lift(
...
void PosesetMakeAggregationPhysicalFunction::combine(
...
Nautilus::Record PosesetMakeAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    // via pose_in into a heap pointer, store it in a Pose*[] array, then
    // build the result set via poseset_make and free each element + the array.
    auto arrState = nautilus::invoke(
        +[](size_t n) -> void* { return malloc(sizeof(Pose*) * (n > 0 ? n : 1)); },
        numberOfEntries);
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        arrIdx = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_posesetmake_mutex);
                std::string s(vp, vs);
                while (!s.empty() && (s.front()=='\'' || s.front()=='"')) s.erase(s.begin());
                while (!s.empty() && (s.back()=='\'' || s.back()=='"')) s.pop_back();
                Pose* e = pose_in(s.c_str());
                if (e) { ((Pose**) a)[i] = e; return i + 1; }
                return i;
            },
            arrState,
            arrIdx,
    ...
    auto boxStr = nautilus::invoke(
        +[](void* a, int64_t n) -> char*
        {
            ...
            Set* s = poseset_make((const Pose **) a, (int) n);
            for (int64_t i = 0; i < n; i++) { free(((Pose**) a)[i]); }
            free(a);
            if (!s) { return (char*)nullptr; }
            char* out = poseset_out(s, 6);
            free(s);
            return out;
        },
        arrState,
...
void PosesetMakeAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t PosesetMakeAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void PosesetMakeAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterPosesetMakeAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("PosesetMake aggregation cannot be created through the registry. "
