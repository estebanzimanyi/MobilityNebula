...
static std::mutex meos_poseuniontransfn_mutex;


PoseUnionTransfnAggregationPhysicalFunction::PoseUnionTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void PoseUnionTransfnAggregationPhysicalFunction::lift(
...
void PoseUnionTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record PoseUnionTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        cstate = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_poseuniontransfn_mutex);
                std::string s(valPtr, valSize);
                while (!s.empty() && (s.front()=='\'' || s.front()=='"')) s.erase(s.begin());
                while (!s.empty() && (s.back()=='\'' || s.back()=='"')) s.pop_back();
                Pose* c = pose_in(s.c_str()); if (!c) return state; Set* ns = pose_union_transfn(static_cast<Set*>(state), c); free(c); return reinterpret_cast<void*>(ns);
            },
            cstate,
            valVar.getContent(),
    ...
    auto boxStr = nautilus::invoke(
        +[](void* state) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_poseuniontransfn_mutex);
            Set* fin = set_union_finalfn(static_cast<Set*>(state)); if (!fin) return (char*)nullptr; char* out = poseset_out(fin, 15); free(fin); return out;
        },
        cstate);
...
void PoseUnionTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t PoseUnionTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void PoseUnionTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterPoseUnionTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("PoseUnionTransfn aggregation cannot be created through the registry. "
