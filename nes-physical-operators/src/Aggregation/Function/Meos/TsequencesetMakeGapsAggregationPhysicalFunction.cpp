...
static std::mutex meos_tsequencesetmakegaps_mutex;


TsequencesetMakeGapsAggregationPhysicalFunction::TsequencesetMakeGapsAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TsequencesetMakeGapsAggregationPhysicalFunction::lift(
...
void TsequencesetMakeGapsAggregationPhysicalFunction::combine(
...
Nautilus::Record TsequencesetMakeGapsAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    // via tgeompoint_in into a heap pointer, store it in a Temporal*[] array, then
    // build the result set via tsequenceset_make_gaps and free each element + the array.
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        arrIdx = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_tsequencesetmakegaps_mutex);
                std::string s(vp, vs);
                while (!s.empty() && (s.front()=='\'' || s.front()=='"')) s.erase(s.begin());
                while (!s.empty() && (s.back()=='\'' || s.back()=='"')) s.pop_back();
                Temporal* e = tgeompoint_in(s.c_str());
    ...
    auto boxStr = nautilus::invoke(
        +[](void* a, int64_t n) -> char*
        {
            ...
            Interval* maxt = interval_in("1 hour", -1);
            Temporal* s = (Temporal*) tsequenceset_make_gaps((TInstant **) a, (int) n, LINEAR, maxt, 5.0);
...
void TsequencesetMakeGapsAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TsequencesetMakeGapsAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TsequencesetMakeGapsAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTsequencesetMakeGapsAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TsequencesetMakeGaps aggregation cannot be created through the registry. "
