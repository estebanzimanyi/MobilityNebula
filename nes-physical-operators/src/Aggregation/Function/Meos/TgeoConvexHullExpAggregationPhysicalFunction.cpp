...
static std::mutex meos_tgeoconvexhullexp_mutex;


TgeoConvexHullExpAggregationPhysicalFunction::TgeoConvexHullExpAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TgeoConvexHullExpAggregationPhysicalFunction::lift(
    const nautilus::val<AggregationState*>& aggregationState, PipelineMemoryProvider& pipelineMemoryProvider, const Nautilus::Record& record)
{
    ...
    nautilus::invoke(
        +[](AggregationState* st, double lonVal, double latVal, int64_t tsVal) -> void
        {
            ...
            std::lock_guard<std::mutex> lock(meos_tgeoconvexhullexp_mutex);
...
void TgeoConvexHullExpAggregationPhysicalFunction::combine(
...
Nautilus::Record TgeoConvexHullExpAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto hexStr = nautilus::invoke(
        +[](AggregationState* st) -> char*
        {
            ...
            GSERIALIZED* res = tgeo_convex_hull(*slot);
            if (!res) {
                return (char*)nullptr;
            }
...
void TgeoConvexHullExpAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TgeoConvexHullExpAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Temporal*);
}

void TgeoConvexHullExpAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* st) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTgeoConvexHullExpAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TgeoConvexHullExp aggregation cannot be created through the registry. "
