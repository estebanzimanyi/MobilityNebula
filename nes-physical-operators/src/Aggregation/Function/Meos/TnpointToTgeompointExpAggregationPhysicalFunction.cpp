...
static std::mutex meos_tnpointtotgeompointexp_mutex;


TnpointToTgeompointExpAggregationPhysicalFunction::TnpointToTgeompointExpAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TnpointToTgeompointExpAggregationPhysicalFunction::lift(
    const nautilus::val<AggregationState*>& aggregationState, PipelineMemoryProvider& pipelineMemoryProvider, const Nautilus::Record& record)
{
    ...
    nautilus::invoke(
        +[](AggregationState* st, int64_t ridVal, double fracVal, int64_t tsVal) -> void
        {
            ...
            std::lock_guard<std::mutex> lock(meos_tnpointtotgeompointexp_mutex);
...
void TnpointToTgeompointExpAggregationPhysicalFunction::combine(
...
Nautilus::Record TnpointToTgeompointExpAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto hexStr = nautilus::invoke(
        +[](AggregationState* st) -> char*
        {
            ...
            Temporal* res = tnpoint_to_tgeompoint(*slot);
            if (!res) {
                return (char*)nullptr;
            }
            size_t hexSize = 0;
...
void TnpointToTgeompointExpAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TnpointToTgeompointExpAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Temporal*);
}

void TnpointToTgeompointExpAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* st) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTnpointToTgeompointExpAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TNPOINT_TO_TGEOMPOINT_EXP aggregation cannot be created through the registry. "
