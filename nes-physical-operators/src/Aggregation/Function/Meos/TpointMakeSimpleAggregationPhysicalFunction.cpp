...
static std::mutex meos_tpointmakesimple_mutex;


TpointMakeSimpleAggregationPhysicalFunction::TpointMakeSimpleAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TpointMakeSimpleAggregationPhysicalFunction::lift(
...
void TpointMakeSimpleAggregationPhysicalFunction::combine(
...
Nautilus::Record TpointMakeSimpleAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto boxStr = nautilus::invoke(
        +[](const char* trajStr) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_tpointmakesimple_mutex);
            ...
            int _cnt = 0;
            void** arr = (void**) tpoint_make_simple(static_cast<Temporal*>(temp), &_cnt);
...
void TpointMakeSimpleAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TpointMakeSimpleAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TpointMakeSimpleAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTpointMakeSimpleAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TpointMakeSimple aggregation cannot be created through the registry. "
