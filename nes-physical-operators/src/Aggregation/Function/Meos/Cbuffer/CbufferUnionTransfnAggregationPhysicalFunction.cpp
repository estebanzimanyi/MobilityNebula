...
static std::mutex meos_cbufferuniontransfn_mutex;


CbufferUnionTransfnAggregationPhysicalFunction::CbufferUnionTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void CbufferUnionTransfnAggregationPhysicalFunction::lift(
...
void CbufferUnionTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record CbufferUnionTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        cstate = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_cbufferuniontransfn_mutex);
                std::string s(valPtr, valSize);
                while (!s.empty() && (s.front()=='\'' || s.front()=='"')) s.erase(s.begin());
                while (!s.empty() && (s.back()=='\'' || s.back()=='"')) s.pop_back();
                Cbuffer* c = cbuffer_in(s.c_str()); if (!c) return state; Set* ns = cbuffer_union_transfn(static_cast<Set*>(state), c); free(c); return reinterpret_cast<void*>(ns);
            },
            cstate,
            valVar.getContent(),
    ...
    auto boxStr = nautilus::invoke(
        +[](void* state) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_cbufferuniontransfn_mutex);
            Set* fin = set_union_finalfn(static_cast<Set*>(state)); if (!fin) return (char*)nullptr; char* out = cbufferset_out(fin, 15); free(fin); return out;
        },
        cstate);
...
void CbufferUnionTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t CbufferUnionTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void CbufferUnionTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterCbufferUnionTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("CbufferUnionTransfn aggregation cannot be created through the registry. "
