...
static std::mutex meos_cbuffersetmake_mutex;


CbuffersetMakeAggregationPhysicalFunction::CbuffersetMakeAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void CbuffersetMakeAggregationPhysicalFunction::lift(
...
void CbuffersetMakeAggregationPhysicalFunction::combine(
...
Nautilus::Record CbuffersetMakeAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    // via cbuffer_in into a heap pointer, store it in a Cbuffer*[] array, then
    // build the result set via cbufferset_make and free each element + the array.
    auto arrState = nautilus::invoke(
        +[](size_t n) -> void* { return malloc(sizeof(Cbuffer*) * (n > 0 ? n : 1)); },
        numberOfEntries);
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        arrIdx = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_cbuffersetmake_mutex);
                std::string s(vp, vs);
                while (!s.empty() && (s.front()=='\'' || s.front()=='"')) s.erase(s.begin());
                while (!s.empty() && (s.back()=='\'' || s.back()=='"')) s.pop_back();
                Cbuffer* e = cbuffer_in(s.c_str());
                if (e) { ((Cbuffer**) a)[i] = e; return i + 1; }
                return i;
            },
            arrState,
            arrIdx,
    ...
    auto boxStr = nautilus::invoke(
        +[](void* a, int64_t n) -> char*
        {
            ...
            Set* s = cbufferset_make((Cbuffer **) a, (int) n);
            for (int64_t i = 0; i < n; i++) { free(((Cbuffer**) a)[i]); }
            free(a);
            if (!s) { return (char*)nullptr; }
            char* out = cbufferset_out(s, 6);
            free(s);
            return out;
        },
        arrState,
...
void CbuffersetMakeAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t CbuffersetMakeAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void CbuffersetMakeAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterCbuffersetMakeAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("CbuffersetMake aggregation cannot be created through the registry. "
