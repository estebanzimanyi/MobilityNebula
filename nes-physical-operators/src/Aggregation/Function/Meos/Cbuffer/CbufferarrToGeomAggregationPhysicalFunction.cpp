...
static std::mutex meos_cbufferarrtogeom_mutex;


CbufferarrToGeomAggregationPhysicalFunction::CbufferarrToGeomAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void CbufferarrToGeomAggregationPhysicalFunction::lift(
...
void CbufferarrToGeomAggregationPhysicalFunction::combine(
...
Nautilus::Record CbufferarrToGeomAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    // build the result set via cbufferarr_to_geom and free each element + the array.
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        arrIdx = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_cbufferarrtogeom_mutex);
                std::string s(vp, vs);
    ...
    auto boxStr = nautilus::invoke(
        +[](void* a, int64_t n) -> char*
        {
            ...
            GSERIALIZED* s = cbufferarr_to_geom((const Cbuffer **) a, (int) n);
...
void CbufferarrToGeomAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t CbufferarrToGeomAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void CbufferarrToGeomAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterCbufferarrToGeomAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("CbufferarrToGeom aggregation cannot be created through the registry. "
