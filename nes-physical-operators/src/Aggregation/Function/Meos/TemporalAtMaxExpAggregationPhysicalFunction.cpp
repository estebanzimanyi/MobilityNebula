...
static std::mutex meos_temporalatmaxexp_mutex;


TemporalAtMaxExpAggregationPhysicalFunction::TemporalAtMaxExpAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TemporalAtMaxExpAggregationPhysicalFunction::lift(
    const nautilus::val<AggregationState*>& aggregationState, PipelineMemoryProvider& pipelineMemoryProvider, const Nautilus::Record& record)
{
    ...
    nautilus::invoke(
        +[](AggregationState* st, double valueVal, int64_t tsVal) -> void
        {
            ...
            std::lock_guard<std::mutex> lock(meos_temporalatmaxexp_mutex);
...
void TemporalAtMaxExpAggregationPhysicalFunction::combine(
...
Nautilus::Record TemporalAtMaxExpAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto hexStr = nautilus::invoke(
        +[](AggregationState* st) -> char*
        {
            ...
            Temporal* res = temporal_at_max(*slot);
            if (!res) {
                return (char*)nullptr;
            }
            size_t hexSize = 0;
...
void TemporalAtMaxExpAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TemporalAtMaxExpAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Temporal*);
}

void TemporalAtMaxExpAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* st) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTemporalAtMaxExpAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TEMPORAL_AT_MAX_EXP aggregation cannot be created through the registry. "
