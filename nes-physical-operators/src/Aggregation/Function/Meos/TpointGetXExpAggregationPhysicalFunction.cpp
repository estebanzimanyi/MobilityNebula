...
static std::mutex meos_tpointgetxexp_mutex;


TpointGetXExpAggregationPhysicalFunction::TpointGetXExpAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TpointGetXExpAggregationPhysicalFunction::lift(
    const nautilus::val<AggregationState*>& aggregationState, PipelineMemoryProvider& pipelineMemoryProvider, const Nautilus::Record& record)
{
    ...
    nautilus::invoke(
        +[](AggregationState* st, double lonVal, double latVal, int64_t tsVal) -> void
        {
            ...
            std::lock_guard<std::mutex> lock(meos_tpointgetxexp_mutex);
...
void TpointGetXExpAggregationPhysicalFunction::combine(
...
Nautilus::Record TpointGetXExpAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto hexStr = nautilus::invoke(
        +[](AggregationState* st) -> char*
        {
            ...
            Temporal* res = tpoint_get_x(*slot);
            if (!res) {
                return (char*)nullptr;
            }
            size_t hexSize = 0;
...
void TpointGetXExpAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TpointGetXExpAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Temporal*);
}

void TpointGetXExpAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* st) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTpointGetXExpAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TpointGetXExp aggregation cannot be created through the registry. "
