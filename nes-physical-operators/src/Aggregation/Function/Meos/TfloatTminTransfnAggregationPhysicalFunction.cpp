...
static std::mutex meos_tfloattmintransfn_mutex;


TfloatTminTransfnAggregationPhysicalFunction::TfloatTminTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TfloatTminTransfnAggregationPhysicalFunction::lift(
...
void TfloatTminTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TfloatTminTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        skipState = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_tfloattmintransfn_mutex);
                ...
                SkipList* ns = tfloat_tmin_transfn(static_cast<SkipList*>(state), inst);
                free(inst);
...
void TfloatTminTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TfloatTminTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TfloatTminTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTfloatTminTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TfloatTminTransfn aggregation cannot be created through the registry. "
