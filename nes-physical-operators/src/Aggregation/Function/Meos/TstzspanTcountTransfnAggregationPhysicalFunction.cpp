...
static std::mutex meos_tstzspantcounttransfn_mutex;


TstzspanTcountTransfnAggregationPhysicalFunction::TstzspanTcountTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TstzspanTcountTransfnAggregationPhysicalFunction::lift(
...
void TstzspanTcountTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TstzspanTcountTransfnAggregationPhysicalFunction::lower(
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        cstate = nautilus::invoke(
            +[](void* state, const char* valPtr, uint32_t valSize) -> void*
            {
                ...
                Span* c = tstzspan_in(s.c_str()); if (!c) return state; SkipList* ns = tstzspan_tcount_transfn(static_cast<SkipList*>(state), c); free(c); return reinterpret_cast<void*>(ns);
            },
            cstate,
            valVar.getContent(),
...
void TstzspanTcountTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TstzspanTcountTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TstzspanTcountTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTstzspanTcountTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TstzspanTcountTransfn aggregation cannot be created through the registry. "
