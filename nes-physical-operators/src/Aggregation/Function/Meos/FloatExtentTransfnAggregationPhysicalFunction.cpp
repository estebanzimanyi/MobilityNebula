...
static std::mutex meos_floatextent_mutex;


FloatExtentTransfnAggregationPhysicalFunction::FloatExtentTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void FloatExtentTransfnAggregationPhysicalFunction::lift(
    const nautilus::val<AggregationState*>& aggregationState, PipelineMemoryProvider& pipelineMemoryProvider, const Nautilus::Record& record)
{
    ...
    nautilus::invoke(
        +[](AggregationState* st, double val) -> void
        {
            ...
            std::lock_guard<std::mutex> lock(meos_floatextent_mutex);
            Span** slot = reinterpret_cast<Span**>(st);
            *slot = float_extent_transfn(*slot, val);
        },
        aggregationState, value);
}

void FloatExtentTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record FloatExtentTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto boxStr = nautilus::invoke(
        +[](AggregationState* st) -> char*
        {
            ...
            char* out = floatspan_out(*slot, 15);
            free(*slot);
            *slot = nullptr;
            return out;
        },
...
void FloatExtentTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Span** slot = reinterpret_cast<Span**>(st); *slot = nullptr; }, aggregationState);
}

size_t FloatExtentTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Span*);
}

void FloatExtentTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Span** slot = reinterpret_cast<Span**>(st); if (*slot) { free(*slot); *slot = nullptr; } }, aggregationState);
}

AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterFloatExtentTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("FLOAT_EXTENT_TRANSFN aggregation cannot be created through the registry. "
