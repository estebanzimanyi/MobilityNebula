...
static std::mutex meos_timestamptzunion_mutex;


TimestamptzUnionTransfnAggregationPhysicalFunction::TimestamptzUnionTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TimestamptzUnionTransfnAggregationPhysicalFunction::lift(
    const nautilus::val<AggregationState*>& aggregationState, PipelineMemoryProvider& pipelineMemoryProvider, const Nautilus::Record& record)
{
    ...
    nautilus::invoke(
        +[](AggregationState* st, int64_t val) -> void
        {
            ...
            long long sec = (val > 1000000000000LL) ? (val / 1000) : val;
            TimestampTz tstz = ((int64_t) sec - 946684800LL) * 1000000LL;
            *slot = timestamptz_union_transfn(*slot, tstz);
        },
        aggregationState, value);
}

void TimestamptzUnionTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TimestamptzUnionTransfnAggregationPhysicalFunction::lower(
...
void TimestamptzUnionTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Set** slot = reinterpret_cast<Set**>(st); *slot = nullptr; }, aggregationState);
}

size_t TimestamptzUnionTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Set*);
}

void TimestamptzUnionTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(+[](AggregationState* st) -> void
        { Set** slot = reinterpret_cast<Set**>(st); if (*slot) { free(*slot); *slot = nullptr; } }, aggregationState);
}

AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTimestamptzUnionTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TIMESTAMPTZ_UNION_TRANSFN aggregation cannot be created through the registry. "
