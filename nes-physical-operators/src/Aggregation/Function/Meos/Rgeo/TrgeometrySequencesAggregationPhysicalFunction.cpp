...
static std::mutex meos_trgeometrysequences_mutex;


TrgeometrySequencesAggregationPhysicalFunction::TrgeometrySequencesAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TrgeometrySequencesAggregationPhysicalFunction::lift(
...
void TrgeometrySequencesAggregationPhysicalFunction::combine(
...
Nautilus::Record TrgeometrySequencesAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto boxStr = nautilus::invoke(
        +[](const char* trajStr) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_trgeometrysequences_mutex);
            ...
            int _cnt = 0;
            void** arr = (void**) trgeometry_sequences(static_cast<Temporal*>(temp), &_cnt);
...
void TrgeometrySequencesAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TrgeometrySequencesAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TrgeometrySequencesAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTrgeometrySequencesAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TrgeometrySequences aggregation cannot be created through the registry. "
