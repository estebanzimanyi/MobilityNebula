...
static std::mutex meos_trgeometryrestrictvalues_mutex;


TrgeometryRestrictValuesAggregationPhysicalFunction::TrgeometryRestrictValuesAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TrgeometryRestrictValuesAggregationPhysicalFunction::lift(
...
void TrgeometryRestrictValuesAggregationPhysicalFunction::combine(
...
Nautilus::Record TrgeometryRestrictValuesAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto boxStr = nautilus::invoke(
        +[](const char* trajStr, const char* constArg0, const char* constArg1) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_trgeometryrestrictvalues_mutex);
            ...
            Set* c0 = geomset_in(constArg0);
            bool c1 = (constArg1[0]=='t'||constArg1[0]=='T'||constArg1[0]=='1');

            Temporal* res = (Temporal*) trgeometry_restrict_values(static_cast<Temporal*>(temp), c0, c1);
...
void TrgeometryRestrictValuesAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TrgeometryRestrictValuesAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TrgeometryRestrictValuesAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTrgeometryRestrictValuesAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TrgeometryRestrictValues aggregation cannot be created through the registry. "
