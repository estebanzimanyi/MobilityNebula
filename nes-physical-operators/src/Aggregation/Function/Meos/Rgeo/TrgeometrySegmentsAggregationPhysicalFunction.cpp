...
static std::mutex meos_trgeometrysegments_mutex;


TrgeometrySegmentsAggregationPhysicalFunction::TrgeometrySegmentsAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TrgeometrySegmentsAggregationPhysicalFunction::lift(
...
void TrgeometrySegmentsAggregationPhysicalFunction::combine(
...
Nautilus::Record TrgeometrySegmentsAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto boxStr = nautilus::invoke(
        +[](const char* trajStr) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_trgeometrysegments_mutex);
            ...
            int _cnt = 0;
            void** arr = (void**) trgeometry_segments(static_cast<Temporal*>(temp), &_cnt);
...
void TrgeometrySegmentsAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TrgeometrySegmentsAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TrgeometrySegmentsAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTrgeometrySegmentsAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TrgeometrySegments aggregation cannot be created through the registry. "
