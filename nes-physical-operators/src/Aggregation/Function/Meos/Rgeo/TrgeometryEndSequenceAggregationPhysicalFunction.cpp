...
static std::mutex meos_trgeometryendsequence_mutex;


TrgeometryEndSequenceAggregationPhysicalFunction::TrgeometryEndSequenceAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TrgeometryEndSequenceAggregationPhysicalFunction::lift(
...
void TrgeometryEndSequenceAggregationPhysicalFunction::combine(
...
Nautilus::Record TrgeometryEndSequenceAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto boxStr = nautilus::invoke(
        +[](const char* trajStr) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_trgeometryendsequence_mutex);
            ...
            Temporal* _e = (Temporal*) trgeometry_end_sequence(static_cast<Temporal*>(temp));
...
void TrgeometryEndSequenceAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TrgeometryEndSequenceAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TrgeometryEndSequenceAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTrgeometryEndSequenceAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TrgeometryEndSequence aggregation cannot be created through the registry. "
