...
static std::mutex meos_trgeometrysequencen_mutex;


TrgeometrySequenceNAggregationPhysicalFunction::TrgeometrySequenceNAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TrgeometrySequenceNAggregationPhysicalFunction::lift(
...
void TrgeometrySequenceNAggregationPhysicalFunction::combine(
...
Nautilus::Record TrgeometrySequenceNAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto boxStr = nautilus::invoke(
        +[](const char* trajStr, const char* constArg0) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_trgeometrysequencen_mutex);
            ...
            int c0 = atoi(constArg0);

            Temporal* res = (Temporal*) trgeometry_sequence_n(static_cast<Temporal*>(temp), c0);
...
void TrgeometrySequenceNAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TrgeometrySequenceNAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TrgeometrySequenceNAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTrgeometrySequenceNAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TrgeometrySequenceN aggregation cannot be created through the registry. "
