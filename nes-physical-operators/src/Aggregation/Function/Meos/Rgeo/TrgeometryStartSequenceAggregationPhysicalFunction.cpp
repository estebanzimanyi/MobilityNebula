...
static std::mutex meos_trgeometrystartsequence_mutex;


TrgeometryStartSequenceAggregationPhysicalFunction::TrgeometryStartSequenceAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TrgeometryStartSequenceAggregationPhysicalFunction::lift(
...
void TrgeometryStartSequenceAggregationPhysicalFunction::combine(
...
Nautilus::Record TrgeometryStartSequenceAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto boxStr = nautilus::invoke(
        +[](const char* trajStr) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_trgeometrystartsequence_mutex);
            ...
            Temporal* _e = (Temporal*) trgeometry_start_sequence(static_cast<Temporal*>(temp));
...
void TrgeometryStartSequenceAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TrgeometryStartSequenceAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TrgeometryStartSequenceAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTrgeometryStartSequenceAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TrgeometryStartSequence aggregation cannot be created through the registry. "
