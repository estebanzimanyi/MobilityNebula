...
static std::mutex meos_trgeometrysetinterp_mutex;


TrgeometrySetInterpAggregationPhysicalFunction::TrgeometrySetInterpAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TrgeometrySetInterpAggregationPhysicalFunction::lift(
...
void TrgeometrySetInterpAggregationPhysicalFunction::combine(
...
Nautilus::Record TrgeometrySetInterpAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto boxStr = nautilus::invoke(
        +[](const char* trajStr, const char* constArg0) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_trgeometrysetinterp_mutex);
            ...
            interpType c0 = (interpType) atoi(constArg0);

            Temporal* res = (Temporal*) trgeometry_set_interp(static_cast<Temporal*>(temp), c0);
...
void TrgeometrySetInterpAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TrgeometrySetInterpAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TrgeometrySetInterpAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTrgeometrySetInterpAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TrgeometrySetInterp aggregation cannot be created through the registry. "
