...
static std::mutex meos_spansetextenttransfn_mutex;


SpansetExtentTransfnAggregationPhysicalFunction::SpansetExtentTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void SpansetExtentTransfnAggregationPhysicalFunction::lift(
...
void SpansetExtentTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record SpansetExtentTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        cstate = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_spansetextenttransfn_mutex);
                std::string s(valPtr, valSize);
                while (!s.empty() && (s.front()=='\'' || s.front()=='"')) s.erase(s.begin());
                while (!s.empty() && (s.back()=='\'' || s.back()=='"')) s.pop_back();
                SpanSet* c = floatspanset_in(s.c_str()); if (!c) return state; Span* ns = spanset_extent_transfn(static_cast<Span*>(state), c); free(c); return reinterpret_cast<void*>(ns);
            },
            cstate,
            valVar.getContent(),
    ...
    auto boxStr = nautilus::invoke(
        +[](void* state) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_spansetextenttransfn_mutex);
            Span* fin = static_cast<Span*>(state); char* out = floatspan_out(fin, 15); free(fin); return out;
        },
        cstate);
...
void SpansetExtentTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t SpansetExtentTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void SpansetExtentTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterSpansetExtentTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("SpansetExtentTransfn aggregation cannot be created through the registry. "
