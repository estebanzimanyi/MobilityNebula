...
static std::mutex meos_textsetmake_mutex;


TextsetMakeAggregationPhysicalFunction::TextsetMakeAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TextsetMakeAggregationPhysicalFunction::lift(
...
void TextsetMakeAggregationPhysicalFunction::combine(
...
Nautilus::Record TextsetMakeAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    // via text_in into a heap pointer, store it in a text*[] array, then
    // build the result set via textset_make and free each element + the array.
    auto arrState = nautilus::invoke(
        +[](size_t n) -> void* { return malloc(sizeof(text*) * (n > 0 ? n : 1)); },
        numberOfEntries);
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        arrIdx = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_textsetmake_mutex);
                std::string s(vp, vs);
                while (!s.empty() && (s.front()=='\'' || s.front()=='"')) s.erase(s.begin());
                while (!s.empty() && (s.back()=='\'' || s.back()=='"')) s.pop_back();
                text* e = text_in(s.c_str());
                if (e) { ((text**) a)[i] = e; return i + 1; }
                return i;
            },
            arrState,
            arrIdx,
    ...
    auto boxStr = nautilus::invoke(
        +[](void* a, int64_t n) -> char*
        {
            ...
            Set* s = textset_make((text **) a, (int) n);
            for (int64_t i = 0; i < n; i++) { free(((text**) a)[i]); }
            free(a);
...
void TextsetMakeAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TextsetMakeAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TextsetMakeAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTextsetMakeAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TextsetMake aggregation cannot be created through the registry. "
