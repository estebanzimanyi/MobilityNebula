...
static std::mutex meos_floatsetmake_mutex;


FloatsetMakeAggregationPhysicalFunction::FloatsetMakeAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void FloatsetMakeAggregationPhysicalFunction::lift(
...
void FloatsetMakeAggregationPhysicalFunction::combine(
...
Nautilus::Record FloatsetMakeAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    // via the MEOS array constructor floatset_make. The array pointer threads across
    // events as an opaque void* (Nautilus invoke ABI), sized to numberOfEntries.
    auto arrState = nautilus::invoke(
        +[](size_t n) -> void* { return malloc(sizeof(double) * (n > 0 ? n : 1)); },
        numberOfEntries);
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        nautilus::invoke(
            +[](void* a, int64_t i, double val) -> void
            { ((double*) a)[i] = (double) val; },
            arrState,
            arrIdx,
            value);
    ...
    auto boxStr = nautilus::invoke(
        ...
        {
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_floatsetmake_mutex);
            Set* s = floatset_make((double*) a, (int) n);
            free(a);
            if (!s) {
                return (char*)nullptr;
            }
            char* out = floatset_out(s, 15);
            free(s);
            return out;
        },
        arrState,
...
void FloatsetMakeAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t FloatsetMakeAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void FloatsetMakeAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterFloatsetMakeAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("FloatsetMake aggregation cannot be created through the registry. "
