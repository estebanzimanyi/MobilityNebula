...
static std::mutex meos_tbooltandtransfn_mutex;


TboolTandTransfnAggregationPhysicalFunction::TboolTandTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TboolTandTransfnAggregationPhysicalFunction::lift(
...
void TboolTandTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TboolTandTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        skipState = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_tbooltandtransfn_mutex);
                ...
                char itemStr[80];
                sprintf(itemStr, "%s@%s", (valueVal ? "t" : "f"), tsS.c_str());
                Temporal* inst = tbool_in(itemStr);
                if (!inst) { return state; }
                SkipList* ns = tbool_tand_transfn(static_cast<SkipList*>(state), inst);
                free(inst);
...
void TboolTandTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TboolTandTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TboolTandTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTboolTandTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TboolTandTransfn aggregation cannot be created through the registry. "
