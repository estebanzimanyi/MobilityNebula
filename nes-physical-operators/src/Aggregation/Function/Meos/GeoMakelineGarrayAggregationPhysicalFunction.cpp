...
static std::mutex meos_geomakelinegarray_mutex;


GeoMakelineGarrayAggregationPhysicalFunction::GeoMakelineGarrayAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void GeoMakelineGarrayAggregationPhysicalFunction::lift(
...
void GeoMakelineGarrayAggregationPhysicalFunction::combine(
...
Nautilus::Record GeoMakelineGarrayAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    // via geom_in into a heap pointer, store it in a GSERIALIZED*[] array, then
    // build the result set via geo_makeline_garray and free each element + the array.
    auto arrState = nautilus::invoke(
        +[](size_t n) -> void* { return malloc(sizeof(GSERIALIZED*) * (n > 0 ? n : 1)); },
        numberOfEntries);
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        arrIdx = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_geomakelinegarray_mutex);
                std::string s(vp, vs);
                while (!s.empty() && (s.front()=='\'' || s.front()=='"')) s.erase(s.begin());
                while (!s.empty() && (s.back()=='\'' || s.back()=='"')) s.pop_back();
                GSERIALIZED* e = geom_in(s.c_str(), -1);
                if (e) { ((GSERIALIZED**) a)[i] = e; return i + 1; }
                return i;
            },
            arrState,
            arrIdx,
    ...
    auto boxStr = nautilus::invoke(
        +[](void* a, int64_t n) -> char*
        {
            ...
            GSERIALIZED* s = geo_makeline_garray((GSERIALIZED **) a, (int) n);
            for (int64_t i = 0; i < n; i++) { free(((GSERIALIZED**) a)[i]); }
            free(a);
            if (!s) { return (char*)nullptr; }
            char* out = geo_as_text(s, 6);
            free(s);
            return out;
        },
        arrState,
...
void GeoMakelineGarrayAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t GeoMakelineGarrayAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void GeoMakelineGarrayAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterGeoMakelineGarrayAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("GeoMakelineGarray aggregation cannot be created through the registry. "
