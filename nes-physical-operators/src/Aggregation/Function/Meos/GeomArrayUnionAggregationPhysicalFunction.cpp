...
static std::mutex meos_geomarrayunion_mutex;


GeomArrayUnionAggregationPhysicalFunction::GeomArrayUnionAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void GeomArrayUnionAggregationPhysicalFunction::lift(
...
void GeomArrayUnionAggregationPhysicalFunction::combine(
...
Nautilus::Record GeomArrayUnionAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    // build the result set via geom_array_union and free each element + the array.
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        arrIdx = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_geomarrayunion_mutex);
                std::string s(vp, vs);
    ...
    auto boxStr = nautilus::invoke(
        +[](void* a, int64_t n) -> char*
        {
            ...
            GSERIALIZED* s = geom_array_union((GSERIALIZED **) a, (int) n);
...
void GeomArrayUnionAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t GeomArrayUnionAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void GeomArrayUnionAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterGeomArrayUnionAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("GeomArrayUnion aggregation cannot be created through the registry. "
