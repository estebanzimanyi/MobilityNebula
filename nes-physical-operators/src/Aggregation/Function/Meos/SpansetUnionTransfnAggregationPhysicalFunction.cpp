...
static std::mutex meos_spansetuniontransfn_mutex;


SpansetUnionTransfnAggregationPhysicalFunction::SpansetUnionTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void SpansetUnionTransfnAggregationPhysicalFunction::lift(
...
void SpansetUnionTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record SpansetUnionTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        cstate = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_spansetuniontransfn_mutex);
                std::string s(valPtr, valSize);
                while (!s.empty() && (s.front()=='\'' || s.front()=='"')) s.erase(s.begin());
                while (!s.empty() && (s.back()=='\'' || s.back()=='"')) s.pop_back();
                SpanSet* c = floatspanset_in(s.c_str()); if (!c) return state; SpanSet* ns = spanset_union_transfn(static_cast<SpanSet*>(state), c); free(c); return reinterpret_cast<void*>(ns);
            },
            cstate,
            valVar.getContent(),
    ...
    auto boxStr = nautilus::invoke(
        +[](void* state) -> char*
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_spansetuniontransfn_mutex);
            SpanSet* fin = spanset_union_finalfn(static_cast<SpanSet*>(state)); if (!fin) return (char*)nullptr; char* out = floatspanset_out(fin, 15); free(fin); return out;
        },
        cstate);
...
void SpansetUnionTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t SpansetUnionTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void SpansetUnionTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterSpansetUnionTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("SpansetUnionTransfn aggregation cannot be created through the registry. "
