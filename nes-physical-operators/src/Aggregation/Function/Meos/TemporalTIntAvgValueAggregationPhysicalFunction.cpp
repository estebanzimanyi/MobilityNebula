...
static std::mutex meos_temporaltintavgvalue_mutex;


TemporalTIntAvgValueAggregationPhysicalFunction::TemporalTIntAvgValueAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TemporalTIntAvgValueAggregationPhysicalFunction::lift(
...
void TemporalTIntAvgValueAggregationPhysicalFunction::combine(
...
Nautilus::Record TemporalTIntAvgValueAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto resultValue = nautilus::invoke(
        +[](const char* seqStr) -> double
        {
            ...
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_temporaltintavgvalue_mutex);
...
void TemporalTIntAvgValueAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TemporalTIntAvgValueAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TemporalTIntAvgValueAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTemporalTIntAvgValueAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TemporalTIntAvgValue aggregation cannot be created through the registry. "
