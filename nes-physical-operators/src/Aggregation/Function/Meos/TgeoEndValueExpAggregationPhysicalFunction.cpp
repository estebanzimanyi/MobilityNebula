...
static std::mutex meos_tgeoendvalueexp_mutex;


TgeoEndValueExpAggregationPhysicalFunction::TgeoEndValueExpAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TgeoEndValueExpAggregationPhysicalFunction::lift(
    const nautilus::val<AggregationState*>& aggregationState, PipelineMemoryProvider& pipelineMemoryProvider, const Nautilus::Record& record)
{
    ...
    nautilus::invoke(
        +[](AggregationState* st, double lonVal, double latVal, int64_t tsVal) -> void
        {
            ...
            std::lock_guard<std::mutex> lock(meos_tgeoendvalueexp_mutex);
...
void TgeoEndValueExpAggregationPhysicalFunction::combine(
...
Nautilus::Record TgeoEndValueExpAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    auto hexStr = nautilus::invoke(
        +[](AggregationState* st) -> char*
        {
            ...
            GSERIALIZED* res = tgeo_end_value(*slot);
            if (!res) {
                return (char*)nullptr;
            }
...
void TgeoEndValueExpAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TgeoEndValueExpAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Temporal*);
}

void TgeoEndValueExpAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* st) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTgeoEndValueExpAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TgeoEndValueExp aggregation cannot be created through the registry. "
