...
static std::mutex meos_datesetmake_mutex;


DatesetMakeAggregationPhysicalFunction::DatesetMakeAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void DatesetMakeAggregationPhysicalFunction::lift(
...
void DatesetMakeAggregationPhysicalFunction::combine(
...
Nautilus::Record DatesetMakeAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    // via the MEOS array constructor dateset_make. The array pointer threads across
    // events as an opaque void* (Nautilus invoke ABI), sized to numberOfEntries.
    auto arrState = nautilus::invoke(
        +[](size_t n) -> void* { return malloc(sizeof(DateADT) * (n > 0 ? n : 1)); },
        numberOfEntries);
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        nautilus::invoke(
            +[](void* a, int64_t i, int32_t val) -> void
            { ((DateADT*) a)[i] = (DateADT) val; },
            arrState,
            arrIdx,
            value);
    ...
    auto boxStr = nautilus::invoke(
        ...
        {
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_datesetmake_mutex);
            Set* s = dateset_make((DateADT*) a, (int) n);
            free(a);
            if (!s) {
                return (char*)nullptr;
            }
            char* out = dateset_out(s);
            free(s);
            return out;
        },
        arrState,
...
void DatesetMakeAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t DatesetMakeAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void DatesetMakeAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterDatesetMakeAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("DatesetMake aggregation cannot be created through the registry. "
