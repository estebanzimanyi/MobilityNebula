...
static std::mutex meos_tintwsumtransfn_mutex;


TintWsumTransfnAggregationPhysicalFunction::TintWsumTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TintWsumTransfnAggregationPhysicalFunction::lift(
...
void TintWsumTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TintWsumTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        skipState = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_tintwsumtransfn_mutex);
                ...
                Interval* iv = interval_in("1 hour", -1); if (!iv) { free(inst); return state; } SkipList* ns = tint_wsum_transfn(static_cast<SkipList*>(state), inst, iv);
                free(iv); free(inst);
...
void TintWsumTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TintWsumTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TintWsumTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTintWsumTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TintWsumTransfn aggregation cannot be created through the registry. "
