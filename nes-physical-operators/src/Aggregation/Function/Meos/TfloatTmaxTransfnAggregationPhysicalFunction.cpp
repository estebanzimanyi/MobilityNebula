...
static std::mutex meos_tfloattmaxtransfn_mutex;


TfloatTmaxTransfnAggregationPhysicalFunction::TfloatTmaxTransfnAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TfloatTmaxTransfnAggregationPhysicalFunction::lift(
...
void TfloatTmaxTransfnAggregationPhysicalFunction::combine(
...
Nautilus::Record TfloatTmaxTransfnAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        skipState = nautilus::invoke(
            ...
            {
                MEOS::Meos::ensureMeosInitialized();
                std::lock_guard<std::mutex> lock(meos_tfloattmaxtransfn_mutex);
                ...
                SkipList* ns = tfloat_tmax_transfn(static_cast<SkipList*>(state), inst);
                free(inst);
...
void TfloatTmaxTransfnAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TfloatTmaxTransfnAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TfloatTmaxTransfnAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTfloatTmaxTransfnAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TfloatTmaxTransfn aggregation cannot be created through the registry. "
