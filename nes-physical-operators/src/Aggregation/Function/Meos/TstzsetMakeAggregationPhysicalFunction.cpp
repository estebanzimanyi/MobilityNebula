...
static std::mutex meos_tstzsetmake_mutex;


TstzsetMakeAggregationPhysicalFunction::TstzsetMakeAggregationPhysicalFunction(
    DataType inputType,
    DataType resultType,
...
void TstzsetMakeAggregationPhysicalFunction::lift(
...
void TstzsetMakeAggregationPhysicalFunction::combine(
...
Nautilus::Record TstzsetMakeAggregationPhysicalFunction::lower(
    const nautilus::val<AggregationState*> aggregationState, [[maybe_unused]] PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    // via the MEOS array constructor tstzset_make. The array pointer threads across
    // events as an opaque void* (Nautilus invoke ABI), sized to numberOfEntries.
    auto arrState = nautilus::invoke(
        +[](size_t n) -> void* { return malloc(sizeof(TimestampTz) * (n > 0 ? n : 1)); },
        numberOfEntries);
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        nautilus::invoke(
            ...
            { ((TimestampTz*) a)[i] = (TimestampTz) val; },
            arrState,
            arrIdx,
            value);
    ...
    auto boxStr = nautilus::invoke(
        ...
        {
            MEOS::Meos::ensureMeosInitialized();
            std::lock_guard<std::mutex> lock(meos_tstzsetmake_mutex);
            Set* s = tstzset_make((TimestampTz*) a, (int) n);
            free(a);
            if (!s) {
                return (char*)nullptr;
            }
            char* out = tstzset_out(s);
            free(s);
            return out;
        },
        arrState,
...
void TstzsetMakeAggregationPhysicalFunction::reset(const nautilus::val<AggregationState*> aggregationState, PipelineMemoryProvider&)
{
    nautilus::invoke(
...
size_t TstzsetMakeAggregationPhysicalFunction::getSizeOfStateInBytes() const
{
    return sizeof(Nautilus::Interface::PagedVector);
}

void TstzsetMakeAggregationPhysicalFunction::cleanup(nautilus::val<AggregationState*> aggregationState)
{
    nautilus::invoke(
        +[](AggregationState* pagedVectorMemArea) -> void
        {
...
AggregationPhysicalFunctionRegistryReturnType AggregationPhysicalFunctionGeneratedRegistrar::RegisterTstzsetMakeAggregationPhysicalFunction(
    AggregationPhysicalFunctionRegistryArguments)
{
    throw std::runtime_error("TstzsetMake aggregation cannot be created through the registry. "
